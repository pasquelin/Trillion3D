...
| Parcours, Hi-Z, binning et rasterisation hybride (`Obs.`) | `NaniteCullRaster.cpp`, `NaniteRasterizer.usf`, `packages/sdk-browser/{gpuSelection,gpuHiz,gpuDraw,visibilityBuffer}.ts` | Coupe résidente GPU consommée par la compaction parallèle et les dessins indirects ; raster matériel seulement | Étendre la rasterisation hybride ; conserver CPU comme oracle identifié | Capture GPU réelle, comparaison de coupe/ID/profondeur, mouvement de caméra et occlusion/révélation |
