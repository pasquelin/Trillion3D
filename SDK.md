...
`createGpuPageCache` is a bounded WebGPU buffer/queue adapter. When a WebGPU device is available, `createExplorer` adds `webgpuPagesBackend` (`webgpu-page-raster`): same Format 1 pages, GPU compute frustum + `lodScore` when compute pipelines exist (`capabilities.gpuDriven`), otherwise the CPU `selectVisiblePages` oracle silently. Vertex pulling from resident slots writes a visibility buffer (packed page+triangle IDs) then a second pass reconstructs attributes and applies source glTF materials (baseColor, maps, metalness/roughness). That second pass is not MeshStandardMaterial pixel-perfect; `runCameraPath` still allows `maxChannelError <= 2` when the renderer string contains WebGPU. GPU color is presented through a pooled blit (CPU `rasterVisibilityIds` / `shadeVisibility` are the test oracles). After this-frame's visbuffer of in-front occluders, a GPU Hi-Z pyramid is built from that pass's depth (r32float, background 1, max reduction) and the rest is retested; non-rejected pages are drawn with `loadOp: 'load'`, then shade once. Remaining ⊆ `selectVisiblePages`. The JS kernel (`hizRejects` / `applyHiz` / `rasterVisibility` depth) is the A/A oracle; a GPU non-rejection is not an error; previous-frame depth is not proof. Node tests do not execute WGSL. If compute Hi-Z is missing, CPU `applyHiz` remains the silent fallback. If `r32uint` vis targets or the shade pipeline cannot be created, the previous untextured page raster stays and Hi-Z stays off. Visible pages stream with `preload: 'visible'`. Silent Three.js fallback if the device is missing or lost; `dispose` destroys the device. Indirect draw is not implemented. The default displayed backend remains the WebGL2 reference.
