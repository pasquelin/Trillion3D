...
R1f. **Lots du chantier « maths sans Three », ordre et critères.** Lancement sur go de l'utilisateur, un Opus 5 par lot à périmètre disjoint, tests par Sonnet 5 après coup, `npm run validate` à la livraison, fusion locale avec preuve, jamais de push hors Validateur.

| Lot | Contenu                                                                                                                                                                                                                           | Critère de sortie                                                                                                                         |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| M1  | Socle vecteurs, matrices, quaternions, couleurs dans `sdk-core` (R1b), en réutilisant les calculs maison de R1a                                                                                                                   | banc d'équivalence contre Three vert, aucun import `three` dans le socle, 200 lignes par fichier, 0 clone                                 |
| M2  | Volumes : boîte, sphère depuis boîte (une implémentation pour les quatre copies), plans de frustum et test boîte/frustum (signatures sans Three), transformation de boîte, sphère/cône                                            | même banc, 0 pixel sur les campagnes, `tri = selected`                                                                                    |
| M3  | Hiérarchie de transformations et caméra maison (R1c) ; remplacement de tous les appels Three du chemin WebGPU par image                                                                                                           | 0 pixel (trois vues, caméra mobile, deux seuils), allocations par image nulles, test de structure : `three` interdit sur le chemin WebGPU |
