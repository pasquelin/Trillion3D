...
R5c. **Ce que la partition ne garantit pas.** Le partage occulteurs/testés décide de l'**ordre** de dessin, et sur une scène qui contient des surfaces opaques exactement coplanaires l'ordre décide l'image : à profondeur égale, le test `less` donne le pixel au premier dessiné. Mesuré sur Emerald, vue générale, seuil 0 : 2 287 pixels changent quand la comparaison passe à `less-equal`, et tout écart observé en changeant la partition est un sous-ensemble de ces 2 287. Tant que le compilateur ne départage pas ces surfaces (étape `coplanar-depth-layers-v1`, qui n'en marque aujourd'hui que quatre clusters), **aucun changement de partition ne peut être prouvé à 0 pixel** contre une référence rendue avec une autre partition. Ce n'est pas une propriété du test Hi-Z.
