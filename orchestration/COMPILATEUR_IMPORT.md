...
État actuel du code (15 sept. 2026, pause) : routeur à pilotes (`scene-plugin-2`, `image-plugin-1`), six pilotes de scène (`gltf`, `fbx` par ufbx, `obj`, `zip`, `unity`, `unitypackage`) et six pilotes d'image (`png`, `jpeg`, `tga`, `tiff`, `dds`, `webp` sans perte) ; un projet Unity est routé vers `unity` avec ses modèles ; une seule racine de résolution des images de l'import aux aperçus. Reprise : `orchestration/REPRISE_COMPILATEUR.md`.
