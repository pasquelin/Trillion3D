...
Résumé court ; le détail chiffré vit dans l'historique git (messages de commit), jamais recopié ici.
...
- **Ombres lointaines du soleil contre le proxy** : **livrées le 15 septembre 2026**, en deux lots. Au-delà de la dernière cascade, `sunShadowFactor` tire un rayon déterministe contre le proxy résident (lot « ombres lointaines du soleil », résolution différée opaque) ; depuis le lot « ombre lointaine sur la passe de mélange », la passe des transparents lie le même proxy et tire le même rayon par le même texte WGSL, le bouchon qui rendait 1 a disparu. Le proxy résident tient dans un seul tampon de stockage (entête de douze mots puis trois colonnes) pour rester sous les huit tampons garantis par étage. Sans proxy dans le cache, les deux passes éclairent sans ombre lointaine et le diagnostic `sun-far-shadow` le dit.
