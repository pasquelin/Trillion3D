...
Version 2. Complète `SPEC_MOTEUR_SANS_THREE.md`, qui reste la référence pour la géométrie. Chaque exigence est numérotée et vérifiable ; une exigence sans mesure associée n'existe pas. Les valeurs marquées « réglage » sont des choix de produit : on part avec, on mesure, on resserre.
...
P6. **Aucune lumière sans source déclarée** : ni ambiance fixe, ni « mode nuit » ; le jour est une lampe soleil ou ciel déclarée dans la scène, qui n'entre que par les ouvertures et projette des ombres — un couloir sans fenêtre reste noir en plein jour, sauf rebond indirect. Vaut pour tous les matériaux, transparents et feuillages compris. Critère : scénario couloir sans fenêtre, radiance nulle hors rebond indirect.
...
LR3. **Direct** : shadow maps par lampe (cascades pour les lampes à grande portée), ombres douces PCSS ; une carte n'est redessinée que si sa lampe ou un objet dans sa portée a bougé, mise en cache sinon ; elle ne reçoit que ce qui peut projeter une ombre, dans sa portée et sa face, sans réduction de résolution ni de détail. Un `alphaMode: MASK` projette sa découpe réelle (masque du matériau) dans la passe de profondeur ; un matériau de mélange (`alphaMode: BLEND`) ne projette encore rien. Ombres atténuées et colorées des semi-transparents (verre, eau) : lot à part, hors périmètre. Critère : contrat E composante « direct » ; retard nul.
...
E5. **Retard de réponse** après événement, mesuré comme le temps pour que l'erreur de la composante indirecte retombe sous le seuil E2 par rapport à l'état stable final : **cible 100 ms sur la machine de référence, limite 250 ms partout, 500 ms sur GPU intégré** (réglages, choisis sur les vidéos du banc 16). Au-delà de la limite : échec.
...
Ordre d'exécution des lots — prime sur l'ordre implicite du tableau ci-dessous, qui reste la référence pour les critères de sortie mesurés : ombres (LR3) → aucune lumière sans source déclarée (P6), opaques puis transparents → reflet pour toute surface (LR7) → lumière qui rebondit (LR4, multi-rebond) → reflets flous (spéculaire rugueux) → optimisation mesurée.
...
- Cadence fixe, convergence variable, même image finale.
...
- Aucune lumière sans source déclarée : suppression de l'ambiance fixe et du « mode nuit ».
- Reflet pour toute surface : mécanisme piloté par la propriété réfléchissante du matériau, réutilisable par toute surface, pas un objet miroir nommé.
- Eau : pas de lot dédié ; matériau réfléchissant, transparent, à relief animé, couvert par les règles existantes (LR7, transparence). Seule la réfraction avec absorption en profondeur reste une propriété à ajouter plus tard. Scène de test avec de l'eau à fournir par l'utilisateur.
...
Repères externes publics, en vocabulaire neutre, chacun avec ce qu'il change chez nous. Aucun ne se copie tel quel : chaque repère se rejoue sur nos propres scènes et son propre budget avant d'entrer dans le code.

RX1. **Champs d'irradiance + occlusion par sondes comme mode « léger »** : c'est déjà notre direction (LR4, LC5) — les sondes clairsemées et le cache de surfaces sont ce mode léger, pas une étape intermédiaire vers autre chose. Priorité : resserrer ce chemin (base d'ordre 2, budget en millisecondes) plutôt que d'ajouter un rebond tracé par pixel séparé.
RX2. **Éclairage stochastique par pixel** (un rayon d'ombre par pixel, accumulation temporelle, rejet des lampes hors portée) : porte des centaines de lampes ombrées sans une carte d'ombre par lampe. Le rejet des lampes hors portée par tuile est notre règle actuelle ; le tirage et l'accumulation temporelle restent un lot ouvert (section 12).
RX3. **Ombres virtualisées** : un cache d'ombre à budget d'invalidation différée, qui ne recalcule que les faces effectivement touchées par un changement. C'est déjà notre règle (LR3 : rejet par portée et par face, résidence, zéro travail en scène immobile). À étendre : remplacer le plafond de lampes (X5) par un budget temporel, comme X4 le demande pour le rebond.
RX4. **Résolution minimale des cartes du cache de surfaces abaissée** (repère externe : 4 → 2, sur l'axe le plus fin d'une carte). Chez nous, LC2 fixe une couverture (≥ 95 %) mais ne borne pas la résolution minimale d'une carte de projection. Paramètre à ajouter à LC2, à exposer côté compilateur, et à mesurer sur Emerald avant tout changement de défaut.

## 12. Restes et ordre des lots

Ordre proposé, prime sur l'ordre implicite de la section 8 : rebond 3 → reflet pour toute surface (LR7) → reflets flous → éclairage stochastique par pixel, historique en cible de rendu (RX2, reste) → optimisation mesurée.

- **Rebond 3** (`lot/rebond-3`) : cascades de sondes autour de la caméra, budget en millisecondes plutôt qu'en nombre de mailles et de rayons (X4, LR1), base d'harmoniques sphériques d'ordre 2, activation par défaut une fois le budget tenu.
- **Ombres des matériaux de mélange** : un cluster transparent n'obtient pas de ligne de visibilité (`webgpuRowSync.ts`), donc n'entre pas dans la table des lignes dessinées des cartes d'ombre et ne projette rien ; les masques projettent déjà leur découpe réelle par le chemin opaque. L'ombre atténuée et colorée d'un semi-transparent reste un lot à part entière.
- **Reflet pour toute surface** (LR7) : mécanisme piloté par la propriété réfléchissante du matériau, pas par un objet « miroir » nommé ; toute surface qui la déclare en bénéficie, l'eau comprise.
- **Reflets flous** (spéculaire rugueux) : hors périmètre de LR7 tel que livré, qui vise la réflexion nette.
- **Éclairage stochastique par pixel** (RX2, reste) : le rejet par tuile est livré, le tirage ne l'est pas. L'historique doit passer par une cible de rendu supplémentaire, en ping-pong, et non par un tampon de stockage (une écriture depuis le nuanceur de fragments coûte cher au tuileur, qui cesse d'éliminer les surfaces cachées). Il faut aussi un banc où des lampes atteignent vraiment les transparents, pas seulement un compte de lampes déclarées.
