...
- L fidelity: **next**. A03 P1 `simplification = none` still builds coarse DAG levels (`compiler_primitive_dag.rs:49`, `dag/groups.rs:116`); A04 P1 an OBJ texture added after a first compile stays absent with the existing cache (`import/runner.rs:71`, `import/textures.rs:103`, `import/external.rs:8`); A06 P1 nodes and lights of non-selected glTF scenes are compiled (`compiler_nodes.rs:17`, `compiler_lights.rs:165`).
- M cache identity: A05 P2 a changed PNG keeps the same key while `clusters.bin` changes (`compiler_build.rs:30`, `texture_preview.rs:155`); A13 P2 `importMs` hashed into the final key, three runs give three keys (`compiler_build.rs:30`, `blend/out.rs:142`).
