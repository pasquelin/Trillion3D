...
- N remaining inputs: **next**. A07 P2 unity merge does not remap `sparse.indices/values.bufferView` (nor `targets`) (`unity/merge.rs:71`); A08 P2 node animations dropped from `scene.gltf` while `autonomousScene` stays available (`compiler_autonomous.rs:11,68`); A09 P2 a node cycle (`children: [0]`) is accepted (`compiler_world.rs:132`); A10 P2 non-manifold edge smoothing depends on face order (`normals.rs:93`); A11 P2 USD `holeIndices` never read (`usd/mesh.rs:11`, `usd/surface.rs:64`); A12 P2 PNG `gAMA = 100000` reported `transfer=Srgb` (`png.rs:80`, `crate_image.rs:38`).
- K' cache lock (after N, before O), confirmed on `compiler_lock.rs` (69 lines, no PID check, `STALE` 6 h, wait loop without cancel): A18 P1 SIGKILL of the owner leaves `native/.lock`, every retry waits 30 s then `CACHE_LOCKED` until 6 h; A19 P2 cancel during the wait is ignored, exits `CACHE_LOCKED` instead of `CANCELLED`. Fix: OS-released file lock (released at process death, multi-OS) instead of a dated marker, never delete a possibly live lock; poll the cancel flag inside the wait. Proofs: `docs/AUDIT_COMPILATEUR_2026-09-16.md:153`.
