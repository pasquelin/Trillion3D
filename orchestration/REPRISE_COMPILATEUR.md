...
- D' fixes on merged batches: **in progress**, worktree `.claude/worktrees/lot-d2-regressions`, branch `compilateur/lot-d2-regressions`, reported by session « Régressions bloquantes develop » on 21a34f4b, each to reproduce in a test first: COMP-01 `ngon.rs:132-137` strict `> 0.0` lets a diagonal pass through an active corner (polygon [(-4,0),(1,-3),(-1,3),(-3,2),(-3,1)] area 13 → 14, last triangle flipped); COMP-02 `import/runner.rs:54-62`, `import/external.rs:155-174` base_key ignores the resolution directory, two identical OBJ with different MTL share one cache entry (`reused`, wrong colour); COMP-03 `usd/opacity.rs:35-46` `carried` compares prim paths only, `outputs:r` taken for the alpha channel. Reversion drivers whose output or cache identity changes.
...
- J partial properties: 19–22,33,34,39,42–52; respective ma/blend/unity/usd drivers.
