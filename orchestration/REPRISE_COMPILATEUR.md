...
- P verification leftovers (after O, confirmed by code reading): **in progress**, branch `compilateur/lot-p-verification` from develop. V01 P2 `normals.rs::edges()` filters sharp faces/corners before the incidence count, a three-face edge with one sharp face joins the other two (contract: >2 faces join nothing); V02 P2 `main.rs:45` pointer projection omits `pruneMs`, `sdk-node/index.mts::prepare` returns manifest metrics without the pointer's `wallMs`/`pruneMs`; V03 P2 `perf.rs` `Timer` is `Instant` elapsed time, documented and named as CPU (`phaseCpuMs`). Proofs: `docs/VERIFICATION_COMPILATEUR_8A293DAE_2026-09-16_PREUVES.zip`.
