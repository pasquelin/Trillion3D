...
- M cache identity: **next**. A05 P2 a changed PNG keeps the same key while `clusters.bin` changes (`compiler_build.rs:30`, `texture_preview.rs:155`); A13 P2 `importMs` hashed into the final key, three runs give three keys (`compiler_build.rs:30`, `blend/out.rs:142`).
- N remaining inputs: A07 P2 unity merge does not remap `sparse.indices/values.bufferView` (nor `targets`) (`unity/merge.rs:71`); A08 P2 node animations dropped from `scene.gltf` while `autonomousScene` stays available (`compiler_autonomous.rs:11,68`); A09 P2 a node cycle (`children: [0]`) is accepted (`compiler_world.rs:132`); A10 P2 non-manifold edge smoothing depends on face order (`normals.rs:93`); A11 P2 USD `holeIndices` never read (`usd/mesh.rs:11`, `usd/surface.rs:64`); A12 P2 PNG `gAMA = 100000` reported `transfer=Srgb` (`png.rs:80`, `crate_image.rs:38`).
