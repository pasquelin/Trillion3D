# Reprise — WebGeometry sans Three.js (15 septembre 2026, ~15 h 45, session Geometry en pause)
...
## En cours à la pause
- `lot/temoin-three` (worktree `.claude/worktrees/lot-temoin-three`, tête 75e620c sur 3657c91, portes vertes, harnais seulement) : le témoin Three.js reçoit les lampes du contrat (`SceneLight`) via `sceneLighting` de `createExplorer`, `--moteur-avant`/`--moteur-apres` donnent un moteur par côté, l'écart avant/après devient le chiffre de fidélité par vue. Limite : le témoin ne projette pas d'ombres (campagne `--ombres off` des deux côtés). Reste : UNE campagne sous verrou (`--moteur webgpu --moteur-avant webgl --vues generale,sol,rue --soleil --ombres off --profil off` sur la scène `transmission` des fixtures, côté avant = témoin Three, côté après = moteur), images dans `.mesure/out/lot-temoin-three`, tableau px / écart canal au journal, fusion ff (aucun préavis nécessaire : pas de sdk-browser). Voir la section « Résultat témoin » ci-dessous si elle existe.
- `docs/geometry-reprise` (worktree `.claude/worktrees/docs-geometry`, 95cfcf3 + ce fichier) : dates du journal corrigées, 0 mot interdit ; à fusionner ff.
