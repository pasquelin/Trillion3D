...
Ambition : un rendu de géométrie virtualisée de niveau AAA dans le navigateur. Les mots « Nanite » et « Unreal » sont **interdits** dans tout le projet (code, commentaires, docs, tests, messages de commit). On dit « géométrie virtualisée », « DAG de clusters ».
...
5. **Portes de validation obligatoires avant toute fusion** : `npm test`, `npm run build`, `cargo test --release --locked --manifest-path packages/asset-compiler-rust/Cargo.toml`, `npm run build:native`, `check:structure`/`check:dts`, `grep -ri nanite` = 0 et `grep -ri unreal` = 0 ; puis preuve navigateur : `walk.mjs` WebGL et WebGPU (`PRELOAD=all`) sans `firstError`, `shots.mjs` avec `MAX_PAGES=4096` donnant `tri = selected` (aucun trou), et captures PNG **identiques** à la référence (pixels différents = 0, ou au niveau du bruit A/A mesuré avec témoin A/A, erreur max ≤ 2 par canal, expliqué). Sinon : pas de fusion.
