...
- Le validateur est la **seule** session qui lance `npm run validate` (consigne de l'utilisateur, 15 septembre 2026, 19 h 50) : aucune autre session ni aucun agent ne le lance, sur aucun worktree. Les sessions livrent avec tests ciblés, `npm run check:changed` et banc ; le validate se fait à la fusion.
