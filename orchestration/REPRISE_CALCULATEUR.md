...
1. **Third guard rule**: `pageCone.ts` and `visibilityShadingNormal.ts` now use `normalMatrix3` from the foundation (no Three); the guard question (absolute threshold vs normalised determinant) is unchanged and still to settle by convention.
