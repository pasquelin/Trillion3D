...
- Verrou de mesure `.claude/mesure.lock` : `mkdir` sans `-p`, verrou laissé vide ; échec = on n'écrit rien et on attend ; `rmdir` en fin, puis « verrou rendu » à qui attendait. Jamais de boucle d'attente.
...
- Liste de travail des appels Three restants : lot T1 de `orchestration/AUDIT_MATH_FORMULES.md`.
