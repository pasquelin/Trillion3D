// Static harness server, for `banc.ts`. Nothing is written here: the server
...
// by esbuild on the way out, so the page modules of `scripts/mesure/` and `test/appui/` are served
