...
- `--textures cache|host` (default `host`): whether the glTF loader opens the source images. `cache` skips every image whose chain the cache carries; `host` decodes them all, which the Three witnesses need. The engine reads the baked levels either way (#289), so the two sides render the same image and differ only in what the loader fetches — the harness keeps `host` by default because a side may be a witness.
