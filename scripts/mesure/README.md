...
Finer than the stages, `series[].sides[].bornesCpu` keeps every `cpu-timing` diagnostic the engine published during the series, in order: `image` is the index of the measured frame it arrived on (`null` during warm-up), `echantillon` the one-frame sample every path publishes (`totalMs`, `lightsMs`, `selectionMs`), and `etapes` the p50/p95/max of each named CPU bound (`gateMs`, `worldMs`, `selectionDispatchMs`, …) over the `images` rendered since the previous report — `null` on the CPU reference cut, which keeps no step profile. The engine publishes at most one report every two seconds: a measured loop shorter than that may carry none of its own, and a report's window may start before the loop. `null` when the engine published nothing.
