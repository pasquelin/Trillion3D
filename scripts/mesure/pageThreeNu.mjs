...
// page (montage `/mesure/`) et n'importe que `three` et son `GLTFLoader`, depuis `/vendor/three/`,
// donc depuis les dépendances de dev du banc — jamais du moteur. Le jour où les adaptateurs Three
// quittent `packages/`, cette page ne bouge pas : c'est ce qui la rend indépendante.
//
// Ce qu'il rend : le glTF source chargé tel quel, `MeshStandardMaterial` de Three, tout dessiné à
// chaque image sans sélection ni diffusion, les lampes du contrat posées en Three (soleil en
// `DirectionalLight` avec UNE carte d'ombre couvrant le modèle, ponctuelles en `PointLight` avec
// leur cube d'ombre), ACES et sRGB comme le moteur. Ce qu'il ne rend pas, nommé : ni cascades, ni
// antialiasing temporel, ni rebond, ni instances.
//
...
// Chrome ; une seule durée, jamais une somme), l'intervalle rAF en boucle de
// profil, les appels et triangles de `renderer.info`, les octets de géométrie et de textures qu'il
// tient, la préparation et le réseau, et sa capture pour l'écart en pixels. Aucun temps GPU par
// passe : WebGL ne l'expose pas dans Chrome, et le relevé le dit par `null`.
...
export async function measureView(options) {
  if ((options.instances ?? 1) !== 1)
    return { erreur: 'le témoin Three nu ne pose pas d’instances' };
