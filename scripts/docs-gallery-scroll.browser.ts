...
test('gallery progressively loads a bounded window and restores navigation state', async () => {
  ...
  try {
    ...
    const before = await page.evaluate(() => ({
      y: scrollY,
      count: document.querySelector<HTMLElement>('[data-progressive-list]')?.dataset.mountedItems,
    }));
    await page.evaluate(() => {
    ...
    await page.waitForFunction((y: number) => Math.abs(scrollY - y) < 80, before.y);
    assert.equal(await list.getAttribute('data-mounted-items'), before.count);

    await page.evaluate(() => scrollTo(0, document.documentElement.scrollHeight));
    await page.waitForFunction(() => {
      const pages = [...document.querySelectorAll<HTMLElement>('[data-progressive-page]')];
    ...
    assert.ok((await page.locator('[data-progressive-page]').count()) <= 3);
    const firstPageBox = await page.locator('[data-progressive-page]').first().boundingBox();
    assert(firstPageBox);
    assert.equal(firstPageBox.width < 390, true);
  } finally {
    await browser.close();
