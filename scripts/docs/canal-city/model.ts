import { box, combine, mesh, triangle, type Mesh } from '../../../site/lessons/offline/mesh.ts';

type MaterialKey = 'brick' | 'ochre' | 'stone' | 'slate' | 'glass' | 'water' | 'metal';
type Roof = 'setback' | 'gable' | 'flat';
type Groups = Record<MaterialKey, Mesh[]>;
...
const add = (out: Groups, material: MaterialKey, center: number[], size: number[]) =>
  out[material].push(box(center, size));

function roofPrism(center: number[], size: number[]) {
...
function canalWindow(out: Groups, side: number, facade: number, y: number, z: number) {
  add(out, 'glass', [facade - side * 0.035, y, z], [0.08, 0.42, 0.34]);
  add(out, 'stone', [facade - side * 0.09, y + 0.27, z], [0.2, 0.08, 0.46]);
}
function endWindow(out: Groups, x: number, y: number, facadeZ: number, direction: number) {
  add(out, 'glass', [x, y, facadeZ + direction * 0.035], [0.36, 0.42, 0.08]);
  add(out, 'stone', [x, y + 0.27, facadeZ + direction * 0.09], [0.48, 0.08, 0.2]);
}

/** Configuration for one canal-front building, read where `canalCity` authors the district. */
interface CanalBuildingConfig {
  side: number;
  z: number;
  width: number;
  depth: number;
...
function canalBuilding(
  out: Groups,
  { side, z, width, depth, floors, material, roof }: CanalBuildingConfig,
) {
...
function warehouse(out: Groups, side: number, z: number, width: number) {
  const facade = side * 2.35,
...
export function canalCity() {
  ...
  const buildings: Array<[number, number, number, number, number, MaterialKey, Roof]> = [
