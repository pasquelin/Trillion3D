import { bone, box, ellipsoid, merge } from './geometry.mjs';

const add = (groups, material, shape) => groups[material].push(shape);
const curvedBone = (groups, points, radius) => {
  for (let index = 1; index < points.length; index++)
    add(groups, 'bone', bone(points[index - 1], points[index], radius));
};

function skeleton(groups) {
...
export function fossilExcavation() {
  const groups = Object.fromEntries(
    ['bone', 'shadow', 'sandstone', 'ochre', 'markerRed', 'markerBlue', 'paper', 'metal'].map(
      (name) => [name, []],
    ),
  );
  skeleton(groups);
  excavation(groups);
  return Object.fromEntries(Object.entries(groups).map(([name, parts]) => [name, merge(parts)]));
}
