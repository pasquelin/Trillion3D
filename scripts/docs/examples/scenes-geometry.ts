...
import { TAU, random, slab, workshop } from './workshop.ts';

const STONE: MaterialRow = ['Grey stone', [0.55, 0.53, 0.5, 1], 0, 0.8],
  SAND: MaterialRow = ['Sand', [0.68, 0.6, 0.45, 1], 0, 0.9];

/** Ten thousand pebbles, one mesh placed ten thousand times, scattered over a sand bed. */
function pebbles(): PartsScene {
  const pebble = workshop(),
    bed = workshop(),
    next = random(7),
    nodes: SceneNode[] = [{ name: 'bed', part: 'bed' }];
...
function vase(): SurfacesScene {
  const shop = workshop(),
    profile: [number, number][] = [[0, 0]];
...
function staircase(): PartsScene {
  const step = workshop(),
    column = workshop(),
    nodes: SceneNode[] = [{ name: 'column', part: 'column' }],
    sector = TAU / 22,
    tread = (u: number, v: number, y: number): Vec3 => {
...
function relief(): SurfacesScene {
  const shop = workshop(),
    ...
    glyphs: Record<string, readonly [number, number, number, number][]> = {
      W: [
        [0, 0, 1, 6],
        [4, 0, 1, 6],
        [1, 6, 1, 1],
...
function boxes(): PartsScene {
  const box = workshop(),
    ground = workshop(),
    nodes: SceneNode[] = [{ name: 'ground', part: 'ground' }];
