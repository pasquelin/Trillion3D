...
import { canonicalizeFossilCache } from './docs/fossil-excavation/canonical.ts';

interface FossilManifestPointer {
  url: string;
}

interface FossilCacheManifest {
  metrics: { compileMs: number | null; importMs: number | null };
  sourceTriangles: number;
  cutouts: { sheet: string };
}
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const manifest = JSON.parse(first.toString()) as FossilCacheManifest;
