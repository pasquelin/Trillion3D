...
# "Lead verification" is required unless PR_DRAFT=true: a draft waits for its lead.
# Usage: [PR_DRAFT=true] check-pr-body.sh < body  (the CI feeds it the pull request body).
set -u
body=$(perl -0pe 's/<!--.*?-->//gs')
