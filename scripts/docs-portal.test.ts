...
test('the header marks the area of the route, the lessons under Learn, the editor apart', () => {
  ...
  assert.deepEqual(
    ...
    ['learn', 'examples', 'editor', 'api', 'reports'],
  );
});
...
test('page resolution distinguishes entries, lessons, and unknown addresses', () => {
  ...
  assert.equal(
    resolvePage({ locale, area: 'examples', id: 'scene-editor' }, entries, [], ['scene-editor'])
      .kind,
    'editor',
  );
  assert.equal(
