...
5. Obtain an independent review and resolve its findings before integration. The maintainer, or
   ...
   verification) and `validate` is green on a head up to date with `develop`; `main` moves only
   on the maintainer's word. Never push directly to `develop` or `main`, or rewrite published
   history.
