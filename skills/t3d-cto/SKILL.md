...
- **Subscription usage.** At every pass read the plan usage (the session-management `get_usage`
  tool when present). At **75 %**, tell every lead to start no coder on an issue one run cannot
  finish. At **80 %** of the window (or the threshold the boss sets), start winding
