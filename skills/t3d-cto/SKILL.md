...
- **Subscription usage.** At every pass read the plan usage (the session-management `get_usage`
  tool when present). Five points below the threshold, tell the leads the first step of AGENTS.md §Roles. At **80 %** of the window (or the threshold the boss sets), start winding
