...
export const FORMAT_GUIDES: PortalEntry[] = [
  {
    ...GUIDE,
    id: 'cluster-format',
...
<li><strong>What is not written.</strong> Tangents: every lighting pass rebuilds one cotangent frame from the triangle (<code>cotangentFrame</code>, WGSL and GLSL); and a texture coordinate set that no texture of the material names in <code>texCoord</code>.</li>
</ul>
<p>The cost is declared, never hidden. Each page carries its worst position displacement in its header, and every decoder returns it — the autonomous backend widens a page's box by exactly that; each primitive publishes <code>quantization</code> — <code>positionExponent</code>, <code>uvExponent</code>, <code>maxPositionError</code> — in the manifest for reporting, and each page descriptor names its resident bytes (<code>geometry.bytes</code>) beside what its float decode occupies (<code>geometry.uncompressedBytes</code>). The cut does not yet add the quantization error to a cluster's error band; the specification's C4 keeps that open.</p>
<h3 class="text-lg font-bold mt-4">How to observe it</h3>
<p>On a live world, a loaded model's <code>metadata.primitives</code> is the opened manifest: sum <code>pages[].geometry.bytes</code> over <code>pages[].count / 3</code> triangles for the packed figure, <code>uncompressedBytes</code> for the float one, and read the largest <code>quantization.maxPositionError</code>. In the repository, <code>node --experimental-strip-types bench/runner/octetsParTriangle.ts &lt;cache&gt;/native/full</code> prints those figures for any compiled cache. These are the cache's figures: the WebGPU drawing engine still uploads the source float vertices and index pages, and its geometry pool says so.</p>
<h3 class="text-lg font-bold mt-4">Contract and refusals</h3>
<p>The manifest declares its page format once, at its top: <code>geometryPages.formatVersion</code> is 3 and <code>codec</code> is <code>quantized</code>; the binary sidecar that names them is version 7, every page header opens with the same version, and a reader refuses another format whole rather than page by page. A page whose header leaves the format — a width above 24 bits, an exponent beyond ±64, an unknown flag, a length that does not match its streams — is refused before any stream is read; an index at or past the vertex count is refused before any float is produced. The WGSL routines are proven on the graphics card against the JavaScript decoder, bit for bit on positions, texture coordinates and colours (<code>tests/browser/probes/decodage-cluster-gpu.ts</code>).</p>`,
  },
];
