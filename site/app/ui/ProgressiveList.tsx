...
export function ProgressiveList<T>({
  ...
  useEffect(() => {
    onStateChange?.({ ...range, heights });
  }, [heights, onStateChange, range]);

  useEffect(() => {
    // The shell's content area is what scrolls, not the window.
    ...
    const recover = () => {
      ...
      const jump = Math.abs(scrollY - lastScrollY.current);
      lastScrollY.current = scrollY;
      const atEnd = area.clientHeight + scrollY >= area.scrollHeight - 2;
      ...
      const next =
        atEnd && jump > average * 4
          ...
          : rangeForScroll(
              scrollY,
              node.getBoundingClientRect().top - area.getBoundingClientRect().top + scrollY,
              heights,
              pageCount,
              maxBatches,
            );
    ...
    <div ref={root} data-progressive-list data-mounted-items={mountedItems}>
      ...
      {pages.map((page) => (
        <div
          ...
          className="grid grid-cols-1 gap-5 pb-5 sm:grid-cols-2 xl:grid-cols-3"
        >
