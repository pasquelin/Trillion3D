...
export function comparison(a, b, metric, variable = 'engine') {
  if (!a || !b || a.id === b.id) return { status: 'choose', delta: null, percent: null };
  const reasons = [];
  ...
  const conditions = ['scene', 'view', 'quality', 'pose', 'canvas', 'pathVersion', 'assetKey'];
  ...
  if (reasons.length)
    return { status: 'incompatible', reasons: [...new Set(reasons)], delta: null, percent: null };
  return {
    status: 'descriptive',
    reasons: [],
    delta: right - left,
