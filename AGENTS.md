...
- Verrou de mesure `.claude/mesure.lock`, commun à toutes les sessions, pour tout `validate`, banc ou campagne : une seule chaîne `mkdir .claude/mesure.lock && echo "<session> <travail> <heure>" > .claude/mesure.lock/proprietaire && { travail; rm .claude/mesure.lock/proprietaire; rmdir .claude/mesure.lock; }`, `mkdir` sans `-p`. Si `mkdir` échoue, ne rien écrire et attendre par une seule commande d'arrière-plan `until mkdir …; do sleep 20; done`, jamais une boucle `pgrep`. Pas de `mv` de dossier pour prendre le verrou : sur un verrou existant, il s'y loge au lieu d'échouer. On ne retire que son propre verrou ; un verrou sans `proprietaire` depuis plus d'une minute est signalé au Validateur, qui interroge les sessions avant de le retirer.
