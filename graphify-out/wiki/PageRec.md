# PageRec

> God node · 91 connections · `packages/sdk-browser/pageSelectionTypes.ts`

**Community:** [Adoption et reprise de la coupe](Adoption_et_reprise_de_la_coupe.md)

## Connections by Relation

### contains
- pageSelectionTypes.ts `EXTRACTED`

### imports
- webgpuPageRow.ts `EXTRACTED`
- webgpuPagesHelpers.ts `EXTRACTED`
- exactPagesBackend.ts `EXTRACTED`
- webgpuPagesPipelineFor.ts `EXTRACTED`
- webgpuPagesRenderCpu.ts `EXTRACTED`
- webgpuVisibilityDrawLayers.test.ts `EXTRACTED`
- webgpuPagesLayersState.test.ts `EXTRACTED`
- webgpuPagesTestOccluder.ts `EXTRACTED`
- webgpuPagesStateRun.ts `EXTRACTED`
- autonomousPages.ts `EXTRACTED`
- webgpuPagesServices.ts `EXTRACTED`
- texturePriorityMeasure.ts `EXTRACTED`
- pageSelectionCollect.ts `EXTRACTED`
- pageRaster.ts `EXTRACTED`
- webgpuPagesHostApi.ts `EXTRACTED`
- exactPagesRender.ts `EXTRACTED`
- gpuDagTotals.test.ts `EXTRACTED`
- mathBatchBoxes.ts `EXTRACTED`
- webgpuTransparentTable.test.ts `EXTRACTED`
- exactPagesRequests.ts `EXTRACTED`
- *…and 68 more `imports` connection(s) not listed (lowest-degree first to go)*

### re_exports
- pageSelection.ts `EXTRACTED`

### references
- WebgpuRunState `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*