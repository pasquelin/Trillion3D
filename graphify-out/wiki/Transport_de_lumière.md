# Transport de lumière

> 43 nodes

## Key Concepts

- **lightingTransport.ts** (28 connections) — `packages/sdk-core/lightingTransport.ts`
- **lightingTransportValidation.ts** (22 connections) — `packages/sdk-core/lightingTransportValidation.ts`
- **fail()** (21 connections) — `packages/sdk-core/lightingTransportValidation.ts`
- **lightingTransportIntersections.ts** (19 connections) — `packages/sdk-core/lightingTransportIntersections.ts`
- **lightingTransportGeometry.ts** (16 connections) — `packages/sdk-core/lightingTransportGeometry.ts`
- **lightingTransportState.ts** (15 connections) — `packages/sdk-core/lightingTransportState.ts`
- **lightingTransportVisibility.ts** (15 connections) — `packages/sdk-core/lightingTransportVisibility.ts`
- **createTransport()** (14 connections) — `packages/sdk-core/lightingTransport.ts`
- **lightingTransportContracts.ts** (14 connections) — `packages/sdk-core/lightingTransportContracts.ts`
- **lightingTransportOracle.ts** (13 connections) — `packages/sdk-core/lightingTransportOracle.ts`
- **checkpoint()** (12 connections) — `packages/sdk-core/lightingTransportValidation.ts`
- **lightingTransportSolve.ts** (12 connections) — `packages/sdk-core/lightingTransportSolve.ts`
- **lightingTransportSolve.test.ts** (11 connections) — `packages/sdk-core/lightingTransportSolve.test.ts`
- **progress()** (10 connections) — `packages/sdk-core/lightingTransportValidation.ts`
- **validateScene()** (10 connections) — `packages/sdk-core/lightingTransportValidation.ts`
- **solveTransportOracle()** (9 connections) — `packages/sdk-core/lightingTransportOracle.ts`
- **updateTransportVisibility()** (9 connections) — `packages/sdk-core/lightingTransportVisibility.ts`
- **lightingTransport.test.ts** (9 connections) — `packages/sdk-core/lightingTransport.test.ts`
- **TransportOptions** (8 connections) — `packages/sdk-core/lightingTransportContracts.ts`
- **updateTransportGeometry()** (8 connections) — `packages/sdk-core/lightingTransportGeometry.ts`
- **packSurface()** (8 connections) — `packages/sdk-core/lightingTransportIntersections.ts`
- **solveTransport()** (8 connections) — `packages/sdk-core/lightingTransportSolve.ts`
- **update()** (7 connections) — `packages/sdk-core/lightingTransport.ts`
- **LightingTransportError** (6 connections) — `packages/sdk-core/lightingTransportContracts.ts`
- **maximumResidual()** (5 connections) — `packages/sdk-core/lightingTransportResidual.ts`
- *... and 18 more nodes in this community*

## Relationships

- [Transport de lumière (2)](Transport_de_lumière_2.md) (13 shared connections)
- [sdk-core · bitangent](sdk-core_·_bitangent.md) (9 shared connections)
- [Scène d'expérience d'éclairage](Scène_d'expérience_d'éclairage.md) (9 shared connections)
- [Sélection DAG et pages WebGPU](Sélection_DAG_et_pages_WebGPU.md) (4 shared connections)
- [Intégration des pages arrivées](Intégration_des_pages_arrivées.md) (3 shared connections)
- [Socle des consommateurs](Socle_des_consommateurs.md) (3 shared connections)
- [sdk-core · faceScratch](sdk-core_·_faceScratch.md) (2 shared connections)
- [bench · longueurs](bench_·_longueurs.md) (2 shared connections)
- [oracles · referenceLinearToSrgb8](oracles_·_referenceLinearToSrgb8.md) (2 shared connections)
- [Vecteurs et normale d'ombrage](Vecteurs_et_normale_d'ombrage.md) (1 shared connections)

## Source Files

- `packages/sdk-core/lightingTransport.test.ts`
- `packages/sdk-core/lightingTransport.ts`
- `packages/sdk-core/lightingTransportContracts.ts`
- `packages/sdk-core/lightingTransportGeometry.ts`
- `packages/sdk-core/lightingTransportIntersections.ts`
- `packages/sdk-core/lightingTransportOracle.ts`
- `packages/sdk-core/lightingTransportResidual.ts`
- `packages/sdk-core/lightingTransportSolve.test.ts`
- `packages/sdk-core/lightingTransportSolve.ts`
- `packages/sdk-core/lightingTransportState.ts`
- `packages/sdk-core/lightingTransportValidation.ts`
- `packages/sdk-core/lightingTransportVisibility.ts`

## Audit Trail

- EXTRACTED: 199 (99%)
- INFERRED: 3 (1%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*