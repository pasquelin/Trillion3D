# Parcours des objets Alembic

> 17 nodes

## Key Concepts

- **walk.rs** (12 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **World** (10 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **Object** (9 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/archive.rs`
- **.mesh_node()** (9 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **.facesets()** (8 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **.top()** (8 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **.xform_node()** (7 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **Result** (6 connections)
- **.visit()** (6 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **.animated()** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`
- **geometry** (4 connections)
- **Kind** (3 connections)
- **Properties** (2 connections)
- **Scene** (2 connections)
- **AtomicBool** (1 connections)
- **Option** (1 connections)
- **xform** (1 connections)

## Relationships

- [Archive Alembic](Archive_Alembic.md) (5 shared connections)
- [Archive Alembic (2)](Archive_Alembic_2.md) (3 shared connections)
- [Géométrie Alembic](Géométrie_Alembic.md) (3 shared connections)
- [Maillages et jeux de faces](Maillages_et_jeux_de_faces.md) (2 shared connections)
- [Oracle : rayons et indirect](Oracle_-_rayons_et_indirect.md) (2 shared connections)
- [alembic · AtomicBool](alembic_·_AtomicBool.md) (1 shared connections)
- [import · Option (2)](import_·_Option_2.md) (1 shared connections)
- [Propriétés Alembic](Propriétés_Alembic.md) (1 shared connections)
- [scene · .accepts_head](scene_·_.accepts_head.md) (1 shared connections)
- [Erreurs du compilateur et USDZ](Erreurs_du_compilateur_et_USDZ.md) (1 shared connections)
- [Import Unity et maillages](Import_Unity_et_maillages.md) (1 shared connections)

## Source Files

- `packages/asset-compiler-rust/src/plugins/scene/alembic/archive.rs`
- `packages/asset-compiler-rust/src/plugins/scene/alembic/walk.rs`

## Audit Trail

- EXTRACTED: 52 (91%)
- INFERRED: 5 (9%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*