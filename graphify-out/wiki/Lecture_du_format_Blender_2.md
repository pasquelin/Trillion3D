# Lecture du format Blender (2)

> 30 nodes · cohesion 0.13

## Key Concepts

- **At** (54 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/view.rs`
- **Scene** (13 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **active.rs** (7 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **.mesh()** (7 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **.slots()** (7 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **objects()** (6 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **.object()** (6 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **short()** (6 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **excluded()** (5 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **.material()** (5 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **gather()** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **layers()** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **HashSet** (4 connections)
- **walker.rs** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **Option** (4 connections)
- **.light()** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **.overridden()** (4 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **pointer_at()** (3 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`
- **view.rs** (2 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/view.rs`
- **Result** (2 connections)
- **Layout** (1 connections)
- **EXCLUDE** (1 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **MAX_DEPTH** (1 connections) — `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- **BlendFile** (1 connections)
- **Option** (1 connections)
- *... and 5 more nodes in this community*

## Relationships

- [Lecture du format Blender (3)](Lecture_du_format_Blender_3.md) (11 shared connections)
- [Lecture du format Blender](Lecture_du_format_Blender.md) (9 shared connections)
- [Blender : pointeurs bruts](Blender_-_pointeurs_bruts.md) (7 shared connections)
- [Lecture du format Blender (4)](Lecture_du_format_Blender_4.md) (6 shared connections)
- [blend · Layout](blend_·_Layout.md) (5 shared connections)
- [Budget et ouvriers du compilateur](Budget_et_ouvriers_du_compilateur.md) (3 shared connections)
- [blend · ALPHA_CHANNEL](blend_·_ALPHA_CHANNEL.md) (2 shared connections)
- [blend · .face](blend_·_.face.md) (2 shared connections)
- [blend · .bools](blend_·_.bools.md) (1 shared connections)
- [tests · HEADER](tests_·_HEADER.md) (1 shared connections)
- [Identité du compilateur et purge](Identité_du_compilateur_et_purge.md) (1 shared connections)
- [unity · Fn](unity_·_Fn.md) (1 shared connections)

## Source Files

- `packages/asset-compiler-rust/src/plugins/scene/blend/active.rs`
- `packages/asset-compiler-rust/src/plugins/scene/blend/view.rs`
- `packages/asset-compiler-rust/src/plugins/scene/blend/walker.rs`

## Audit Trail

- EXTRACTED: 104 (99%)
- INFERRED: 1 (1%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*