# Manifeste binaire (Rust) (2)

> 5 nodes · cohesion 0.40

## Key Concepts

- **split()** (5 connections) — `packages/asset-compiler-rust/src/manifest_binary.rs`
- **Templates** (4 connections) — `packages/asset-compiler-rust/src/manifest_binary.rs`
- **Result** (1 connections)
- **TexturePreview** (1 connections)
- **Value** (1 connections)

## Relationships

- [Manifeste binaire (Rust)](Manifeste_binaire_Rust.md) (2 shared connections)
- [Encodage du manifeste binaire](Encodage_du_manifeste_binaire.md) (2 shared connections)

## Source Files

- `packages/asset-compiler-rust/src/manifest_binary.rs`

## Audit Trail

- EXTRACTED: 8 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*