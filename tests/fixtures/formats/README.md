...
(generated by `test/assets/tools/model_formats_blender.py`, trillion3D corpus, 15 September 2026,
...
`test/assets/blend/procedural-materials` (trillion3D corpus, procedural generation, 15 September
...
- `tronque.bmp` is taken as-is from `test/assets/limites/truncated-bmp/truncated.bmp`, also
  **CC0-1.0** (trillion3D corpus). The `test/assets/` folder is shipped off git: the file is
...
- `basis.ktx2` is copied as-is from `test/assets/textures/ktx2-matrix/basis.ktx2`, trillion3D
...
- `tronque.tga` is taken as-is from `test/assets/limites/truncated-tga/truncated.tga`, also
  **CC0-1.0** (trillion3D corpus, generated by `test/assets/tools/texture_assets.py`). The
  `test/assets/` folder is shipped off git: the file is copied here so the golden does not depend on it.
...
- `tronque.tif` is taken as-is from `test/assets/limites/truncated-tif/truncated.tif` (trillion3D
...
(generated by `test/assets/tools/unity_assets.py`, trillion3D corpus, 15 September 2026). The repository
...
- `test.unitypackage` and `hors-paquet/`: trillion3D corpus
  ...
  **CC0-1.0**, see [LICENSE.txt](unitypackage/LICENSE.txt). `test/assets/` is not tracked by git: these bytes
...
- `tronque.unitypackage`: trillion3D corpus (`test/assets/limites/truncated-unitypackage`),
  **CC0-1.0**, same notice.
...
- `corpus/`: trillion3D corpus (`test/assets/usd/procedural-usda` and `procedural-usdc`),
  **CC0-1.0**, see [LICENSE.txt](usd/LICENSE.txt). `test/assets/` is not tracked by git: these bytes
...
- `scene.usdz`: trillion3D corpus (`test/assets/usd/procedural-usdz`), **CC0-1.0**, see
  [LICENSE.txt](usdz/LICENSE.txt). `test/assets/` is not tracked by git: these bytes are copied here
...
- `sans-perte.webp` and `avec-perte.webp` are taken as-is from
  `test/assets/textures/legacy-web-matrix/` (`lossless.webp`, 184 bytes, and `lossy.webp`), trillion3D
  corpus produced by `test/assets/tools/texture_assets.py`, **CC0-1.0**, see
  [LICENSE.txt](webp/LICENSE.txt). `test/assets/` is shipped off git: the bytes are copied here so
...
- `scene.zip`, `tronquee.zip` and `hors-archive/`: trillion3D corpus
  ...
  [LICENSE.txt](zip/LICENSE.txt). `test/assets/` is not tracked by git: these bytes are copied here
  so the golden holds without it.
