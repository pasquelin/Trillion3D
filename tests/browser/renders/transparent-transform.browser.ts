...
//
// The page (`tests/browser/support/transparentTransformPage.ts`) is bundled by esbuild and run in Chromium
// with a real WebGPU device. Nothing is simulated: `webgpuPagesBackend`, its passes and its image readout.
//
//   node --experimental-strip-types tests/browser/renders/transparent-transform.browser.ts
...
interface EtapeTransparentTransform {
  name: string;
  rouge: boolean[];
  dessins: number;
  rejetes: number;
...
interface Resultat extends ResultatPagePreuve {
  adaptateur?: string;
  passes: Record<string, EtapeTransparentTransform[]>;
}

const resultat = (await preuveDansLaPage(
  'transparentTransformPage.ts',
...
for (const [passe, etapes] of Object.entries(resultat.passes)) {
  const de = (nom: string) =>
    etapes.find((e) => e.name === nom) ?? assert.fail(`${passe}: ${nom} missing`);
  const dit = (nom: string, message: string) => `${passe} / ${nom}: ${message}`;
  ...
  const stables = etapes.filter((e) => e.name.startsWith('stabilisation-'));
  assert.ok(
