...
//
//   node --experimental-strip-types tests/browser/renders/hiz-deux-passes.browser.ts
...
interface Resultat extends ResultatPagePreuve {
  adaptateur?: string;
  etapes: EtapeHizDeuxPasses[];
}

const resultat = (await preuveDansLaPage(
  'hizDeuxPassesPage.ts',
  'hizDeuxPasses',
  'Two-phase Hi-Z',
)) as Resultat;
console.log(
