...
// `SUBSTITUTIONS` (`lightingNormalGpu.ts`) alters `xformNormal`'s output at the exact
...
// protect nothing in `tests/browser/renders/normal-transform-arithmetic.browser.ts` — that is
...
//
// node --experimental-strip-types tests/browser/renders/lighting-normal-refused-substitutions.browser.ts
import assert from 'node:assert/strict';
import { verdictNormale } from '../probes/inverseTransposeF32.ts';
import {
  APLATIES,
  CAS,
  DECROCHE_DEG,
  EFFONDREES,
  REGULIERE_MINUSCULE,
} from '../probes/normalTransformCases.ts';
import { SUBSTITUTIONS, eclairageGpu } from '../probes/lightingNormalGpu.ts';
