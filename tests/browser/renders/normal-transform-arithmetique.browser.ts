...
// same cases (`tests/browser/probes/normalTransformCas.ts`), and its output must match the
// model. A shader that does not compile fails this test, and a drifting model too.
//
// node --experimental-strip-types tests/browser/renders/normal-transform-arithmetique.browser.ts
...
import { eclairageGpu } from '../probes/normaleEclairageGpu.ts';
