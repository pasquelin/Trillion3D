...
// same cases (`tests/browser/probes/normalTransformCases.ts`), and its output must match the
// model. A shader that does not compile fails this test, and a drifting model too.
//
// node --experimental-strip-types tests/browser/renders/normal-transform-arithmetic.browser.ts
import assert from 'node:assert/strict';
import { angleEntre, verdictNormale, xformNormalModele } from '../probes/inverseTransposeF32.ts';
import {
  CAS,
  DECROCHE_DEG,
  DEG,
...
//    normal is that of the transformed face, computed by hand in `normalTransformCases.ts`. The
