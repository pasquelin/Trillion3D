// Defect 6 (`inverseTranspose3` threshold, packages/sdk-browser/src/gpu/dag/shader/shader.ts): the WGSL kernel actually run in
...
// `tests/browser/probes/inverse-transpose-small-scale.ts`.
//
// node --experimental-strip-types tests/browser/renders/inverse-transpose-small-scale.browser.ts
...
import {
  ...
  empaqueteCas,
  veriteTerrain,
} from '../probes/inverseTransposeCases.ts';
...
console.log(
  `OK: ${lignes.length} cases, one run of the WGSL kernel — see` +
    ' tests/browser/renders/inverse-transpose-small-scale.browser.ts',
);
