...
test('each listed probe exists and bears the name required by convention', () => {
  ...
  for (const sonde of sondes) {
    assert.match(sonde, /^tests\/browser\/probes\/.*-.*\.ts$/);
    assert.ok(existsSync(join(RACINE, sonde)), `${sonde} does not exist`);
  }
});

// The count that matters: a file in `tests/browser/probes/` skipped by hyphen must be someone's support module —
// imported, or given as input to esbuild for the browser page. Without it, a misnamed probe would never run, silently.
test('no tests/browser/probes file remains orphaned: run, or named by a probe', async () => {
  const dossier = join(RACINE, 'tests/browser/probes');
  const tous = readdirSync(dossier).filter((f) => f.endsWith('.ts'));
  const lances = new Set(listJustesseTests().map((s) => s.slice('tests/browser/probes/'.length)));
...
test('each run render proof exists and bears the name required by convention', () => {
  ...
  for (const cible of lancees) {
    assert.match(cible, /^tests\/browser\/renders\/[a-z0-9]+(-[a-z0-9]+)*\.browser\.ts$/);
    assert.ok(existsSync(join(RACINE, cible)), `${cible} does not exist`);
  }
});

// The count that matters, twin of `tests/browser/probes`: the entire directory is either run or excluded with a reason.
...
test('no render proof disappears: each file is run or discarded with its reason', () => {
  ...
  const lancees = new Set(
    listBrowserTests().map((c) => c.slice('tests/browser/renders/'.length, -'.browser.ts'.length)),
  );
