...
//     tests/browser/probes/addressing-gpu.ts [sortie.json]
...
/** The addressing nibble of a map, the one `packages/sdk-browser/src/webgpu/row/pageRow.ts` and `packages/sdk-browser/src/webgpu/blend/prepare.ts`
...
const tous = cas();
const lots: Lot[] = [];
for (const filtre of ['nearest', 'linear'] as const)
...
/** One read-back row: the engine and Three targets, and the case's rank within its lot. */
interface Lecture {
  moteur: number[];
  three: number[];
  i: number;
}
type Cote = 'moteur' | 'three';

/** Each case finds its read-back row, nearest then linear (same lot order). */
const lecture: Record<GPUFilterMode, Map<AdressageCas, Lecture>> = {
  nearest: new Map(),
  linear: new Map(),
};
lots.forEach((lot, l) =>
...
const exemple = (c: AdressageCas, a: string, b: string) =>
  `${c.largeur}x${c.hauteur} S=${c.nomS} T=${c.nomT} uv=(${c.u}, ${c.v}) ${a} ≠ ${b}`;
// Every case built by `cas()` was inserted above, for both filterings: the lookup never misses.
const texel = (cote: Cote) => (c: AdressageCas, k: number) => {
...
const regle = (c: AdressageCas, k: number) =>
...
const contreRegle = (cote: Cote) => (c: AdressageCas, k: number) => {
...
// The marks `addressingCases.ts` writes on a key, and the only ones: a gap at a boundary or on a
