...
//
// The formula is that of `shadow_fs` (packages/sdk-browser/src/gpu/shadow/shader.ts) to the character: squared distance
...
//
// node --experimental-strip-types tests/browser/probes/sphere-emitter-gpu.ts
...
@fragment fn fs(in:VsOut){
 ...
 // Identical to shadow_fs (packages/sdk-browser/src/gpu/shadow/shader.ts): exclusion is exactly the announced sphere.
...
console.log('OK: spherical emitter exclusion actually run — see sphere-emitter-gpu.ts');
