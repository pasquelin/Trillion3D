...
/** Replaces, in `page`, the pages backend and the visibility buffer with those of the `sdk-browser`
 *  package at `baselineDir`, and records their footprints in `provenance.baselineOverrides` and
 *  their sources under `out`. */
export async function routeBaseline(
  ...
  for (const file of ['src/webgpu/pages/pages', 'src/visibility/buffer']) {
    const source = await readFile(resolve(baselineDir, file + '.ts'), 'utf8');
    provenance.baselineOverrides.push({
      file,
      sha256: createHash('sha256').update(source).digest('hex'),
    });
    await writeFile(resolve(out, file.replaceAll('/', '-') + '.baseline.ts'), source);
    const js = ts
