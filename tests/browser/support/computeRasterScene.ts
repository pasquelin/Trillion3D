...
export function sceneCarreaux(): ScenePreparee {
  ...
  for (let i = 0; i < 12; i++) {
    const mesh = G.mesh(carre(0.45), material);
    mesh.name = `carreau-${i}`;
  ...
  const face = G.mesh(carre(24 / pixelsParUnite(3)), material);
  face.name = 'face';
  ...
  const immense = G.mesh(carre(60), material);
  ...
  const proche = G.mesh(carre(3), material);
  proche.name = 'proche';
