// The hardware-test runner: correctness probes from `tests/browser/probes/` then render proofs from
// `tests/browser/renders/`, one by one. Paths are resolved from the repo root, never from the current
...
/**
 * Executable probes of `tests/browser/probes/`: those whose name carries a hyphen. The other files in
