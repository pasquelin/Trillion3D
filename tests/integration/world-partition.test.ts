...
async function openWorld(t: TestContext, pointer: URL) {
  const seen: { primed?: Primed; bytes?: number } = {};
  await openedWorld(t, pointer, {
    camera: pageCamera(),
    options: { onDiagnostic: primedBy(seen) },
    opened: (read) => void (seen.bytes = read.bytes),
  });
  assert.ok(seen.bytes !== undefined && seen.primed, 'the session opened and primed its cells');
