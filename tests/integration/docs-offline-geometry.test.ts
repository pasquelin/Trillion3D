...
const volume = ({ positions: p, indices: t }: Mesh) => {
  let sum = 0;
...
test('authored optional attributes cover every exported vertex', async () => {
  ...
  const cases: readonly [Mesh, 'colors' | 'normals' | 'uv', number][] = [
    [paintedTerrain(), 'colors', 4],
    [splitEdges(box()), 'normals', 3],
    [uvTiles(), 'uv', 2],
  ];
  for (const [g, key, width] of cases) {
    const values = g[key];
    assert.ok(values);
    assert.equal(values.length, (g.positions.length / 3) * width);
    assert.ok(values.every(Number.isFinite));
  }
});
/** What a compiled cache's pointer and manifest carry, as far as this test reads them. */
...
test('every published cache preserves the authored triangle count', async () => {
  ...
  for (const [index, example] of offlineExamples.entries()) {
    ...
    const pointer = JSON.parse(await readFile(url, 'utf8')) as PointerFile;
    const manifest = JSON.parse(await readFile(new URL(pointer.url, url), 'utf8')) as ManifestFile;
