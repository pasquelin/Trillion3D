...
| Pointer                        | Payload                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `native/<scope>/manifest.json` | `native/<scope>/<key>/clusters.json`, `clusters.bin`, `source.gltf`, `source.bin`, SHA-addressed objects under `native/objects/`: `<digest>.bin`, one file per index page, geometry page or streaming bundle — and baked texture levels under `native/textures/v<N>/<digest>/<kind>-<level>.<format>`, one lossless PNG plus one BC7 and one ASTC 4×4 file per mip level above the sidecar's tail |
...
- `binary` — `{ version, url, sha256, bytes, pageUrl, geometryUrl, bundleUrl, texturePreviews, texturePreviewBytes, texturePreviewBlockBytes }`, the descriptor of the [binary sidecar](#clustersbin). Absent from caches compiled before the sidecar, which carry every array inline; the reader accepts both.
...
- The **tail**, from the first level no side of which exceeds `PREVIEW_BASE` (64 px) down to 1×1, lives in the sidecar (`clusters.bin`) and is on the card before the first frame — three times over: raw RGBA8 in `texturePreviewPixels`, and block-compressed in `texturePreviewBc7` (BC7) and `texturePreviewAstc` (ASTC 4×4), one byte per texel each. One entry per `(texture, atlas)` pair, twelve `u32` in `texturePreviewU32`: `texture`, `image`, `width`, `height`, source kind and buffer view, first level, level count, pixel offset and byte length, `atlas` (0 colour, 1 data) and `bakedLevels`. Entries are strictly increasing by `(texture, atlas)`, a reader recomputes every level's geometry from `width`/`height` instead of trusting the entry, and the block columns carry no offset at all: each entry's range follows the previous one's at the length its dimensions imply (whole 4×4 blocks of 16 bytes per level, a 1×1 level costing one block), and a column that ends before or after the last entry is refused whole. `binary.texturePreviewBytes` and `binary.texturePreviewBlockBytes` in `clusters.json` declare the column lengths.
- The **head**, levels `0` to `bakedLevels - 1`, is one file per level and per format at the template `clusters.json` publishes in `textures.url` (`../../textures/v<N>/{sha}/{kind}-{level}.{format}`, relative to `clusters.json`): `{sha}` is the SHA-256 of the source image bytes, `{kind}` is `srgb` or `linear`, `{level}` the mip rank, `{format}` `png` (lossless), `bc7` or `astc` (the level's blocks, row-major, a side that is not a multiple of four padded by its edge). Levels are content-addressed, shared by every scene that shares the image, never rewritten once present, and pruned like objects when no surviving manifest names their digest. `v<N>` is `TEXTURE_PREVIEW_VERSION`: a change of the reduction rule or of a block codec changes the path, so stale levels are never served.

The block codecs are the compiler's own, pure Rust, one layout each and no mode search: BC7 mode 6 (one subset, 7-bit RGBA endpoints with a shared low bit, 4-bit weights) and ASTC 4×4 single-partition, colour endpoint mode 12 (RGBA direct) at the 192-level range with 3-bit weights. Both fit a block to one segment of RGBA space (principal axis, then least-squares endpoints), and both are proved on an independent decoder (`texture2ddecoder`). The loss is the codec's and is declared, never the engine's, measured by `pnpm run mesure:blocs:natif` (an ignored test that encodes every image of a folder, decodes it with the independent decoder and writes a JSON report): on Emerald's 336 textures (21 Sept. 2026, report `.mesure/out/45-blocks/psnr-emerald.json`) the PSNR reads median 53.0 dB BC7 / 53.6 dB ASTC, p10 43.6 / 43.4 dB, minimum 23.3 dB on both (ASTC reproduces 8 flat images exactly, BC7 none — its shared low bit cannot hit every byte) — 13 images under 35 dB, brick normal maps and fine foliage whose blocks vary in two directions, the case a two-subset mode or BC5 would serve better, left to a measured batch. Which of the three the engine samples is the device's choice (`textureCompression`, [SDK.md](SDK.md)); the pool then costs one byte per texel instead of four, and the same budget holds four times the tiles.
...
An image whose decode fails has no entry: its textures load from the source as before. A texture whose chain is not whole (`bakedLevels < firstLevel`) also keeps the source path — and, since a source image cannot fill a block pool, one such texture brings the session's pools back to RGBA8, by name (`texturePoolFormat`).
