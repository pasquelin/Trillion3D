...
`vertices` — `{ source, used, welded, coarse }`, `null` on a `shared-blend` primitive: the vertex count of the source primitive, how many of them its triangles name, how many the compiler welded onto an earlier copy identical in position and attributes, and how many the coarse levels created (`qem-attributes` only; zero otherwise).
