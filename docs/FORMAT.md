...
`vertices` — `{ source, used, welded, weldRefused }`, `null` on a `shared-blend` primitive: the vertex count of the source primitive, how many of them its triangles name, how many the compiler welded onto an earlier copy identical in position and attributes, and the non-manifold edges that weld would have created when it was refused for that reason (`null` when it was not).

Static BLEND geometry joins the DAG on the same terms as opaque geometry but keeps its transparent forward pass, original material flags, vertex attributes and source mesh sorting; `start` restores the source draw order that spatial clustering would otherwise scramble. Simplification locks every vertex shared with another group, so group borders stay watertight, and never merges the two sides of a texture seam. It changes indices only, in both `qem-endpoints` and `qem-attributes`: every collapse lands on one of its own endpoints, so a coarse cluster names source vertices and carries their attributes unchanged. A group that cannot be reduced keeps its children. The positional error does not bound texture-alpha or compositing error: `pixelError=0` is the exact-geometry comparison, and nonzero error settings require a visual check. Transmission remains unsplit because its refraction semantics are separate from alpha blending.
