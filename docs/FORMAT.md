...
- `cone` — `{ axis: [x, y, z], angle }`, the normal cone of the cluster's triangles: the unit mean of their non-degenerate face normals and the half-angle, in radians, that holds them all; `{ axis: [0, 0, 1], angle: π }`, which rejects nothing, when no face is left or the normals cancel out. The compiler cooks it (`normal_cone.rs`) with the operations of the runtime's `triangleCone` (`page/cone/build.ts`), in the same float64 order: the axis has the runtime's bits, `Math.hypot` ported as V8 computes it. `Math.acos` follows the machine the browser runs on, so the angle is fdlibm's (the same bits on every machine that compiles) raised by 4 ulps, never narrower than the cone any runtime would build. The WebGPU prepare posts it on a one-sided cluster and reads no vertex for it; `tests/integration/cooked-cones.test.ts` checks every compiled scene against `triangleCone`. In `clusters.bin` it is the `pageCone` column, four `f64` per page, flagged by bit `1024` of the page's flag word: sidecar version 9 added it, and a reader of version 8 refuses the file
