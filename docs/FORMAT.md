...
- `primitives[]` — `{ mesh, primitive, pass, clusterStrategy, pages, culling, structure, streams, dag, vertices, topology }`
  ...
  - `simplification` is `true` when the compiler ran with `qem-endpoints` or `qem-attributes`.
...
`vertices` — `{ source, used, welded, weldRefused, coarse }`, `null` on a `shared-blend` primitive: the vertex count of the source primitive, how many of them its triangles name, how many the compiler welded onto an earlier copy identical in position and attributes, the non-manifold edges that weld would have created when it was refused for that reason (`null` when it was not), and how many the coarse levels created (`qem-attributes` only; zero otherwise).

Static BLEND geometry joins the DAG on the same terms as opaque geometry but keeps its transparent forward pass, original material flags, vertex attributes and source mesh sorting; `start` restores the source draw order that spatial clustering would otherwise scramble. Simplification locks every vertex shared with another group, so group borders stay watertight, and never merges the two sides of a texture seam. Under `qem-endpoints` it changes indices only: coarse clusters keep existing vertices. Under `qem-attributes` a coarse cluster may name vertices the solve created, which the primitive's vertex buffer in `source.gltf` carries after the source ones; a level-0 page never does. A group that cannot be reduced keeps its children. The positional error does not bound texture-alpha or compositing error: `pixelError=0` is the exact-geometry comparison, and nonzero error settings require a visual check. Transmission remains unsplit because its refraction semantics are separate from alpha blending.
...
The compiler writes a compacted `source.gltf` + `source.bin` for the selected nodes. A primitive whose DAG created vertices (`qem-attributes`) has its vertex attributes rewritten as float accessors — POSITION, then every attribute a page carries — holding the source vertices first and the created ones after them, in fresh buffer views appended to `source.bin`; its index accessor is left as it came, so a glTF reader that knows nothing of the cache still draws the source surface from the first part of the buffer, and every cluster page indexes that same buffer. The source buffer views only those attributes read are not copied. An attribute a page does not carry — `TEXCOORD_2` and beyond, `COLOR_1` and beyond, application-specific `_*` attributes — leaves such a primitive: it cannot count the created vertices, and no reader of the cache drew it. Relative image URIs are rewritten against the host `resourceBaseUrl`. `images` may be omitted. Images that use `bufferView` (no `uri`) keep their view; the view is copied into `source.bin`. Sparse accessors (`accessor.sparse`) are decoded and their bufferViews are compacted and remapped. Skinned meshes (`skin`, `JOINTS_0`, `WEIGHTS_0`), morph targets (`targets`), and animations are preserved in `source.gltf` and routed to the `shared-blend` reference pass.
