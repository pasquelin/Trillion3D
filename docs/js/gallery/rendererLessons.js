...
export const rendererLessons = [
  ...
  },
  {
    ...
    title: title('Resize the two pools', 'Redimensionner les deux pools'),
    description: title(
      'Apply bounded runtime geometry and texture budgets and read back what the engine accepted — and in which texture format.',
      'Appliquez des budgets géométrique et texture bornés et lisez ce que le moteur a accepté — et dans quel format de texture.',
    ),
    controls: [
      control('geometryMiB', 'Geometry pool', 'Pool géométrique', 4, 64, 16, 4),
      control('textureMiB', 'Texture pool', 'Pool de textures', 32, 256, 128, 32),
    ],
    kind: 'memory',
    try: title(
      'Shrink both pools, then restore 16 MiB and 128 MiB; read the texture pool format.',
      'Réduisez les deux pools, puis restaurez 16 Mio et 128 Mio ; lisez le format du pool de textures.',
    ),
    changes: title(
      'The engine reports the accepted allocations. A block-compressed texture pool (bc7 or astc, chosen by the device) holds one byte per texel: the same budget carries four times the tiles of an rgba8 pool, and the layer count says so.',
      'Le moteur publie les allocations acceptées. Un pool de textures compressé par blocs (bc7 ou astc, au choix de la carte) tient un octet par texel : le même budget porte quatre fois plus de tuiles qu’un pool rgba8, et le nombre de couches le dit.',
    ),
  },
