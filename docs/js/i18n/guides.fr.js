...
<ol class="list-decimal pl-6 space-y-1">
<li><strong>Arrière-plan figé.</strong> L’image éclairée et la profondeur opaque sont copiées une fois. Chaque surface transmissive lit ces copies, si bien que l’ordre entre deux d’entre elles ne change rien — et aucune ne voit à travers une autre, la même limite déclarée que la visionneuse de référence glTF.</li>
<li><strong>Étape de surface</strong> (<code>WG water surfaces</code>). Les éléments transmissifs se dessinent avec l’étage de sommets du mélange et sa lecture du matériau, dans un tampon de surface à la disposition de la résolution opaque — couleur de base, normale, rugosité, émission, occlusion — plus le rang de l’élément et l’opacité, avec la profondeur matérielle testée contre la copie opaque et écrite : la surface la plus proche d’un pixel est celle qui reste, quel que soit le nombre de surfaces empilées, et une surface derrière un opaque n’atteint jamais la composition.</li>
...
<ul class="list-disc pl-6 space-y-1">
<li><code>stageProfile()</code> dépose les deux passes sur l’étape <em>transparents</em>, par étiquette ; une scène sans matériau transmissif ne les encode jamais et n’alloue qu’un texel à leurs cibles.</li>
<li>Le budget des cibles d’image compte l’arrière-plan (12 octets par pixel), les surfaces d’eau (28) et leur profondeur (4) seulement quand la scène transmet : <code>gpuFrameTargetBytes</code> le dit.</li>
<li>Une vue de diagnostic — grappes, fil de fer, erreur écran — ou une variante GPU de diagnostic dessine la tranche de transmission comme un mélange de plus, si bien que la variante mesure le même étage de fragments sur tous les transparents.</li>
</ul>`,
  },
  architecture: {
