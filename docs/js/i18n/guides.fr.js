...
<p>Lisez la cadence sur les pics, jamais sur les médianes : <code>textureUploadPeakMs</code> est la pire passe budgétée depuis le départ, <code>textureUploadMs</code> la dernière passe (<code>null</code> quand elle n’avait rien à servir), <code>textureTilesDeferred</code> ce que le budget a repoussé à l’image suivante, et <code>stageProfile()</code> donne les p50/p95 de l’étape « Textures ». Mesuré sur le cache Emerald, vue générale, cache froid et caméra mobile (1280×720, DPR 1, seuil 1 px, deux exécutions) : le p95 de l’étape « Textures » passe de 4,2–9,3 ms à 1,1–1,2 ms, le p99 de l’intervalle d’image du navigateur de 33–133 ms à 16,8 ms ; le coût déclaré est une image plus grossière pendant que les tuiles arrivent — 8 à 12 tuiles par image, 1,2–1,8 niveau manquant en moyenne en fin de traversée contre 0,6–0,7 avant — et une pose fixe converge vers la même capture à 0 px. Le banc la lit avec <code>--budget-textures &lt;ms&gt;</code> (<code>scripts/mesure/README.md</code>).</p>`,
  },
  'example-diagnostics': {
