...
<h3 class="text-lg font-bold mt-4">Sun cascades are sliding windows</h3>
<p>Each cascade covers a sphere around a slice of the camera frustum — a sphere, so that turning the camera changes nothing. Its map is a window of whole pages on the light plane, addressed by absolute page modulo the face, as the pages of a virtual shadow map are. When the camera moves less than a page, the window does not move. When it moves by whole pages, the window slides: the pages that stay inside keep their depth and only the strip that entered is redrawn. A move of a whole window side, a step of the depth anchor along the light axis, a new sun direction or a new lens restart the cascade whole. Until an entered strip is drawn, the shader reads it as absent and takes the next, coarser cascade, exactly as an unmapped page falls back to a coarser clipmap level.</p>
<h3 class="text-lg font-bold mt-4">What stales a page</h3>
<ul class="list-disc pl-6 space-y-1">
<li><strong>A world change</strong> — a node moved with <code>setNodeMatrix</code>, a light moved with <code>setLight</code> — stales the pages its box covers at once, in every face whose range it touches.</li>
...
<p>Stale pages are grouped into rectangles and admitted by priority until <code>shadowBudgetMs</code> (1 ms by default), the pass's own GPU time smoothed per page; the first region of a frame always passes. Each region rejects, before its draw, every cluster whose world sphere misses the box it cuts in the window — its page rectangle by the depth bounds of the map. The frame publishes <code>shadowPagesDrawn</code>, <code>shadowPagesTotal</code> (cumulative, settle drains included), <code>shadowPagesPending</code> and <code>shadowWaitMs</code> in its <code>FrameMetrics</code>; the per-stage profile adds, under <em>Shadows</em>, the pages invalidated, redrawn and pending, the oldest wait in frames, and the occluders tested and kept by the region culls. The <a class="link link-primary" href="#/en/playground/directional-shadow">shadow sun lesson</a> shows these counters live: turn the sun and every page restarts; zoom or orbit and only strips are redrawn; stand still and nothing is.</p>`,
    example: `const explorer = await createExplorer('viewer', { manifestUrl, scope: 'full', shadowBudgetMs: 1 });
explorer.addLight({ id: 'sun', kind: 'directional', direction: [0.3, -0.8, 0.5], color: [1, 0.95, 0.85], intensity: 2.5, castsShadow: true });
let drawn = 0;
const before = explorer.render().shadowPagesTotal ?? 0;
explorer.setPose({ ...explorer.homePose(), position: [12, 3, 8] }); // a step of a few metres
await explorer.flush(); // drains what the budget left, deterministically
const after = explorer.render();
drawn = (after.shadowPagesTotal ?? 0) - before; // the strips that entered, not the whole cascades
console.log(drawn, after.shadowPagesPending, after.shadowWaitMs);`,
  },
];
