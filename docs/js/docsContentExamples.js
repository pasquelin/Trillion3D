...
<p>Read the cadence on peaks, never on medians: <code>textureUploadPeakMs</code> is the worst budgeted pass since the start, <code>textureUploadMs</code> the last pass (<code>null</code> when it had nothing to serve, or under a barrier), <code>textureTilesDeferred</code> what the budget pushed to the next frame, and <code>stageProfile()</code> gives the p50/p95 of the "Textures" stage. Measured on the Emerald cache at commit 8c20f71b, general view, cold cache and moving camera (1280×720, DPR 1, threshold 1 px, two runs, Apple M2 Max, Chrome 153): the "Textures" stage p95 went from 4.2–9.3 ms to 1.1–1.2 ms, the browser frame interval p99 from 33–133 ms to 16.8 ms; the declared cost is a coarser image while tiles land — 8–12 tiles per frame, 1.2–1.8 missing levels on average at the end of the traversal against 0.6–0.7 before — and a still pose converges to the same 0 px capture. With the pass clock also covering the shadow follow of the landed tiles (619e34fb, same command, two runs): "Textures" 1.0 / 1.2 ms p50/p95, session peak 5.1–16.6 ms over the four sessions, on the one frame that lands the first colour tile. The bench reads it with <code>--budget-textures &lt;ms&gt;</code> (<code>scripts/mesure/README.md</code>).</p>`,
