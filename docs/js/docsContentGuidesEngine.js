...
<p>Nothing is refused and nothing stops: the cut is <strong>coarsened, never truncated</strong>. When the pages the cut asks for — root cover included — exceed the slots, the admission raises the screen error one rung of a ladder of powers of two (1 px first, then 2, 4…), and relaxes it rung by rung down to 0.125 px once the cut fits with room to spare. Two rules keep it still: a rung moves only on a cut sampled at the threshold currently asked, and a rung that overflowed for this view is not asked again until the view changes. A still camera therefore settles in a few samples and holds its frame.</p>
