...
1. **Pick.** First count your open pull requests: at 3, take no issue and unblock them (red CI,
   ...
   `measure ko` or `audit ko`, then 🔴 critical ones (the children and To-do items of a programme such as
   #483, in its order), then the oldest open one without `in progress` or `in review`
