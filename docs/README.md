...
The GitHub Pages site (`/docs` on `main`) serves the latest campaign report: `index.html` and `vignettes/`, generated by `node scripts/mesure/publierRapport.mjs` following `rapportGlobal.mjs`. `.nojekyll` prevents Jekyll from processing this page.
