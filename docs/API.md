...
| `boxTransformBatch(out, boxes, mats[], n)`                                | `out[i] = boxTransform(boxes[i], mats[i])`                                                                            | `for … box.applyMatrix4(m)`                | `mathBatch.test.ts`; WebAssembly kernel bit-identical (`math.rs`)                                                  |
...
| `multiplyMatrix4Batch(out[], a[], b[], n)`                                | `out[i] = a[i] · b[i]`, sub-views                                                                                     | `for … m.multiplyMatrices(a, b)`           | `mathBatch.test.ts`; WebAssembly kernel bit-identical (`math.rs`)                                                  |
...
| `hierarchyUpdateBatch(worldViews[], positions[], rotations[], scales[], parents, n, local)` | a whole hierarchy, parents before children, `composeMatrix4` then `multiplyMatrix4`                      | `Object3D.updateMatrixWorld` over a scene  | `mathBatch.test.ts` against `updateNodeMatrixWorld`; WebAssembly kernel bit-identical (`math_hierarchy.rs`)        |
