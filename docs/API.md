...
| `submittedDraws(backend)` (`ClusterDrawSource`)               | the batch records or diagnostic page meshes the owner submits for the cut; oracle and test access only | reading `ClusterDrawMesh` objects off the host scene            | `pagesBackend*.test.ts`, `pageRaster.ts` oracle                                        |
...
| `autonomousCopyDraws`, `transmissionBackdropBytes`            | owned scene copies submitted this frame; bytes of the two backdrop copies                              | inference from host-renderer counters                           | explorer proof: one copy, 64 × 64 × 12 bytes, none in `WebGLRenderer.render`           |
