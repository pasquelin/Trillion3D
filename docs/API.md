...
| `PageSurface`, `surfaceOf`, `refreshSurface`, `surfaceSide` — `packages/sdk-browser/pageSurface.ts`                                 | the surface record a page carries: the shaded fields of `VisMaterial` and the raster facts declared beside them (version, opacity, blended, single pass, declared as a list of materials). One record per declaration, held by it and refilled in place — the raster facts at every read, since a host writes `side` without bumping a version, the shaded ones when it does | `PageRec.material`, `VisPage.material`, `BatchPage.material` and `BlendGpuItem.material` as host declarations, and the `visMaterial`/`sideOf` calls the cut, the rows, the raster, the plan and the audit each made on them | `pageSurface.test.ts`, `moteur-sans-three.test.ts` (closed list of `declaration` readers) |
