...
| `autonomousCopyDraws`, `transmissionBackdropBytes`            | owned scene copies submitted this frame; bytes the two backdrop copies hold since the first glass      | inference from host-renderer counters                           | explorer proof: one copy, 64 × 64 × 12 bytes, none in `WebGLRenderer.render`           |
