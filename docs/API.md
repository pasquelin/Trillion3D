...
| `HostLight`, `HostPlaced` — `packages/sdk-browser/sceneLighting.ts`; `HostDiagnosticMesh`, `HostDiagnosticMaterial`, `HostDiagnosticGeometry` — `packages/sdk-browser/hostResources.ts` | a source-graph light and the copy a display graph holds of it, and the mesh a diagnostic view repaints: the kind flags and values the host writes on its own objects, nothing of a library class | `THREE.Light`, `THREE.Mesh`, `THREE.Material` and `THREE.BufferGeometry` in `sceneLighting.ts`, `explorerDiagnosticApi.ts`, `triangleDiagnostic.ts` and `pageRaster.ts` | `sceneLighting.test.ts`, `triangleDiagnostic.test.ts`, `moteur-sans-three.test.ts` (closed list) |
