...
| Function or contract                                    | Computes                                                                                                                                                      | Replaces                                                                               | Proof                                                                                                   |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `createWebglRenderTarget(gl, width, height)`            | an RGBA8 colour texture (unfiltered) and a 24-bit depth renderbuffer on one framebuffer, in drawing-buffer pixels; `resize`, `dispose`                        | `new THREE.WebGLRenderTarget(width, height, { type, colorSpace })`, `setSize`          | `webglRenderTarget.test.ts`; `transmission-explorateur-webgl.browser.mjs` (target pixel = canvas pixel) |
| `bindWebglTarget(gl, target)`                           | the framebuffer bound — the page's for `null` — and the viewport set to its whole size                                                                        | `renderer.setRenderTarget(target)`                                                     | `webglRenderTarget.test.ts`                                                                             |
| `HostDrawOutput`                                        | where a host draw lands (`framebuffer`, `width`, `height`) and its display chain (`encodeSrgb`, `toneMapped`), handed to `drawHostGeometry`                   | the render target the renderer held in its state                                       | `explorerCompose.ts`, every `drawHostGeometry`                                                          |
| `createFrameComposer(gl, camera)`                       | one engine's frame on the surface or a target: bind, clear with the engine's background (sRGB-encoded by `mathColor`), `drawHostGeometry`, keep or present    | `createSceneDrawer`: `renderer.clear`, `render(scene)`, `copyFramebufferToTexture`     | byte proof above; `image-tenue-couleur.browser.mjs`; `explorateur-clusters-autonome.browser.mjs`        |
| `createHeldFrame(gl)`                                   | the last complete frame, blitted into an RGBA8 texture after a complete frame and blitted back on a held one; lost with the context, kept again after         | `FramebufferTexture` + `copyFramebufferToTexture` + a full-screen `Mesh` presented     | `explorerHeldFrame.test.ts`; `image-tenue-couleur.browser.mjs` on the engine surface (`alpha: false`)   |
...
| `createBackendPresenter(gl)`                            | the copy of a presented surface into the bound framebuffer, by the engine's blit program, rebuilt after a loss                                                | `host.getContext()` / `host.resetState()` around the same copy                         | `explorerComposeSurface.test.ts`                                                                        |
| `boundToContext(gl, build, discard)`                    | a GPU object that lives as long as the context does: built on first use, forgotten on `webglcontextlost`, rebuilt on the restored context                     | the renderer's own re-initialisation on `webglcontextrestored`                         | `explorerHeldFrame.test.ts`, `comparison.test.ts` (loss, restore)                                       |
| `createWebglProgram(gl, vertex, fragment)`              | one linked program or the shader / link log, shaders deleted either way; shared by the blit, the cluster program and the compositor                           | three copies of the same compile-and-link                                              | `check:duplicates`; every program proof                                                                 |
| `WebglSurface.restorations`                             | how many times the context came back; a target built before the last restoration is forgotten and rebuilt                                                     | `WebGLProperties` recreated by the renderer                                            | `explorerHostState.ts`                                                                                  |
| `createThreeSceneDraw(gl, scene)` (`threeSceneAdapter`) | `drawHostGeometry` of an engine whose image is a Three scene: one shared renderer per context, reset before each draw, an engine target bound as its XR layer | the host's `WebGLRenderer` drawing `backend.scene`                                     | `image-tenue-couleur.browser.mjs`; byte proof (witness captures identical, comparison sides identical)  |
| `createExplorerCapture({ context, compose })`           | the composition on the page's drawing buffer, read back on the engine context; the visible sample skipped while measuring                                     | `renderer.getRenderTarget()` / `setRenderTarget(null)` / `render(scene)`               | `explorerComposeSurface.test.ts`: read, bind, compose, read; byte proof                                 |
