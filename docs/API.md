...
| `RenderBackend.clusterDraws()`                                | the batch records or diagnostic page meshes the owner submits for the cut                              | reading `ClusterDrawMesh` objects off the host scene            | `pagesBackend*.test.ts`, `pageRaster.ts` oracle                                        |
