...
| `createThreeSceneDraw(gl, scene)` (`threeSceneAdapter`)                                       | `drawHostGeometry` of an engine whose image is a Three scene: one shared renderer per context, reset before each draw, an engine target bound as its XR layer; counters of the frame, zero on a held one                                  | the host's `WebGLRenderer` drawing `backend.scene`                                                                                | `threeSceneAdapter.test.ts` (no context); `image-tenue-couleur.browser.mjs`; byte proof (witness captures identical, comparison sides identical) |
