...
missing or older. Nothing built is committed — the site, the API files, the scene caches: the
...
git does not ignore runs again only the steps of `buildSite()` that read it or what an earlier one
wrote (the list `SITE_STEPS` in `scripts/docs/site.ts`), then every open page reloads. The reload
script is added by that server to the pages it serves, never written into `dist/site/`
(`scripts/docs-dev.test.ts`). A new language's flag needs a restart: the languages are loaded once
per process. `docs:serve` stays the production-path server of the proofs.
...
does not serve it, nor the mountain terrain beside it. Its source is generated by
`scripts/docs/garden-source.ts` under the repository license; it contains no third-party models,
textures, shaders or sample code. Provenance and limitations live beside it in its `README.md`. It
is not a performance or large-residency benchmark.
...
this checkout's native compiler to write its cache; `docs:gallery` does the same for the scenes
modelled in code, the gallery's observatory and the mountain terrain. Every generator runs the
