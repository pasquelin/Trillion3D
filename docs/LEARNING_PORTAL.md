...
files work on GitHub Pages and with the local documentation server. `docs/react/main.jsx` is the
composition root; route components own their interactive effects and dispose them before the next
route.

## Source layout

- `docs/react/` owns the shell, pages and reusable presentation components. `Entry.jsx` renders API
  entries, `ApiDemo.jsx` renders the pure demo models, and `engine-scene/` hosts the real WebGPU
...
- `docs/js/demo*.js` contains the pure per-entry demonstration models. `demoKit.js` defines their
  controls and result views; `docs/react/ApiDemo.jsx` localizes and presents them.
- `docs/react/components/` contains the shared DaisyUI primitives. `CodeBlock.jsx` owns one inner
  scrolling region for all numbered lines and copies the original source string. `Stats.jsx` owns
  metric presentation. `gallery/ExampleCard.jsx` composes the same `Card` primitive for both the home
