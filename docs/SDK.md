...
`textureCompression` (`'auto'` by default; `'bc7'`, `'astc'` or `'none'`) is the block format of the two pools, the reference's cook-time compression brought to the web: the cache bakes every level in BC7 and in ASTC 4×4 beside the lossless PNG ([FORMAT.md](FORMAT.md), _Textures_), the device says which it samples — `texture-compression-bc` on desktop cards, `texture-compression-astc` on mobile ones, both on Apple silicon, neither on a software adapter —, and the pool then holds one byte per texel instead of four: the same `texturePoolBytes` carries four times the tiles (16 layers per atlas at 512 MiB instead of 4), and the tail of every texture pinned at `prepare()` costs a quarter. `'auto'` takes BC7 before ASTC; `'bc7'` or `'astc'` insist on one; `'none'` keeps RGBA8 — the lossless "before" of a comparison. It applies under `textureSource: 'cache'` and needs a whole baked chain for every texture the render reads, since a host image cannot fill a block pool: one texture without brings both pools back to RGBA8, by name. `texturePoolFormat` publishes the colour pool's format actually held (`bc7-rgba-unorm-srgb`, `astc-4x4-unorm-srgb` or `rgba8unorm-srgb`), and the `material-textures-ready` diagnostic carries the choice with its reason (`pool.compression`) — a device without either feature reads there, never in an estimate. The loss is the codec's, declared and measured at cook time ([FORMAT.md](FORMAT.md)), and the bench pits the two formats against RGBA8 on one cache (`--compression-avant none --compression-apres bc7`, `scripts/mesure/README.md`).
