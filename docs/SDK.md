...
`maxFrameAllocationBytes` defaults to 288 MiB — what a 2496×1404 image needs with every pipeline-version-1 target, the Hi-Z reservation and the two temporal-antialiasing history targets (275.7 MB measured), rounded up to 32 MiB; it was 256 MiB before the history existed. It bounds logical WebGPU frame targets, a conservative reservation for Hi-Z image/pyramid resources, the two history targets, one live surface capture and explicit asynchronous image staging. It excludes geometry, material texture atlases, fixed per-page work buffers, CPU memory and the optional WebGL diagnostic capture context. It is not a total-device memory limit. Allocation logs describe that scope; physical `vramBytes` remains `null`. Geometry allocation metrics count allocated geometry buffers, including concatenated attributes, rather than just currently selected CPU arrays.
...
`textureSource` (`'host'` by default, `'cache'` to opt in) says where material texels come from. With `'host'` the glTF loader fetches and decodes every source image, as it always did — what a backend that draws the host scene (the Three witness) requires — and the WebGPU backend keeps the path it always took, source image then mips regenerated on the card: no baked level is read from the cache, so nothing is downloaded twice. With `'cache'`, every image whose mip chain is baked in the compiled cache (see [FORMAT.md](FORMAT.md), _Textures_) is replaced by a one-pixel placeholder at load time, and the WebGPU backend reads its levels one at a time from the cache, coarse to fine, in the order the screen asks for them: the sidecar's tail is on the card before the first frame, `bakedLevels` PNG files follow once the screen has said what it looks at — never before the first camera, like the full resolution — at most six of them decoded and waiting at a time, through `createImageBitmap` and `copyExternalImageToTexture` into the mip level of the same rank, and a layer whose whole chain has landed takes the sampling path it always took. Both atlases carry a residency word now, so a data map whose 128 px level is the finest present is sampled at that level, never at a finer, unwritten one. Ask for it only when every backend of the session reads the atlas and none the scene. Measured on Emerald at 2496×1404 (`scripts/mesure/banc.mjs --textures cache`): preparation 1,84 s → 1,16 s, the browser no longer decoding 565 MB of PNG; GPU time unchanged within dispersion (5,9 → 5,5 ms without the sun, 10,0–10,5 → 10,2–11,0 ms with it); card memory unchanged, the levels being lossless RGBA8; 0 pixels beyond ±2 per channel on the general view, 1 720 isolated cutout flips (0,05 %) on the lawn view where a coarse texel's median alpha rounds to the other side of the 0,5 cutoff.

When a colour level lands, every shadow page is invalidated: a shadow map drawn with the previous alpha level would describe foliage the image no longer shows. Pages are redrawn under the ordinary shadow budget, and `flush()` redraws the pending ones — up to eight frames, only when a level actually landed during that flush — so a flushed pose is settled, shadows included.

`atlasClasses` (`2` by default, `1` to opt out) sets how many size classes the prepared WebGPU renderer may allocate for its material atlases; at `1` it allocates one texture array sized like the largest texture, and `2` lets it add a second, smaller array when that saves bytes and the device limits allow it — the automatic split under `maxTextureArrayLayers` happens at either setting. Measured on the Emerald cache, `2` saves 872 359 272 calculated bytes (−11,5 %) and changes the image: 15 142 pixels over ten 1246×1000 captures (0,012 %, maximum amplitude 159) against a bit-identical `1`, because a small texture packed in its own atlas no longer blends the surrounding white fill into its coarse mip levels.
