...
Nothing lights an opaque surface except a light the host declared. The deferred resolve has no fixed ambient term, no constant sky and no authored scene lighting: a surface no declared light reaches is exactly zero, so an extentless corridor stays black at noon. Emission is a material property and is added as before.
