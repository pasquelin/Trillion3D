...
| `image-depth-unsupported` | Image plugin read a bit depth the `Rgba8` image contract cannot carry (e.g. TIFF 16 bits per channel); reported per texture, does not fail the job |
