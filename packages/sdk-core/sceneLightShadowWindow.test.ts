// A cascade map is addressed by absolute page modulo the face: the mask is physical, window
// rectangles are written through the wrap, a slide marks the entering strip only, and the
// regions cut at the seam carry the translation that lands window pages on physical pages.
...
import { markWindowRect } from './sceneLightShadowPages.ts';
...
test('markWindowRect wraps a window rectangle onto the face through the origin page', () => {
  const mask = new Uint8Array(ROWS);
  // Window columns 6..7 with the origin at physical column 3 land on physical 1..2 (6+3, 7+3 mod 8);
  // window rows 7 with the origin at physical row 5 lands on physical row 4.
  markWindowRect(mask, 0, ROWS, 3, 5, 6, 7, 7, 7);
...
test('a slide marks the strip that entered, at its physical place, and nothing else', () => {
  ...
  // Origin at physical (2, 0): the window slid right by one page, its last column is new.
  dirty.setWindow(0, 0, 2, 0);
  dirty.slide(0, 0, ROWS, 1, 0, 0, 0);
  assert.equal(dirty.pages(0, 0), ROWS, 'one column of pages');
  // Window column 7 lives at physical column (7 + 2) mod 8 = 1.
...
test('a slide by two pages down marks two rows, wrapped at the bottom of the face', () => {
  const dirty = createShadowDirty();
  dirty.setWindow(0, 0, 0, 7);
  dirty.slide(0, 0, ROWS, 0, 2, 0, 0);
  // Window rows 6 and 7 live at physical rows (6 + 7) mod 8 = 5 and (7 + 7) mod 8 = 6.
...
test('a region straddling the seam is cut in two, each side with its own translation', () => {
  ...
  // Origin at physical column 3: physical columns 3..7 are window 0..4, physical 0..2 are window 5..7.
  dirty.setWindow(0, 0, 3, 0);
