...
const TEMPLATES={url:'clusters.bin',pageUrl:'../../objects/{sha}.bin',geometryUrl:'../../objects/{sha}.bin',bundleUrl:'../../objects/{sha}.bin'};
...
function manifest():ClusterManifest{
 const dagPages=[
  {id:0,url:url('a'),sha256:sha('a'),bytes:48,count:12,start:0,min:[-1,-2,-3],max:[1,2,3],role:'exact' as const,
   geometry:{url:url('b'),sha256:sha('b'),bytes:32,formatVersion:2 as const,codec:'meshopt' as const,vertexCount:8,indexCount:12,flags:15,uncompressedBytes:128},
  ...
  {id:1,url:url('c'),sha256:sha('c'),bytes:48,count:12,start:12,min:[0,0,0],max:[2,2,2],role:'coarse' as const,
 ...
 const sparsePages=[{id:0,url:url('d'),sha256:sha('d'),bytes:24,count:6,min:[0,0,0],max:[1,1,1],
  level:0,lodError:0,sphere:[0.5,0.5,0.5,0.8660254037844386],parentError:null,parentSphere:null}];
 return {
  ...
  primitives:[
   {mesh:0,primitive:0,pass:'exact-clusters',clusterStrategy:'dag-groups',pages:dagPages,
    ...
    streams:{version:1,pinned:1,bundleBytes:131072,pages:[{url:url('e'),sha256:sha('e'),bytes:96,count:2}]},
   ...
   {mesh:1,primitive:0,pass:'exact-clusters',pages:sparsePages,culling:null,structure:null,streams:null},
  ],
 } as ClusterManifest;
}
