...
// asked for with it, so a reader that falls back past a withdrawn page reads a current floor. Like
// every page a frame marks, it is drawn in that frame (#489): none waits a turn.
...
test('a static caster over every page each frame: the sun floor and pages read stay current', () => {
  ...
  for (let frame = 2; frame < 10; frame++) {
    ...
    for (const entry of [sunFloor(plan, slice), ...read])
      assert.ok(plan.table.words[entry] & PAGE_VALID, `entry ${entry} at frame ${frame}`);
  }
  assert.equal(plan.counts.pendingPages, 0, 'no page waits');
});

test('a lamp that moves every frame keeps the floor of each face read current', () => {
...
test('five lamps moving: every face floor and every page read is drawn in the frame', () => {
  const store = createSceneLightStore();
  const plan = createShadowPlan(32);
  ...
  for (let frame = 1; frame < 20; frame++) {
    // Thirty lamp floors, more than one batch holds, and the sun floor pages a walking camera
    ...
    for (const slice of slices) {
      ...
      for (let face = 0; face < 6; face++) {
        ...
        assert.ok(drawn.has(page), `slice ${slice} face ${face} at ${frame}`);
      }
    }
