...
/**
 * Words of SOFT before its vertices: `op, engine id, flags, px, py, pz, qx, qy, qz, qw, sx, sy,
 * sz, friction, restitution, gravityScale, linearDamping, stretch, bend, pressure, vertexCount,
 * indexCount` — the body's place, turn and scale in the world, its matter, its compliances (m/N;
 * `Infinity` is no constraint) and its gauge pressure at rest (Pa, 0 none). Then per vertex
 * `SOFT_VERTEX_WORDS`: `x, y, z, mass` in the geometry's own frame, a mass of 0 held in place (a
 * pin); then `indexCount` triangle corners. A body with no triangle is a rope: each vertex joined
 * to the next. A soft body is removed as any body (REMOVE).
 */
