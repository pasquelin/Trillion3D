...
const clock = process.env.DUEL_CLOCK;
if (clock === 'ticking' || clock === 'frozen') {
  process.hrtime.bigint = CLOCKS[clock]();
  const { duel } = await import('../oracles/three-duel.ts');
  const out = new Float64Array(3);
  await duel({
    name: 'replaced clock',
    fichier: 'packages/sdk-core/bench/oracles/three-duel.ts',
    size: 1,
    three: () => {},
    oracle: () => out,
    core: () => out,
  ...
  const run = (clock: 'ticking' | 'frozen') =>
