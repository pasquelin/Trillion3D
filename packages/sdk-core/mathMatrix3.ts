 ...
 *
 * DÉTERMINANT EXACTEMENT NUL : l'ADJOINTE, non divisée, et non la matrice nulle de la référence.
 ...
 * même, et la normale sort nulle sans qu'aucun cas particulier ne l'écrive. C'est la convention du
 * moteur, la même que celle du noyau WGSL de `packages/sdk-browser/inverseTransposeWgsl.ts`, où
 * elle est écrite en entier ; la référence, elle, rend zéro et perd la surface.
 */
export function normalMatrix3<T extends NumberSink>(out: T, m: ArrayLike<number>) {
  ...
  // Déterminant exactement nul : l'adjointe brute, donc le même facteur d'échelle inconnu sur les
  // neuf termes, que la normalisation du consommateur efface. Les trois premiers cofacteurs sont
  // déjà là — le déterminant les a exigés — et les six autres sont ceux de la division ci-dessous.
  const detInv = det === 0 ? 1 : 1 / det;
  out[0] = t11 * detInv;
