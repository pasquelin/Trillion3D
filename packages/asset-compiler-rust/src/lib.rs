...
fn concat_gltf_buffers(dir:&Path,g:&Value,embedded:Option<&[u8]>,declared:Option<&Value>)->Result<(Binary,Vec<usize>,Vec<(String,String)>)>{
 ...
 if buffers.len()==1{
  ...
  if let Some(bin)=embedded{return Ok((Binary::Owned(bin.to_vec()),vec![0],Vec::new()));}
 }
...
struct Accessor<'a>{bin:&'a [u8],base:usize,stride:usize,count:usize,component:usize,bytes:usize,width:usize,has_buffer_view:bool,sparse:Option<SparseAccessor<'a>>}
fn accessor<'a>(g:&'a Value,bin:&'a [u8],id:usize)->Result<Accessor<'a>>{let accessors=values(g,"accessors")?;let views=values(g,"bufferViews")?;let a=item(accessors,id,"accessor")?;if a.get("normalized").and_then(Value::as_bool)==Some(true){return Err(CompilerError::new("UNSUPPORTED_ACCESSOR","Normalized accessors are unsupported"));}
 let component=required_index(a.get("componentType"),"accessor.componentType")?;let bytes=match component{5121=>1,5123=>2,5125|5126=>4,_=>return Err(CompilerError::new("UNSUPPORTED_COMPONENT",component.to_string()))};let width=match a.get("type").and_then(Value::as_str){Some("SCALAR")=>1,Some("VEC2")=>2,Some("VEC3")=>3,Some("VEC4")=>4,_=>return Err(CompilerError::new("UNSUPPORTED_ACCESSOR_TYPE","Unsupported or missing accessor type"))};let count=required_index(a.get("count"),"accessor.count")?;
 ...
 Ok(Accessor{bin,base,stride,count,component,bytes,width,has_buffer_view,sparse})
}
impl Accessor<'_>{
 ...
 fn collect_u32(&self)->Result<Vec<u32>>{
  ...
  if let Some(sparse)=&self.sparse{
   for k in 0..sparse.count{
    ...
    let val=match self.component{5121=>b[0] as u32,5123=>u16::from_le_bytes([b[0],b[1]]) as u32,5125=>u32::from_le_bytes([b[0],b[1],b[2],b[3]]),5126=>f32::from_le_bytes([b[0],b[1],b[2],b[3]]) as u32,_=>return Err(invalid("Unsupported component"))};
    out[idx]=val;
   }
  }
  Ok(out)
 ...
 fn collect_f32(&self)->Result<Vec<f32>>{
  ...
  if let Some(sparse)=&self.sparse{
   for k in 0..sparse.count{
    let idx=match sparse.indices_component{5121=>*sparse.indices_bin.get(sparse.indices_offset+k).ok_or_else(||invalid("Sparse index out of bounds"))? as usize,5123=>{let o=sparse.indices_offset+k*2;let b=sparse.indices_bin.get(o..o+2).ok_or_else(||invalid("Sparse index out of bounds"))?;u16::from_le_bytes([b[0],b[1]]) as usize},5125=>{let o=sparse.indices_offset+k*4;let b=sparse.indices_bin.get(o..o+4).ok_or_else(||invalid("Sparse index out of bounds"))?;u32::from_le_bytes([b[0],b[1],b[2],b[3]]) as usize},_=>return Err(invalid("Unsupported sparse indices component"))};
    if idx>=self.count{return Err(CompilerError::new("INDEX_OUT_OF_BOUNDS","Sparse index exceeds count"));}
    for c in 0..self.width{let vo=sparse.values_offset+(k*self.width+c)*self.bytes;let b=sparse.values_bin.get(vo..vo+self.bytes).ok_or_else(||invalid("Sparse value out of bounds"))?;let val=match self.component{5126=>f32::from_le_bytes([b[0],b[1],b[2],b[3]]),5121=>b[0] as f32,5123=>u16::from_le_bytes([b[0],b[1]]) as f32,5125=>u32::from_le_bytes([b[0],b[1],b[2],b[3]]) as f32,_=>return Err(invalid("Unsupported component"))};if !val.is_finite(){return Err(CompilerError::new("NONFINITE_POSITION","Position is not finite"));}out[idx*self.width+c]=val;}
   }
  }
  Ok(out)
 }
...
pub fn compile(o:&Options,strategy:&dyn ClusterStrategy,progress:impl Fn(Value)+Sync)->Result<Value>{
 ...
 let key=hash(serde_json::to_string(&json!({"source":hash(&loaded.manifest_bytes),"binary":bin_hash,"compiler":COMPILER_VERSION,"implementation":format!("{:x}",implementation.finalize()),"strategy":strategy.id(),"strategyVersion":strategy.version(),"scope":o.scope,"budget":o.triangle_budget,"resourceBase":o.resource_base,"simplification":o.simplification,"errorModel":LOD_ERROR_MODEL}))?.as_bytes());
  ...
  let primitives:Vec<Value>=pool.install(||jobs.par_iter().map(|(old,primitive)|->Result<Value>{check(o)?;let p=item(values(item(mesh_values,*old,"mesh")?,"primitives")?,*primitive,"primitive")?;if optional_index(p.get("mode"),"primitive.mode",4)?!=4{return Err(CompilerError::new("UNSUPPORTED_PRIMITIVE","Only static triangles are supported"))}
   let positions=accessor(g,bin,required_index(p.get("attributes").and_then(Value::as_object).and_then(|a|a.get("POSITION")),"primitive.attributes.POSITION")?)?;if positions.width!=3{return Err(invalid("POSITION must be VEC3"));}
