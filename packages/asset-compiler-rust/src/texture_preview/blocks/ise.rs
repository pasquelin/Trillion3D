//! Integer sequence encoding of ASTC for the one range this writer uses: 192
//! levels, a trit and six bits per value. Five trits share one byte, laid out
//! between the bit fields of their five values; the byte is not a base-3
//! number but the layout the specification decodes, so the table is built by
//! running that decode over the 256 bytes and keeping the first that yields
//! each of the 243 quintuples. The value a level decodes to comes from the same
//! specification: `(trit · 5 + B(m)) ^ A(m)`, its top bit kept, then over four.
