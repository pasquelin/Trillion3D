...
fn mesh_extent(positions:&[f32],indices:&[u32])->f64{
 let mut min=[f64::INFINITY;3];let mut max=[f64::NEG_INFINITY;3];
 for &id in indices{
  let i=id as usize*3;if i+2>=positions.len(){continue;}
  for axis in 0..3{let value=positions[i+axis] as f64;min[axis]=min[axis].min(value);max[axis]=max[axis].max(value);}
 }
 (0..3).map(|axis|max[axis]-min[axis]).fold(0.0_f64,f64::max).max(1e-12)
}
...
pub fn simplify_fast(positions:&[f32],indices:&[u32],target_triangles:usize)->Result<SimplifiedMesh>{
 ...
 let bytes=unsafe{std::slice::from_raw_parts(positions.as_ptr() as *const u8,positions.len()*4)};
 let vertices=VertexDataAdapter::new(bytes,12,0).map_err(|_|invalid("POSITION adapter"))?;
 let mut result_error=0.0_f32;
 let out=meshopt::simplify::simplify(indices,&vertices,(target_triangles.max(1))*3,0.01,SimplifyOptions::LockBorder,Some(&mut result_error));
 let triangles=out.len()/3;
 let progressed=triangles<current&&!out.is_empty();
 Ok(SimplifiedMesh{indices:if progressed{out}else{indices.to_vec()},triangles:if progressed{triangles}else{current},error_object:if progressed{(result_error as f64)*mesh_extent(positions,indices)}else{0.0}})
}
 ...
 while faces.len()>target_triangles{
  ...
  for triangle in &faces{
   for e in 0..3{
    ...
    let (keep,drop,cost)=if cost_left<=cost_right{(left,right,cost_left)}else{(right,left,cost_right)};
