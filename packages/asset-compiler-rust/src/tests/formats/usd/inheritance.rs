...
//!
//! Textures have their own file, `textures.rs`; invalid faces `faces.rs`.
use super::driver::{compile_layer, wrap};
use super::*;

/// Two named materials, enough to tell which one wins.
pub(in crate::tests) const PAIR: &str = r#"
    def Material "M"
    {
...
pub(in crate::tests) fn bound(gltf: &Value, mesh: usize) -> Vec<Value> {
...
pub(in crate::tests) fn quad(name: &str, extra: &str) -> String {
    format!(
        r#"
    def Mesh "{name}"
    {{
