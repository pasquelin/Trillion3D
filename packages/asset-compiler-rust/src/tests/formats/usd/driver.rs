...
//! `refusal.rs`; the golden scene is in `golden.rs`.
//!
...
pub(in crate::tests) fn plugin(name: &str) -> &'static dyn plugins::scene::ScenePlugin {
...
pub(in crate::tests) fn temp_dir(tag: &str) -> PathBuf {
    scratch("usd", tag)
}
...
pub(in crate::tests) fn compile_layer(tag: &str, body: &str) -> GoldenRun {
...
pub(in crate::tests) fn wrap(header: &str, body: &str) -> String {
    format!("#usda 1.0\n(\n    defaultPrim = \"Root\"\n{header})\n\ndef Xform \"Root\"\n{{\n{body}\n}}\n")
}

/// A square mesh, two triangles, without a material: enough that a scene is not empty.
pub(in crate::tests) const QUAD: &str = r#"
    def Mesh "Quad"
    {
