...
pub(in crate::tests) fn quad(name: &str, parent: &str) -> String {
    format!(
...
pub(in crate::tests) fn shaded(name: &str, shader: &str) -> String {
    format!(
...
pub(in crate::tests) fn compile_ma(tag: &str, body: &str) -> GoldenRun {
...
pub(in crate::tests) fn translations(gltf: &Value, name: &str) -> Vec<[f64; 3]> {
    gltf["nodes"]
        .as_array()
        .expect("nodes")
        .iter()
...
pub(in crate::tests) fn matrix(node: &Value) -> Vec<f64> {
...
pub(in crate::tests) fn normals(run: &GoldenRun, gltf: &Value, part: &Value) -> Vec<[f32; 3]> {
...
pub(in crate::tests) fn close(found: [f32; 3], wanted: [f32; 3]) -> bool {
