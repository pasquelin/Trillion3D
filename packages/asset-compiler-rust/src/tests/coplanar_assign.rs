...
fn assign_layers_gives_the_longest_overlap_chain_not_a_rank() {
  ...
  let overlaps = vec![
    Overlap { a: 0, b: 1, cells: 5, area: 1.0 },
    Overlap { a: 0, b: 2, cells: 5, area: 1.0 },
  ];
  let (assigned, overflow) = assign::layers(&surfaces, &overlaps, 15);
  assert_eq!(overflow, 0);
  assert_eq!(assigned[0], 0, "le sol ne recouvre rien : couche 0");
  assert_eq!(assigned[1], 1, "un seul recouvrement au-dessus du sol : couche 1");
  assert_eq!(assigned[2], 1, "deux surfaces disjointes au-dessus du même sol partagent la couche 1");
}
...
fn assign_layers_respects_priority_over_area() {
  ...
  let overlaps = vec![Overlap { a: 0, b: 1, cells: 5, area: 1.0 }];
  let (assigned, _) = assign::layers(&surfaces, &overlaps, 15);
  assert_eq!(assigned[1], 0, "priorité 0 : la petite surface reste dessous malgré son aire");
  assert_eq!(assigned[0], 1, "priorité 5 : la grande surface passe dessus malgré son aire");
}
...
fn assign_layers_breaks_area_ties_by_source_order() {
  ...
  let overlaps = vec![Overlap { a: 0, b: 1, cells: 5, area: 1.0 }];
  let (assigned, _) = assign::layers(&surfaces, &overlaps, 15);
  assert_eq!(assigned[0], 0, "l'objet source le plus ancien reste dessous à aire égale");
  assert_eq!(assigned[1], 1, "l'objet source le plus récent passe dessus à aire égale");
}
...
fn assign_layers_caps_at_fifteen_and_counts_the_overflow() {
  ...
  let overlaps: Vec<Overlap> = (0..17)
    .map(|i| Overlap { a: i, b: i + 1, cells: 5, area: 1.0 })
    .collect();
  let (assigned, overflow) = assign::layers(&surfaces, &overlaps, 15);
  assert_eq!(assigned[14], 14, "sous le plafond, la couche suit la profondeur de la chaîne");
  assert_eq!(assigned[15], 15, "à la limite : couche 15, pas encore de débordement");
  assert_eq!(assigned[16], 15, "au-delà : plafonné à 15");
  assert_eq!(assigned[17], 15, "toujours plafonné à 15");
  assert_eq!(overflow, 2, "deux surfaces (16 et 17) ont dépassé le plafond");
}
