...
fn a_jpeg_icc_profile_is_counted_unless_it_names_srgb() {
    ...
    assert_eq!(notes("jpeg autre", &with_app2(&base, OTHER)), vec![IGNORED]);
...
fn a_psd_icc_profile_is_counted_unless_it_names_srgb() {
    ...
    assert_eq!(
        notes("psd autre", &with_resource(&base, OTHER)),
        vec![IGNORED]
    );
    assert!(notes("psd sRGB", &with_resource(&base, SRGB)).is_empty());
}
...
fn jpeg_without_profile() -> Vec<u8> {
...
fn with_app2(base: &[u8], description: &str) -> Vec<u8> {
    ...
    payload.extend_from_slice(&icc_profile(description));
...
fn with_resource(base: &[u8], description: &str) -> Vec<u8> {
    let icc_profile = icc_profile(description);
    let mut block = Vec::from(*b"8BIM");
    block.extend_from_slice(&1039u16.to_be_bytes());
    block.extend_from_slice(&[0, 0]);
    block.extend_from_slice(&(icc_profile.len() as u32).to_be_bytes());
    block.extend_from_slice(&icc_profile);
    if block.len() % 2 != 0 {
        block.push(0);
    }
    let at = 26 + 4;
