...
pub(super) fn compile_primitive(
    ...
    let mut attributes = if dag_primitive {
        compiler_primitive_attributes::decode_page_attributes(
            g,
            bin,
            p,
            positions.count,
    ...
    let DagResult {
        ...
        build_dag_primitive(o, &mut pos, &mut attributes, &index_values, demand)?
    } else {
        DagResult::default()
    };
    ...
    let coarse_vertices = (added_vertices > 0)
        .then(|| {
            compiler_source_extend::write_coarse_vertices(
                directory,
                (*old, *primitive),
                &pos,
                &attributes,
