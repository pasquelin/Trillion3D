...
/// previews in the head's sidecar; returns the root.
pub(crate) fn write_manifest(
    ...
    // The page of `primitives` and `previews` under `top`, written with its sidecar; its slot.
    let page = |top: &Map<String, Value>, primitives: &[Value], previews| -> Result<String> {
        ...
        write_page(&MANIFEST_PAGES, directory, &page, &[])
    };
    let primitives = result[MANIFEST_PAGES.records]
        .as_array()
        .map_or(&[][..], Vec::as_slice);
