...
fn classify_link(links:&[(u32,u32)])->&'static str{
 ...
 let slot=|ids:&mut Vec<u32>,neighbours:&mut Vec<[u32;2]>,degree:&mut Vec<u8>,vertex:u32|->usize{
...
pub fn classify_topology(indices:&[u32],vertex_count:usize)->Result<TopologyReport>{
 ...
 for face in 0..triangle_count{
  ...
  for e in 0..3{
   let start=tri[e];let end=tri[(e+1)%3];
 ...
 while cursor<halves.len(){
  ...
  let n=end-cursor;
  if n==1{boundary+=1;}
  else if n==2&&halves[cursor].2==halves[cursor+1].3&&halves[cursor].3==halves[cursor+1].2{manifold+=1;}
  else{non_manifold+=1;}
  cursor=end;
 }
 ...
 Ok(TopologyReport{triangles:triangle_count,boundary_edges:boundary,manifold_edges:manifold,non_manifold_edges:non_manifold,interior_vertices:interior,boundary_vertices,locked_vertices:locked,unused_vertices:unused,manifold:non_manifold==0&&locked==0})
}
#[cfg(test)] mod tests{
 use super::*;
