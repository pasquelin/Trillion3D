...
pub fn parse_compiler_args(
  ...
  _=>return Err("Usage: web-geometry-compiler SOURCE CACHE [slice|full] [triangles] RESOURCE_BASE_URL\n       web-geometry-compiler SOURCE CACHE [slice|full] [triangles] [threads] [RAM_MB] RESOURCE_BASE_URL [none|qem-endpoints|qem-attributes] [--textures-format=bc7|astc|both|none]\n       SOURCE is a directory or a file in one of the formats --version lists".into()),
 };
    if !crate::dag::DagStrategy::NAMES.contains(&simplification.as_str()) {
        return Err("simplification must be none, qem-endpoints or qem-attributes".into());
    }
    Ok(Options {
