...
/// Le relevé d'une conversion précédente vit hors de `imports/`, qui ne porte que des scènes.
fn probe_path(cache: &Path, base: &str) -> PathBuf {
    cache
        .join("native")
        .join("imports-externes")
        .join(format!("{base}.json"))
}

/// Les fichiers qu'une conversion précédente des mêmes entrées avait ouverts, rehachés maintenant.
pub(super) fn expected(cache: &Path, base: &str) -> Vec<External> {
    let Ok(bytes) = fs::read(probe_path(cache, base)) else {
        return Vec::new();
    };
...
pub(super) fn write_probe(cache: &Path, base: &str, files: &[External]) -> Result<()> {
    let path = probe_path(cache, base);
    if files.is_empty() {
