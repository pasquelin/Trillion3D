...
`scene.gltf` and `scene.bin` are the quad from `fixtures/hdr/`, its base colour replaced by
...
- `scene.gltf` and `scene.bin` take the geometry of the `fixtures/hdr/` golden, from the same corpus and
