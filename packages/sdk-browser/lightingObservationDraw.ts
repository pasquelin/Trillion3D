...
import { createObservationProgram } from './lightingObservationProgram.ts';
import type { ObservationResources, ObservationTexture } from './lightingObservationResources.ts';
import { WebglClusterGeometry } from './webglClusterGeometry.ts';
import { submitDiagnosticMesh } from './webglClusterSubmit.ts';
 ...
 *  geometry cache, rebuilt whole after a context restore. */
class ObservationPass {
  ...
  constructor(gl: WebGL2RenderingContext, surfaceCount: number) {
    this.gl = gl;
    this.program = createObservationProgram(gl, surfaceCount);
  ...
  draw(
    ...
    gl.useProgram(this.program);
    this.bindTextures(
      [resources.texture, resources.surfaceTexture, resources.bvhTexture],
      uploadAll,
    );
    gl.disable(gl.BLEND);
    ...
    gl.uniform2f(this.at('cacheSize'), uniforms.cacheSize[0], uniforms.cacheSize[1]);
 ...
 * a context restore the pass is rebuilt and every texture uploaded again.
 */
export function createObservationDraw(
  ...
  return {
    drawHostGeometry(camera: HostDrawCamera, output: HostDrawOutput) {
      ...
      pass ??= new ObservationPass(gl, resources.surfaceCount);
      pass.draw(resources, meshes, camera, output.toneMapped, uploadAll);
      uploadAll = false;
    },
    dispose() {
