...
test('THREE.LOD includes transparent simplification and merges its mixed cover in source order',()=>{
 ...
 const pages=[0,1,2,3].map(id=>({id,url:String(id),count:3,min:[-1,-1,0],max:[1,1,0],bytes:12,sha256:'x',role:id<3?'exact' as const:'coarse' as const}));
 const leaf=(id:number)=>({min:pages[id].min,max:pages[id].max,page:id});
 const hierarchy={min:[-1,-1,0],max:[1,1,0],children:[leaf(2),{min:[-1,-1,0],max:[1,1,0],errorObject:0,coarsePages:[3],children:[leaf(1),leaf(0)]}]};
 const backend=threeLodBackend({source,metadata:{primitives:[{mesh:0,primitive:0,pass:'clustered-blend',pages,hierarchy}]},indices:new Map([['0',new Uint32Array([0,1,2])],['1',new Uint32Array([0,2,3])],['2',new Uint32Array([0,3,4])],['3',new Uint32Array([0,1,3])]]),associations:new Map([[mesh,{meshes:0,primitives:0}]])});
...
test('THREE.LOD keeps one exact level for source-ordered transparent pages',()=>{
 ...
 const pages=[{id:0,url:'0',count:3,min:[-1,-1,0],max:[1,1,0],bytes:12,sha256:'x',role:'exact' as const}];
 const backend=threeLodBackend({source,metadata:{primitives:[{mesh:0,primitive:0,pass:'clustered-blend',pages,hierarchy:{min:[-1,-1,0],max:[1,1,0],page:0}}]},indices:new Map([['0',new Uint32Array([0,1,2])]]),associations:new Map([[mesh,{meshes:0,primitives:0}]])});
 const lod=backend.scene.children.find(o=>(o as THREE.LOD).isLOD) as THREE.LOD;
