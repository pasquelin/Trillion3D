...
test('autonomous pages add, move and remove an instance while keeping page coverage',async()=>{
 ...
 const descriptor={url:'triangle-geometry.bin',sha256:'x',bytes:encoded.data.length,formatVersion:2 as const,codec:'meshopt' as const,vertexCount:encoded.vertexCount,indexCount:encoded.indexCount,flags:encoded.flags,uncompressedBytes:encoded.uncompressedBytes};
 const page={id:0,url:'triangle.bin',sha256:'x',bytes:12,count:3,min:[-.5,-.5,0],max:[.5,.5,0],role:'exact' as const,start:0,
  level:0,lodError:0,sphere:[0,0,0,1],parentError:null,parentSphere:null,group:null,source:null,geometry:descriptor};
 const metadata={errorModel:'dag-group-qem-v1',clusterStrategy:'dag-groups',primitives:[{mesh:0,primitive:0,pass:'exact-clusters',clusterStrategy:'dag-groups',pages:[page],structure:{version:1,roots:[0],groups:[]}}]} as unknown as ClusterManifest;
 ...
 try{
  ...
  backend.replaceGeometryPage?.('triangle-geometry.bin',replacementPage);backend.acceptGeometryPage?.('triangle-geometry.bin',await decodeGeometryPage(encoded.data));backend.dropPage?.('triangle-geometry.bin');backend.render(camera);
