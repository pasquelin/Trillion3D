...
test('light packing preserves world transforms, linear color, intensity and spot range', () => {
  ...
  parent.add(spot);
  spot.target.position.set(10, 0, 0);
  source.add(spot.target);
  const { count, data } = packSceneLights(source);
  assert.equal(count, 1);
  assert.deepEqual([...data.slice(4, 12)], [10, 5, 0, 3, 0.25, 0.5, 1, 3]);
  assert.deepEqual([...data.slice(12, 16)], [0, 1, 0, 20]);
  assert.ok(Math.abs(data[20] - 0.5) < 1e-6);
  assert.ok(Math.abs(data[21] - Math.sqrt(3) / 2) < 1e-6);
  spot.intensity = 0;
  assert.equal(
    packSceneLights(source).data[11],
    0,
    'an extinguished light must not activate the default rig',
  );
});
...
test('Three reference and GPU adapter use the same directional target in world space', () => {
  ...
  for (const [i, value] of [1, 3, 2].entries())
    assert.ok(Math.abs(packed[12 + i] - value / Math.sqrt(14)) < 1e-6);
});

test('unsupported and excessive lights fail explicitly instead of being silently omitted', () => {
  const source = new THREE.Group();
  source.add(new THREE.RectAreaLight());
  assert.throws(() => packSceneLights(source), /UNSUPPORTED_SCENE_LIGHT/);
  source.clear();
  for (let i = 0; i < 257; i++) source.add(new THREE.PointLight());
  assert.throws(() => packSceneLights(source), /LIGHT_BUDGET/);
});
