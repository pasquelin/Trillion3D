...
function target(){
 const accepted:string[]=[];let syncs=0;
 return {accepted,get syncs(){return syncs;},acceptPage(url:string){accepted.push(url);},syncResident(){syncs++;}};
}
...
test('a page already waiting for a target is queued once, and each target keeps its own residency',()=>{
 ...
 assert.equal(queue.queue({} as ArrivalTarget,'p0',page),false,'sans acceptPage, rien à livrer');
 assert.equal(queue.drain(),2,'le budget de pages arrête le drain');
 assert.deepEqual(a.accepted,['p0']);assert.deepEqual(b.accepted,['p0']);
 assert.equal(a.syncs,1);assert.equal(b.syncs,1);
 ...
 assert.equal(a.syncs,2);assert.equal(b.syncs,1,'un destinataire non touché ne resynchronise pas');
});
