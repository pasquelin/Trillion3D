 ...
 * against them at the two boundaries that read a surface (`hostSurfaceImport.ts`,
 * `hostSurfaceGate.ts`). Naming the rendering library to get those numbers would put the library
 * back on the engine path for a comparison that is not a computation, so they are named here
 * instead — the same move `materialSide.ts` already makes for the three face constants.
 *
 ...
 *
 * `hostSurfaceConstants.test.ts` holds them to the host library's own constants, value by value:
