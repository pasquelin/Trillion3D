 ...
 * the blend material read (`blendSurface`), and store what they read instead of lighting it — a
 * surface buffer of the opaque resolve's layout (`surfaceBuffer.ts`), plus the hardware depth of
 * the surface. The fullscreen composite (`webgpuWaterCompositeWgsl.ts`) lights it once per pixel,
 * on the frozen backdrop, whatever the number of surfaces the pixel stacked.
 *
 * The fourth target carries the item rank, one-based so that zero means "no water here", and the
...
@fragment fn fsWater(in:VSOut,@builtin(front_facing) front:bool)->WaterOut{
 ...
 return WaterOut(vec4f(s.rgb,s.metal),vec4f(s.N,s.rough),vec4f(s.emissive,s.ao),(in.item+1u)|(opacity<<${WATER_RANK_BITS}u),s.request);
}
`;
...
fn waterOpacity(packed:u32)->f32{return f32(packed>>${WATER_RANK_BITS}u)/65535.0;}
`;
