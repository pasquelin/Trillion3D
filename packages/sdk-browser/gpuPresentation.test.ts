...
test('presentation acquires a fresh canvas target after resizing, shared by fused and copy paths',()=>{
 ...
 const sizes:number[][]=[],views:object[]=[],passes:GPURenderPassDescriptor[]=[];
 let configured:GPUCanvasConfiguration|undefined,unconfigured=false;
 const canvas={width:1,height:1,getContext:()=>({
  configure(value:GPUCanvasConfiguration){configured=value;},
  getCurrentTexture(){sizes.push([canvas.width,canvas.height]);return {createView(){const view={};views.push(view);return view;}};},
  unconfigure(){unconfigured=true;},
 })} as unknown as HTMLCanvasElement;
 const device={createBindGroupLayout:()=>({}),createShaderModule:()=>({}),createRenderPipeline:()=>({}),createPipelineLayout:()=>({}),createBindGroup:()=>({})} as unknown as GPUDevice;
 const encoder={beginRenderPass(descriptor:GPURenderPassDescriptor){passes.push(descriptor);return {setPipeline(){},setBindGroup(){},draw(){},end(){}};}} as unknown as GPUCommandEncoder;
 const presenter=createGpuPresenter(device,canvas);
 try{
  const first=presenter.targetView(32,24),second=presenter.targetView(64,48);
  assert.notEqual(first,second,'swapchain views cannot survive a new frame');
  assert.deepEqual(sizes,[[32,24],[64,48]]);
