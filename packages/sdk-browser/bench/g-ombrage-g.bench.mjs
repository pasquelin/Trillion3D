// G3 : l'ombrage par pixel du visbuffer CPU. La direction du soleil, sa longueur, la couleur du sol
// et celle du ciel ne dépendent pas du pixel ; elles étaient recalculées et réallouées à chacun.
import * as THREE from 'three';
import { shadeLit } from '../visibilityLighting.ts';
import { triangleAt } from '../visibilityMath.ts';
import { compare, graine } from '../../sdk-core/bench/banc.mjs';
import { verifieEtDeposeG } from '../../sdk-core/bench/bancG.mjs';
import { referenceShadeLit } from './oracles/g-ombrage.mjs';
import { cameraMoteur } from '../cameraFixture.ts';

const alea = graine(0x6017);
/** Poids barycentriques hostiles : zéro signé, NaN, infinis, dénormal. */
const HOSTILES = [0, -0, NaN, Infinity, -Infinity, 5e-324, 1];

/** Une texture RGBA de 8×8 tirée de la graine : assez pour exercer les trois échantillonnages. */
function texture(depart) {
  const data = new Uint8Array(8 * 8 * 4),
    tire = graine(depart);
  for (let i = 0; i < data.length; i++) data[i] = Math.floor(tire() * 256) & 255;
  const map = new THREE.Texture();
  map.image = { data, width: 8, height: 8 };
  map.wrapS = THREE.RepeatWrapping;
  map.wrapT = THREE.ClampToEdgeWrapping;
  return map;
}

/** Une page d'un seul triangle, avec normales, tangentes et coordonnées de texture. */
function page(depart) {
  const tire = graine(depart);
  const positions = new Float32Array(9),
    normales = new Float32Array(9),
    tangentes = new Float32Array(12),
    uvs = new Float32Array(6);
  for (let i = 0; i < 9; i++) {
    positions[i] = (tire() - 0.5) * 4;
    normales[i] = tire() * 2 - 1;
  }
  for (let i = 0; i < 12; i++) tangentes[i] = tire() * 2 - 1;
  for (let i = 0; i < 6; i++) uvs[i] = tire();
  return {
    array: new Uint32Array([0, 1, 2]),
    attributes: {
      position: new THREE.BufferAttribute(positions, 3),
      normal: new THREE.BufferAttribute(normales, 3),
      tangent: new THREE.BufferAttribute(tangentes, 4),
      uv: new THREE.BufferAttribute(uvs, 2),
    },
    matrix: new THREE.Matrix4().makeRotationY(0.7).setPosition(0.3, -0.2, 0.9),
    material: new THREE.MeshStandardMaterial(),
  };
}

const camera = new THREE.PerspectiveCamera(60, 16 / 9, 0.1, 500);
camera.position.set(2, 3, 8);
camera.updateMatrixWorld();
camera.updateProjectionMatrix();
const viewProj = new THREE.Matrix4().multiplyMatrices(
  camera.projectionMatrix,
  camera.matrixWorldInverse,
);
/** La vue-projection du banc et la convention de profondeur de sa caméra, qui est celle de WebGL. */
const depthCam = { viewProjection: viewProj.elements };

const matiere = (cartes) => ({
  baseColor: [0.8, 0.6, 0.4],
  metalness: 0.3,
  roughness: 0.4,
  lit: true,
  doubleSided: true,
  backSide: false,
  alphaTest: 0,
  normalMap: cartes ? texture(0x11) : undefined,
  normalScale: 1.25,
  normalScaleY: -0.75,
  aoMap: cartes ? texture(0x22) : undefined,
  aoIntensity: 0.8,
  emissive: [0.05, -0, 0.2],
  emissiveMap: cartes ? texture(0x33) : undefined,
  transmission: 0,
});

/** Un lot de pixels : la même page, le même triangle, des poids et des matières qui varient. */
function pixels(nombre, cartes, hostiles) {
  const p = page(0x77 ^ nombre),
    tri = triangleAt(p, 0, depthCam, 1600, 900),
    mat = matiere(cartes);
  const lot = [];
  for (let i = 0; i < nombre; i++) {
    const w0 = hostiles ? HOSTILES[i % HOSTILES.length] : alea(),
      w1 = hostiles ? HOSTILES[(i + 3) % HOSTILES.length] : alea() * (1 - w0);
    lot.push({
      page: p,
      tri,
      affine: { area: alea() * 40 - 20 },
      bary: { w0, w1, w2: 1 - w0 - w1 },
      uv: [alea() * 3 - 1, alea() * 3 - 1],
      mat,
      rgb: [alea(), alea(), i % 5 === 0 ? -0 : alea()],
      metalness: i % 7 === 0 ? 1 : alea(),
      roughness: i % 11 === 0 ? 0 : alea(),
    });
  }
  return lot;
}

/** La caméra du moteur du décor : l'ombrage optimisé ne lit plus que celle-ci. */
const vue = cameraMoteur(camera);

/** Un tour : chaque pixel du lot ombré, les trois canaux écrits bout à bout. */
const passe = (ombre, oeil) => (lot) => {
  const sortie = new Float64Array(lot.length * 3);
  for (let i = 0; i < lot.length; i++) {
    const p = lot[i];
    const rgb = ombre(
      p.page,
      p.tri,
      p.affine,
      p.bary,
      p.uv,
      p.mat,
      p.rgb,
      p.metalness,
      p.roughness,
      oeil,
    );
    sortie[i * 3] = rgb[0];
    sortie[i * 3 + 1] = rgb[1];
    sortie[i * 3 + 2] = rgb[2];
  }
  return sortie;
};

const lignes = [
  await compare({
    calcul: 'G3 ombrage par pixel du visbuffer',
    fichier: 'packages/sdk-browser/visibilityLighting.ts',
    cas: [
      {
        nom: '20 000 pixels, cartes normale/occlusion/émission',
        entree: pixels(20000, true, false),
        taille: 20000,
      },
      { nom: '20 000 pixels sans carte', entree: pixels(20000, false, false), taille: 20000 },
      {
        nom: 'poids hostiles : zéro signé, NaN, infinis',
        entree: pixels(49, true, true),
        taille: 49,
      },
      { nom: 'un seul pixel', entree: pixels(1, false, false), taille: 1 },
      { nom: 'aucun pixel', entree: [], taille: 0 },
    ],
    reference: passe(referenceShadeLit, camera),
    optimisee: passe(shadeLit, vue),
    options: { tours: 200, budgetMs: 4000, alterne: true },
  }),
];

verifieEtDeposeG(
  'g-ombrage',
  'G3 rend les mêmes trois canaux, au bit près, sur chaque pixel',
  lignes,
);
