// Mesure absolue du Hi-Z : profondeur visbuffer et test d'occlusion rectangle. Aucun oracle ici :
// ces deux calculs n'ont pas d'implémentation d'avant à confronter, leur justesse est tenue par
// `hizDepth.test.ts` et `hizOcclusion.test.ts`. Chaque ligne le dit plutôt que de le taire.
...
const depthResult = await mesure({
  ...
  motif: 'temps seul — justesse dans hizDepth.test.ts',
...
const hizResult = await mesure({
  ...
  calcul: parcours,
  motif: 'temps seul — justesse dans hizOcclusion.test.ts',
  options: { tours: 200, budgetMs: 1000 },
});
...
rapport(
  ...
  'Hi-Z : profondeur et occlusion — mesure absolue',
);
