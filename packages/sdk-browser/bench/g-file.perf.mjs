// G5 et G6 : les deux recherches linéaires du chemin de streaming.
...
const resG5 = await mesure({
  nom: 'G5 retrait demande annulée de file',
...
const resG6 = await mesure({
  nom: 'G6 empilement en attente',
  fichier: 'packages/sdk-browser/explorerDraw.ts',
  cas: [
    { nom: '4 000 adresses à empiler', entree: urlsEmpilees, taille: 4000 },
  ],
  calcul: (urls) => {
    const arr = [];
    const set = new Set();
...
rapport('g-file', [resG5, resG6], 'G5 et G6 retirent et empilent exactement les mêmes adresses');
