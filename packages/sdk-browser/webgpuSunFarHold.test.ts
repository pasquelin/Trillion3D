...
function sunFarRt(readSceneProxy?: () => Promise<SceneProxy>) {
  const run = { revisions: createFrameRevisions(), frameHold: createFrameHold(1) };
  ...
  run.frameHold.keep(run.revisions);
  run.frameHold.keep(run.revisions);
  const rt = {
    run,
...
function assertAdoption(rt: ReturnType<typeof sunFarRt>, avant: number) {
  ...
  assert.equal(rt.run.revisions.resources, avant + 1, 'le proxy adopté est une ressource de plus');
  assert.equal(rt.run.frameHold.stable, false, 'la tenue est cassée');
  assert.equal(rt.run.frameHold.same(rt.run.revisions), false, 'les révisions ont bougé');
}

test('GEO-02 : le proxy chargé pour l’ombre lointaine annonce son adoption', async () => {
  const rt = sunFarRt(async () => sceneProxy());
  assert.equal(rt.run.frameHold.stable, true, 'la tenue est armée avant l’adoption');
  const avant = rt.run.revisions.resources;
