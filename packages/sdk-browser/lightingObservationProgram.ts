 ...
 * display chain in place of the two host includes: the filmic curve when the frame is tone
 * mapped, then the sRGB transfer, the same links as the cluster program's.
 */
...
`;
const TONE_MAPPING = 'if(toneMapped)gl_FragColor.rgb=aces(gl_FragColor.rgb);';
