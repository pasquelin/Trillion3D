...
export async function awaitBackendPages(backend:PageBackend,camera:THREE.PerspectiveCamera,load:(urls:string[])=>Promise<void>){
 ...
 if(backend.flush){
  ...
  if(backend.syncResident)backend.syncResident();else backend.render(camera);
 }
}
