...
test('a corrupt page aborts sibling fetches before they allocate remaining indices',async()=>{
 ...
 const pages=[{url:'good.bin',bytes:4,sha256:'pending'},{url:'bad.bin',bytes:4,sha256:'nope'},{url:'later.bin',bytes:4,sha256:'pending'}];
 ...
 globalThis.fetch=async(url,init)=>{
  const name=String(url).split('/').pop()??'';started.push(name);
  const signal=init?.signal;
  if(name==='later.bin')await Promise.race([hold.promise,new Promise((_,reject)=>{const fail=()=>reject(signal?.reason??new DOMException('Aborted','AbortError'));signal?.addEventListener('abort',fail,{once:true});if(signal?.aborted)fail();})]);
  finished.push(name);
