...
${FULLSCREEN_VERTEX}
${OUTPUT_COLOR_WGSL}
@fragment fn compose(@builtin(position) pixel:vec4f)->@location(0) vec4f{
...
export async function createDeferredLighting(device:GPUDevice,lights:GPUBuffer){
 ...
 try{
  ...
  const make=(module:GPUShaderModule,bind:GPUBindGroupLayout,entryPoint:string,format:GPUTextureFormat)=>{const descriptor:GPURenderPipelineDescriptor={layout:device.createPipelineLayout({bindGroupLayouts:[bind]}),vertex:{module,entryPoint:'fullscreen'},fragment:{module,entryPoint,targets:[{format}]},primitive:{topology:'triangle-list'}};return device.createRenderPipelineAsync?device.createRenderPipelineAsync(descriptor):Promise.resolve(device.createRenderPipeline(descriptor));};
  ...
  return {
   ...
   compose(encoder:GPUCommandEncoder,target:GPUTextureView,clear:GPUColor){if(!composeGroup)throw new Error('SURFACE_NOT_BOUND');const pass=encoder.beginRenderPass({label:'WG HDR composition',colorAttachments:[{view:target,loadOp:'clear',storeOp:'store',clearValue:clear}]});pass.setPipeline(compose);pass.setBindGroup(0,composeGroup);pass.draw(3);pass.end();},
