 ...
 * locale de caméra, et nomme les consommateurs autorisés à lire la pose résolue.
 *
 ...
 *  2. LIRE. `cameraWorldPosition` et `cameraPose` sont les lectures de la pose monde. Elles
 *     résolvent d'abord : aucun appelant n'a à se souvenir de le faire. Une lecture
 *     directe de `camera.position` est un défaut, jamais une optimisation — sous un rig elle nomme
 *     un point qui n'existe pas dans le monde.
 *
 ...
 * WebGL appelle `viewChanged`), qui compare les seize nombres de `matrixWorldInverse`. L'ORDRE est
 * donc la garantie : une entrée d'image résout la pose AVANT le seuil adaptatif
 * (`resolvePixelError`) et AVANT l'empreinte de vue, et l'empreinte est relue AVANT la décision de
 * tenue. Un rig que l'hôte déplace sans toucher à la caméra fait ainsi bouger l'empreinte et rejoue
 * l'image ; sans la résolution préalable, l'empreinte ne verrait rien bouger et l'image serait
 * tenue à tort. Une résolution faite en aval, elle, n'ouvre ni ne ferme aucune porte : elle
 * n'atteint aucune révision.
 */
