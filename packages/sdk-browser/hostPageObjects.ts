 ...
 * renderer — the shared adapter of `threeSceneAdapter.ts`, with the host lights the contract
 ...
 * the explorer's camera is (`hostGraphObjects.ts`, which builds what the EXPLORER publishes to
 * its host; this file builds what a BACKEND draws). Since #78 lot 3 the path itself —
 * `autonomousPages.ts`, `autonomousGeometry.ts`, `autonomousInstances.ts` — names no library:
 * it holds these objects through the shapes of `hostResources.ts` and hands them back here.
 *
...
import type { HostGraphGeometry } from './hostGraphResources.ts';
 ...
 *  holder of host buffers counts them (`sceneMeshes.ts`). */
 ...
 *  names it (`materialSide.ts`); nothing else is converted. */
