...
export interface PriorityRecord {
 array?:Uint32Array;url:string;streamUrl?:string;
 lodError?:number;sphere?:number[];parentError?:number|null;parentSphere?:number[]|null;
 min:number[];max:number[];matrix:THREE.Matrix4;
}
export interface PriorityCamera {matrixWorldInverse:THREE.Matrix4;near:number}
...
export const PRIORITY_VISIBLE=1,PRIORITY_PREFETCH=3;
...
const centre=new Float64Array(4),bounds=new Float64Array(4);
function boundsSphere(record:PriorityRecord,out:Float64Array){
 out[0]=(record.min[0]+record.max[0])/2;out[1]=(record.min[1]+record.max[1])/2;out[2]=(record.min[2]+record.max[2])/2;
 out[3]=Math.hypot(record.max[0]-out[0],record.max[1]-out[1],record.max[2]-out[2]);
 return out;
}

interface Slot {url:string;error:number;distance:number}
/**
 ...
 */
export function orderPendingUrls(records:readonly PriorityRecord[],camera:PriorityCamera,pixelScale:readonly number[],into:string[]):string[]{
 into.length=0;
 const focal=Math.max(pixelScale[0],pixelScale[1]),near=camera.near;
 ...
 for(let index=0;index<records.length;index++){
  ...
  if(!frame){
   ...
   frame={view,stretch:maxStretch(view as unknown as readonly number[])};views.set(record.matrix,frame);
  }
  ...
  if(sphere&&sphere.length===4){
   ...
   pixels=error==null?Infinity:clusterErrorPixels(error,frame.stretch,centre[0],centre[1],centre[2],centre[3],focal,near);
  ...
  if(!held){slots.set(key,{url:key,error:pixels,distance});continue;}
  if(pixels>held.error||(pixels===held.error&&distance<held.distance)){held.error=pixels;held.distance=distance;}
 }
...
export function pixelScaleOf(camera:{projectionMatrix:THREE.Matrix4},viewport:readonly number[]|undefined,into:number[]){
