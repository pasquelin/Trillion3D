 ...
 * `hostShadedMaterial.ts` and the named constants of `hostSurfaceConstants.ts`; what the engine
 * computes with afterwards is the imported record of `hostSurfaceImport.ts`.
 */

import type { HostAttribute, HostAttributes, HostMaterials } from './hostResources.ts';
import type { HostMap, HostShadedMaterial, HostStoredTexture } from './hostShadedMaterial.ts';
import {
  HOST_BLENDING_NORMAL,
  HOST_MAPPING_UV,
  HOST_NORMAL_MAP_TANGENT_SPACE,
} from './hostSurfaceConstants.ts';
import { declaresCompileHook } from './hostMaterialHook.ts';
