import type {BackendDiagnostic,DiagnosticDetail} from './backendTypes.ts';
...
const newSessionId=()=>`diagnostics-${Date.now().toString(36)}-${(++sessionCounter).toString(36)}`;
const isFrameDiagnostic=(diagnostic:BackendDiagnostic)=>diagnostic.phase==='frame'||diagnostic.context?.kind==='frame';
 ...
 */
export function createDiagnosticChannel(observer:DiagnosticObserver|undefined,options:DiagnosticChannelOptions={}):DiagnosticChannel{
 const enabled=options.enabled??observer!==undefined;
 const detail=options.detail??'trace';
 const maxBuffer=Number.isSafeInteger(options.maxBuffer)&&options.maxBuffer!>0?options.maxBuffer!:65536;
 ...
 const stamp=(input:BackendDiagnostic,sequence:number,queuedAt:number):BackendDiagnostic=>({
  ...input,
  sequence,
  sessionId,
  queuedAt,
  createdAt:input.createdAt??(typeof input.context.createdAt==='number'?input.context.createdAt:queuedAt),
 });
 const lossRecord=():BackendDiagnostic|undefined=>{
  if(!droppedCount||firstDroppedSequence===undefined||lastDroppedSequence===undefined)return undefined;
  const queuedAt=now();
  const record:BackendDiagnostic={
   phase:'diagnostic-loss',
   message:'Diagnostics were dropped because the bounded buffer was full',
   context:{kind:'loss',dropped:true,lostCount:droppedCount,firstSequence:firstDroppedSequence,lastSequence:lastDroppedSequence,bufferCapacity:maxBuffer},
   sequence:lastDroppedSequence,
   sessionId,
   queuedAt,
   createdAt:queuedAt,
  };
  droppedCount=0;firstDroppedSequence=undefined;lastDroppedSequence=undefined;
  return record;
 };
 const deliver=(record:BackendDiagnostic)=>{try{observer?.(record);}catch{/* Observer failures are isolated from the engine. */}};
 const drain=()=>{
  if(!enabled)return;
  const batch=queue.splice(0,queue.length);
  for(const record of batch)deliver(record);
  const loss=lossRecord();if(loss)deliver(loss);
 ...
 const schedule=()=>{
  ...
  scheduled=true;
  queueMicrotask(()=>{scheduled=false;void channel.flush();});
 };
 const channel:DiagnosticChannel={
  enabled,detail,sessionId,
  emit(input){
  ...
  flush(){
   ...
   flushing=Promise.resolve().then(drain).finally(()=>{flushing=undefined;if(queue.length||droppedCount)schedule();});
   return flushing;
  },
  flushSync(){if(!enabled)return;scheduled=false;drain();},
  close(){if(closed)return;closed=true;schedule();},
