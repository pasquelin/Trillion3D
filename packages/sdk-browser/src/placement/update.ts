 ...
 * the engine has one; `follow` names each root that reads a written row. Returns the box the change
 * touched, where it was and where it now is, or `null` when no drawn root moved: a still scene pays
 * nothing downstream.
 */
