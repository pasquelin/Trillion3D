...
uniform float lineWidth,pixelRatio;uniform vec2 viewport,sprite;
 ...
 * an opaque surface (`covering`) covers its pixel whatever its alpha. Its second output marks the
 * coverage of the surfaces the curve skips (`toneMapped` false), blended as the colour is, so the
 * display chain leaves them as drawn.
 */
