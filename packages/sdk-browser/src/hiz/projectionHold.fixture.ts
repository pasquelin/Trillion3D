import { createViewFingerprint } from '../frame/viewFingerprint.ts';
...
export function createProjectionHold(slots: number) {
  ...
  // view revision also compares (`../frame/viewFingerprint.ts`); the table age is ours alone.
  ...
  return {
     ...
     * (`../camera/world.ts`): under a host rig, a camera of which only an ancestor moved has no new
