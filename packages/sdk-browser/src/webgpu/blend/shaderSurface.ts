...
fn blendSurface(in:VSOut,front:bool)->BlendSurface{
 ...
 let sample=colorSample(in.ids.x,in.uv,gradX,gradY);
 let alpha=sample.w*in.color.w;
 let rgb=in.color.xyz*sample.xyz;
 var rough=in.pbr.x;var metal=in.pbr.y;var ao=1.0;
 if(in.maps.x!=0u){rough*=dataSample(in.maps.x,in.uv,gradX,gradY).g;}
 if(in.maps.y!=0u){metal*=dataSample(in.maps.y,in.uv,gradX,gradY).b;}
 if(in.maps.w!=0u){ao+=in.alphaAo.y*(dataSample(in.maps.w,in.uv,gradX,gradY).r-1.0);}
 if(in.maps.z!=0u){
  let mapN=dataSample(in.maps.z,in.uv,gradX,gradY).xyz*2.0-vec3f(1.0);
 ...
 if(in.ids.z!=0u){emissive*=colorSample(in.ids.z,in.uv,gradX,gradY).rgb;}
