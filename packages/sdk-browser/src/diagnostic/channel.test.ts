...
import { createDiagnosticChannel } from './channel.ts';
...
test('diagnostic delivery is deferred, isolated from throwing observers, and idempotent', async () => {
  ...
  const channel = createDiagnosticChannel(
    ...
    { sessionId: 'test-session', now: () => 1700000000000 },
  );
  channel.emit(event('one'));
  assert.equal(seen.length, 0);
  await channel.flush();
  assert.equal(seen.length, 1);
  const delivered = seen[0] as {
    sequence: number;
    sessionId: string;
    queuedAt: number;
    createdAt: number;
...
test('bounded overflow emits an explicit loss record and keeps sequence provenance', async () => {
  ...
  const channel = createDiagnosticChannel((record) => seen.push(record), {
    maxBuffer: 2,
    now: () => 1700000000100,
  });
  channel.emit(event('one'));
  ...
  await channel.flush();
  assert.deepEqual(
    seen.map((record) => record.phase),
    ['one', 'two', 'diagnostic-loss'],
  );
  assert.deepEqual(
    seen.map((record) => record.sequence),
    [1, 2, 4],
  );
...
test('disabled channels and summary detail do not deliver frame volume', async () => {
  ...
  await channel.flush();
  assert.deepEqual(
    (summary as { phase: string }[]).map((record) => record.phase),
    ['lifecycle'],
  );
});
...
test('a deferred backend keeps its original event creation time', async () => {
  ...
  channel.emit({
    phase: 'backend',
    message: 'created before delivery',
    context: { createdAt: 100 },
  });
  await channel.flush();
