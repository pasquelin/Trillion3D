...
@fragment fn shade_fs(@builtin(position) pos:vec4f)->SurfaceOut{
 ...
 // and a depth view's own flag keeps the composition's display curve off it.
 ...
 if(model>=${SURFACE_MODEL.normal}u){return SurfaceOut(vec4f(rgb,0.0),vec4f(N,1.0),vec4f(0.0,0.0,0.0,1.0),select(select(3u,${DEPTH_VIEW_FLAG}u,model==${SURFACE_MODEL.depth}u),1u,model==${SURFACE_MODEL.matcap}u),request);}
