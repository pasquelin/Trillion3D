 ...
 * or stop at once: it gathers speed and brakes over `responseTime`, keeps its momentum in the
 * air, falls under `gravity`, climbs ledges up to `stepHeight`, slides along walls and stands
 * on slopes up to `maxSlope` (`sdk-core/src/collision/characterBody.ts`).
 *
 * The camera is the eye, `eyeHeight` above the feet. A host that moves the camera itself
 * teleports the body there, at rest. A still character — grounded, no speed, no key — emits
 * nothing, so the scene stays still.
 */
...
export function createCharacterCameraControls(
  ...
  const api: CharacterCameraControls = {
    ...
    unlock: () => head.unlock(),
    update(delta = 0) {
      pose.readPosition(eye);
      if (!eye.every((value, k) => value === written[k]))
        body.place(eye[0], eye[1] - api.eyeHeight, eye[2]);
  ...
  const events = {
    onLand: (impact: number) => api.onLand?.(impact),
    onJump: () => api.onJump?.(),
  };
