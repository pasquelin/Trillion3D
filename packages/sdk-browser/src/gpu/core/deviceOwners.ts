 ...
 * A closed claim's first error is said as a warning: on the console, and to the live claims'
 * diagnostics when there are any, with a count its later errors raise in place — none is
 * swallowed without a trace. The browser's own console line for an error of a closed claim's is
...
type GpuDeviceOwner = {
  ...
  /** The first error of a closed claim's; `count` then counts its errors, the later ones included. */
  closedError(message: string, count: ClosedErrorCount): void;
...
/** How many errors a closed claim raised: its tag, and a count raised in place at each error. */
type ClosedErrorCount = { readonly session: string; count: number };
...
function ownerRegistry(device: OwnedDevice) {
  ...
  /** Closed claims whose error was said, and how many they raised. */
  const said = new Map<number, ClosedErrorCount>();
