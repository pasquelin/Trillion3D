...
test("an object the closing session creates after the next claim keeps the closing one's tag", async (t) => {
  ...
  // One of its own objects, or one no tag names once the closed one's work is done: the live
  // session's, as before.
  await device.settle();
...
test("an error naming no object is the closed session's while its work runs, then the live one's", async (t) => {
  const warned = t.mock.method(console, 'warn', () => {});
  const device = fakeDevice();
  const first = owner(),
    second = owner();
  ...
  closing.release();
  // Its last submission still runs: an error that names nothing (the queue, the device) is its.
  const cancelled = device.raise('[Queue] Submit failed');
  assert.deepEqual([second.errors, second.closed.length, warned.mock.callCount()], [[], 1, 1]);
  // Cancelled: the warning carries the text, the browser prints no red line of its own.
  assert.equal(cancelled, true);
  await device.settle();
  assert.equal(device.raise('[Queue] Submit failed'), false, 'a live error keeps its line');
  assert.deepEqual(second.reasons, ['uncaptured-error']);
});

test("running out of memory is the live sessions' loss, whoever made the object", async () => {
  const device = fakeDevice();
  ...
  closing.release();
  // Named on the closed session's object, and while its work runs: still the device's condition.
...
test("with no session live, a closed one's error is said once on the console, never later", (t) => {
  const warned = t.mock.method(console, 'warn', () => {});
  const device = fakeDevice();
