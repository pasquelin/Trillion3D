...
const rankOf = (word: number) => requestRank(requestPriority(word));
...
test('the requests reach the host in requestRank order, sorted by the GPU', async () => {
  ...
  assert.ok(staged.some((word, i) => i > 0 && rankOf(word) > rankOf(staged[i - 1])));
  ...
  for (let i = 1; i < words.length; i++)
    assert.ok(rankOf(words[i]) <= rankOf(words[i - 1]), `rank ${i} rises`);
...
test('the sort mirror keeps every word through massive ties at both ends of the range', () => {
  ...
  for (let i = 1; i < sorted.length; i++) {
    assert.ok(rankOf(sorted[i]) <= rankOf(sorted[i - 1]), `rank ${i} rises`);
    // Within a rank, the order the words came in.
    if (rankOf(sorted[i]) === rankOf(sorted[i - 1])) assert.ok(sorted[i] > sorted[i - 1]);
  }
});
