 ...
 *
 * `uni.viewFlags` says what kind of view the cut serves. A camera sends zero and none of this
 * runs: its text and its verdicts are those of before the light cut.
 * - `VIEW_APPEND`: the requests of this run add to those already in the output — a frame runs
 *   one light cut per redrawn face and reads all their requests back at once.
 * - `VIEW_LIGHT`: every face of a caster writes depth, so the normal cone rejects nothing.
 * - `VIEW_PAGES`: a box reaches the cut only if it covers a page `uni.pageMask` marks.
 */
export const VIEW_APPEND = 1,
  VIEW_LIGHT = 2,
  VIEW_PAGES = 4;

export const DAG_PAGES_WGSL = `const VIEW_APPEND:u32=${VIEW_APPEND}u;
const VIEW_LIGHT:u32=${VIEW_LIGHT}u;
const VIEW_PAGES:u32=${VIEW_PAGES}u;
fn rowBits(row:u32)->u32{return (uni.pageMask[row>>2u]>>((row&3u)*8u))&0xffu;}
fn pageMissed(w:u32,bmin:vec3f,bmax:vec3f)->bool{
 if((uni.viewFlags&VIEW_PAGES)==0u){return false;}
 ...
 let s=uni.clipScale;let pad=uni.clipPad;
 ...
 let halfRows=f32(uni.pageRows)*0.5;let lastRow=i32(uni.pageRows)-1;
