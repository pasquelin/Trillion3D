...
function projectAttribute(matrix:THREE.Matrix4,position:THREE.BufferAttribute|THREE.InterleavedBufferAttribute,vi:number,viewProj:THREE.Matrix4,width:number,height:number,out:{x:number;y:number}){
 const v=projectScratch.set(position.getX(vi),position.getY(vi),position.getZ(vi)).applyMatrix4(matrix).applyMatrix4(viewProj);
 out.x=(v.x*0.5+0.5)*width;out.y=(1-(v.y*0.5+0.5))*height;
}

function project(mesh:THREE.Mesh,position:THREE.BufferAttribute|THREE.InterleavedBufferAttribute,index:THREE.BufferAttribute|null,i:number,viewProj:THREE.Matrix4,width:number,height:number,out:{x:number;y:number}){
 const vi=index?index.getX(i):i;
 const v=projectScratch.set(position.getX(vi),position.getY(vi),position.getZ(vi)).applyMatrix4(mesh.matrixWorld).applyMatrix4(viewProj);
 out.x=(v.x*0.5+0.5)*width;out.y=(1-(v.y*0.5+0.5))*height;
}

function fillTriangle(pixels:Uint8Array,width:number,height:number,a:{x:number;y:number},b:{x:number;y:number},c:{x:number;y:number},rgb:number[]){
 ...
 const area=(b.x-a.x)*(c.y-a.y)-(c.x-a.x)*(b.y-a.y);if(area===0)return;
 for(let y=minY;y<=maxY;y++)for(let x=minX;x<=maxX;x++){
  const w0=((b.x-x)*(c.y-y)-(c.x-x)*(b.y-y))/area;
  const w1=((c.x-x)*(a.y-y)-(a.x-x)*(c.y-y))/area;
  const w2=1-w0-w1;
  if(w0<0||w1<0||w2<0)continue;
  const o=(y*width+x)*4;pixels[o]=rgb[0];pixels[o+1]=rgb[1];pixels[o+2]=rgb[2];pixels[o+3]=255;
 }
}
