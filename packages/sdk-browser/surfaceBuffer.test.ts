...
test('surface budgets reject invalid dimensions, device limits and excess allocation before creation',()=>{
 ...
 assert.equal(frameTargetBytes(3,3,true),9*48+9*4+(9+4+1)*8,'odd Hi-Z levels must reserve ceil dimensions');
});

test('a partial surface allocation failure destroys all textures already allocated',()=>{
 Object.assign(globalThis,{GPUTextureUsage:{RENDER_ATTACHMENT:16,TEXTURE_BINDING:4,COPY_SRC:1,COPY_DST:2}});
 let created=0,destroyed=0;
 const device={limits:{maxTextureDimension2D:1024},createTexture(){if(++created===3)throw new Error('NO_MEMORY');return {destroy(){destroyed++;}};}} as unknown as GPUDevice;
 assert.throws(()=>createSurfaceBuffer(device,16,16,10000),/NO_MEMORY/);assert.equal(destroyed,2);
});
