 ...
 */
export function createGpuSmallTriangles(device:GPUDevice,width:number,height:number,capacity:number){
 const target=device.createBuffer({label:'WG small triangle target',size:Math.max(8,width*height*8),usage:GPUBufferUsage.STORAGE});
 ...
 return {
  ...
  encode(encoder:GPUCommandEncoder,input:{indices:GPUBuffer;positions:GPUBuffer;pages:GPUBuffer;hizFlags:GPUBuffer;uniform:GPUBuffer;uvs:GPUBuffer;maps:GPUTextureView;sampler:GPUSampler;pageRows:number;maxTriangles:number;idsView:GPUTextureView;depthView:GPUTextureView;hizView?:GPUTextureView;selection?:{maskBuffer:GPUBuffer;maskOffset:number};groups:Array<unknown>;groupKey:number}){
   ...
   const compute=(input.groups[input.groupKey]??=device.createBindGroup({layout:computeLayout,entries:[
    {binding:0,resource:{buffer:input.indices}},{binding:1,resource:{buffer:input.positions}},
    {binding:2,resource:{buffer:input.pages}},{binding:3,resource:{buffer:input.hizFlags}},
    {binding:4,resource:{buffer:input.uniform,offset:0,size:96}},{binding:5,resource:{buffer:input.uvs}},
    {binding:6,resource:input.maps},{binding:7,resource:input.sampler},
   ...
   binning.setPipeline(clear);binning.dispatchWorkgroups(Math.ceil(width*height/64));
  ...
  dispose(){target.destroy();small.destroy();indirect.destroy();},
 };
}
