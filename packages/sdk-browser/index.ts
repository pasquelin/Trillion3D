...
function clusterColor(id: string, saturation = 0.75) {
  return new THREE.Color().setHSL((hashId(id) * 0.61803398875) % 1, saturation, 0.55);
}
function lighting(scene: THREE.Scene, clearColor: number, source: THREE.Object3D) {
...
export const exactPagesBackend: BackendFactory = (context) => {
  ...
  let visible = 0,
    ...
    lodLevel = 0,
    urlStamp = 0,
    displayDetachments = 0,
    diagnostic: DiagnosticMode = 'beauty';
  ...
  const minPoint = new THREE.Vector3(),
    ...
    metricsSeen = new Set<ArrayBufferView>();
  const paint = (
    mesh: THREE.Mesh,
    ...
    mesh.material = material;
    mesh.geometry =
      diagnostic === 'wireframe' ? triangleGeometry(sourceGeometry, salt) : sourceGeometry;
  };
  const paintBlend = () => {
    for (const copy of blendCopies) {
  ...
  return {
    ...
    render(camera) {
      ...
      frame++;
      lastCamera = camera;
      lastPixelError = resolvePixelError(context, camera, motion);
      const selected = selectVisiblePages(
        roots,
        camera,
        {
          pixelError: lastPixelError,
          viewport,
          frame,
          holdResident: true,
          pageBudget: cap,
      ...
      overBudget = selected.shown.length > cap;
    ...
    pendingUrls() {
      ...
      return orderPendingUrls(
        waiting,
        lastCamera,
        pixelScaleOf(lastCamera, viewport, pixelScaleScratch),
        pendingScratch,
      );
    },
    prefetchUrls() {
      ...
      const ring = selectVisiblePages(
        ...
        lastCamera,
        {
          pixelError: lastPixelError > 0 ? lastPixelError * 0.5 : 0.5,
          viewport,
          frame,
          holdResident: false,
        },
    ...
    pageUrls() {
      ...
      urlStamp++;
      batches.markUrls(bootstrap, urlStamp, urlScratch);
      batches.markUrls(shown, urlStamp, urlScratch);
      batches.markUrls(desired, urlStamp, urlScratch);
      return urlScratch;
    },
    ...
    metrics() {
      ...
      return {
        ...
        displayDetachments: displayDetachments + batched.detachments,
        pageRangeWrites: batched.pageRangeWrites,
        subDraws: batched.subDraws,
      };
    },
    dispose() {
      diagnosticMaterials.forEach((m) => m.dispose());
      batches.dispose();
      for (const rec of allPages) {
        if (rec.attached) release(rec);
...
function disposeSource(source: THREE.Object3D) {
...
export async function createExplorer(canvas: HTMLCanvasElement, options: ExplorerOptions) {
  ...
  let sceneLightingSource: THREE.Object3D | undefined,
    source: THREE.Object3D | undefined,
    renderer: THREE.WebGLRenderer | undefined,
    gpuDevice: GPUDevice | undefined,
    measurementTarget: THREE.WebGLRenderTarget | undefined;
  const backends: RenderBackend[] = [];
  const disposeTargets = () => {
    measurementTarget?.dispose();
  ...
  try {
    ...
    source = gltf.scene;
    sceneLightingSource = options.sceneLighting ?? source;
    signal?.throwIfAborted();
    ...
    const baseline = backends.find((b) => b.id === 'three-webgl-reference') ?? backends[0];
    const optimized =
      backends.find((b) => b.id === 'exact-cluster-pages') ??
      backends.find((b) => b.id === 'webgpu-page-raster') ??
      baseline;
    let active = optimized,
      disposed = false,
      diagnostic: DiagnosticMode = 'beauty';
    ...
    let capturingSurface = false,
      measuring = false;
    const capturePool = [new Uint8Array(0), new Uint8Array(0), new Uint8Array(0)];
    let captureSlot = 0;
    const lookAtTarget = new THREE.Vector3().copy(center);
    let hostFrame = 0;
    let comparisonLayout: ComparisonLayout = options.comparisonLayout ?? 'single',
      comparisonPair: [string, string] = options.comparisonPair ?? [
        baseline.id,
        backends.find((b) => b.id === 'exact-cluster-pages')?.id ??
          backends[backends.length - 1].id,
      ],
      wipe = 0.5,
      toggle: 0 | 1 = 0;
    if (directGpu && comparisonLayout !== 'single') throw new Error('SINGLE_BACKEND_COMPARISON');
    let pairTargetA: THREE.WebGLRenderTarget | undefined,
      pairTargetB: THREE.WebGLRenderTarget | undefined;
    const compositor = directGpu ? undefined : createComparisonCompositor(ownedRenderer);
    ...
    const ensureTarget = (current: THREE.WebGLRenderTarget | undefined) =>
      current ?? new THREE.WebGLRenderTarget(canvas.width, canvas.height, targetOptions);
    const check = () => {
      if (disposed) throw new Error('Explorer disposed');
      if (capturingSurface) throw new Error('SURFACE_CAPTURE_BUSY');
      signal?.throwIfAborted();
    };
    ...
    const fillMetrics = (backend: RenderBackend) => {
      ...
      metricsScratch.totalSubmittedTriangles =
        ...
        (backendMetrics.submittedTriangles == null
          ? null
          : backendMetrics.submittedTriangles +
            (backendMetrics.transparentSubmittedTriangles ?? 0));
      ...
      streamingPromise = streamer
        ...
        .catch((error) => {
          ...
          if (streamingError !== detail) {
            ...
            const recovered = active.metrics().coverageReady === true;
            emit(
              recovered
                ? {
                    eventVersion: 1,
            ...
            diagnose(
              ...
              'Échec du chargement des pages ; couverture GPU de secours conservée si disponible',
              {
                ...
                coverageReady: active.metrics().coverageReady ?? null,
                recovered,
                scope,
              },
            );
    ...
    const drawBackend = (backend: RenderBackend, target: THREE.WebGLRenderTarget | null) => {
      ...
      if (missing.length > 0) {
        ...
        if (needFetch.length > 0) {
          ...
          else if (!measuring && streamingPromise) {
            for (const url of needFetch) if (!queuedFetch.includes(url)) queuedFetch.push(url);
        ...
        !streamingPromise &&
        !queuedFetch.length &&
        performance.now() - lastPrefetch > PREFETCH_INTERVAL_MS &&
      ...
      if (backend.overBudget && backend !== baseline) {
        ...
        fallbackReason = 'Visible pages exceed resident budget';
        active = baseline;
    ...
    const render = (pose?: CameraPose): FrameMetrics => {
      ...
      arrivals.drain();
      (active as { cpuStep?: (index: number, ms: number) => void }).cpuStep?.(
        4,
      ...
      try {
        if (comparisonLayout === 'single' || measuring)
          drawBackend(
            active,
            measuring && !directGpu ? (measurementTarget = ensureTarget(measurementTarget)) : null,
          );
        else {
          const left = backends.find((b) => b.id === comparisonPair[0]) ?? active,
            right = backends.find((b) => b.id === comparisonPair[1]) ?? active;
        ...
        if (ownedRenderer?.getContext().isContextLost()) {
          fallbackReason =
            'Context lost: host must retain a stable preview and recreate the renderer';
          emit({
            eventVersion: 1,
            type: 'fatal',
            audience: 'blocking',
        ...
        fallbackReason = `Backend error: ${String(error)}`;
        active = baseline;
        try {
    ...
    const capture = () => {
      ...
      try {
        ...
        if (capturePool[captureSlot].length !== size)
          capturePool[captureSlot] = new Uint8Array(size);
    ...
    const dispose = () => {
      ...
      disposed = true;
      profiler.dispose();
      hostedControls.splice(0).forEach((c) => {
        try {
          c.dispose();
        } catch {
    ...
    const awaitPages = async () => {
      ...
      loaded = streamer.stats().loaded;
      pageBytesRead = streamer.stats().bytesRead;
    };
    ...
    return {
      ...
      replaceGeometryPage(url: string, data: import('./geometryPage.ts').DecodedGeometryPage) {
        check();
      ...
      async renderViews(poses: readonly CameraPose[]) {
        check();
      ...
      resize(width: number, height: number) {
        ...
        } else ownedRenderer.setSize(width, height, false);
        camera.aspect = width / height;
        camera.updateProjectionMatrix();
        viewport[0] = canvas.width;
        viewport[1] = canvas.height;
        measurementTarget?.setSize(width, height);
        pairTargetA?.setSize(width, height);
        pairTargetB?.setSize(width, height);
      },
      select(id: string) {
        check();
        if (
        ...
        const selected = backends.find((b) => b.id === id);
      ...
      setComparison(
        ...
        if (pair) comparisonPair = pair;
        if (nextWipe !== undefined) wipe = nextWipe;
        if (nextToggle !== undefined) toggle = nextToggle;
      },
      get comparison() {
      ...
      pointsOfInterest(): Array<PointOfInterest> {
        ...
        return [{ id: 'home', label: 'Home', pose: home }, ...extras];
      },
      resetHome() {
        check();
        camera.position.copy(center).add(homeOffset);
      ...
      restoreAfterCampaign(id: string, saved: THREE.PerspectiveCamera) {
        ...
        measuring = false;
        ownedRenderer?.setRenderTarget(null);
        active = backends.find((b) => b.id === id)!;
        camera.copy(saved);
        lookAtTarget.copy(center);
      },
      setMeasurementSurface(enabled: boolean) {
        check();
        measuring = enabled;
        if (!enabled) ownedRenderer?.setRenderTarget(null);
      },
      get backend() {
        return active.id;
      },
      ...
      setDiagnostic(mode: DiagnosticMode) {
        ...
        if (
          (mode === 'clusters' ||
            ...
            mode === 'visibility' ||
            mode === 'screen-error') &&
          active.id === 'three-webgl-reference'
        )
