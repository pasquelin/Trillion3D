// Defect 6 (`inverseTranspose3` threshold, gpuDagShader.ts): a 3×3's degeneracy is judged
...
// Real GPU behaviour is proved by `test/browser/inverse-transposee-petite-echelle.browser.mjs`;
// this test replays the same f32 arithmetic so `pnpm test` catches the regression without GPU.
// The f32 model lives in `test/justesse/inverseTransposeF32.mjs`, shared with the lighting
...
// on a real GPU — see `test/justesse/inverse-transposee-petite-echelle.mjs`.
...
test('the shipped shader no longer carries an absolute threshold on the raw determinant', () => {
  ...
  // the CPU (`mathSingular.ts`), rendered as text. A threshold changed on one side only is
  // impossible.
  assert.equal(
...
// The pre-defect-6 form lives against the shipped kernel (`inverseTransposeWgsl.ts`), so
...
// `test/justesse/substitutionAvant.mjs`. The real GPU is measured by
// `test/justesse/inverse-transposee-petite-echelle.mjs`, which separates face culls the
