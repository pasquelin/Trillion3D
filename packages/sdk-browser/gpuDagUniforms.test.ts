...
// (`gpuDagLayout.ts`). The oracle is frozen at four. Each side therefore receives a readback IN
...
// Bit 0 of word 3 changed meaning with the readback cap (`gpuDagLayout.ts`). It used to say "the
...
test('the readback is returned sorted: the costliest request first', () => {
  // Each rank is a request word, page and priority mixed (`gpuDagRequest.ts`). The GPU writes them
