...
export function createGpuPresenter(device:GPUDevice,canvas:HTMLCanvasElement){
 const context=canvas.getContext('webgpu');if(!context)throw new Error('WEBGPU_CANVAS_UNAVAILABLE');
...
export async function readGpuImage(device:GPUDevice,texture:GPUTexture,width:number,height:number,signal?:AbortSignal){
 signal?.throwIfAborted();
 const bytesPerRow=Math.ceil(width*4/256)*256;
 const buffer=device.createBuffer({label:'WG explicit capture',size:bytesPerRow*height,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});
 try{
  const encoder=device.createCommandEncoder();encoder.copyTextureToBuffer({texture},{buffer,bytesPerRow,rowsPerImage:height},{width,height,depthOrArrayLayers:1});device.queue.submit([encoder.finish()]);
  await buffer.mapAsync(GPUMapMode.READ);signal?.throwIfAborted();
  const mapped=new Uint8Array(buffer.getMappedRange()),pixels=new Uint8Array(width*height*4);
  for(let y=0;y<height;y++)pixels.set(mapped.subarray(y*bytesPerRow,y*bytesPerRow+width*4),(height-1-y)*width*4);
...
export function createSynchronousCanvasCapture(){
 ...
 try{
  for(const [type,source] of [[gl.VERTEX_SHADER,'#version 300 es\nvoid main(){gl_Position=vec4(float((gl_VertexID&1)*4-1),float((gl_VertexID>>1)*4-1),0.,1.);}'],[gl.FRAGMENT_SHADER,'#version 300 es\nprecision highp float;uniform sampler2D image;out vec4 color;void main(){color=texelFetch(image,ivec2(gl_FragCoord.xy),0);}']] as const){
   const shader=gl.createShader(type)!;shaders.push(shader);gl.shaderSource(shader,source);gl.compileShader(shader);if(!gl.getShaderParameter(shader,gl.COMPILE_STATUS))throw new Error(gl.getShaderInfoLog(shader)??'CAPTURE_SHADER');gl.attachShader(program,shader);
  }
  gl.linkProgram(program);if(!gl.getProgramParameter(program,gl.LINK_STATUS))throw new Error(gl.getProgramInfoLog(program)??'CAPTURE_PROGRAM');
 }catch(error){shaders.forEach(shader=>gl.deleteShader(shader));gl.deleteProgram(program);gl.deleteTexture(texture);gl.deleteVertexArray(vao);gl.getExtension('WEBGL_lose_context')?.loseContext();throw error;}
 shaders.forEach(shader=>gl.deleteShader(shader));
 return {
  read(source:HTMLCanvasElement){
   if(gl.isContextLost())throw new Error('CAPTURE_CONTEXT_LOST');
   if(canvas.width!==source.width)canvas.width=source.width;if(canvas.height!==source.height)canvas.height=source.height;
   gl.viewport(0,0,canvas.width,canvas.height);gl.disable(gl.DITHER);gl.useProgram(program);gl.bindVertexArray(vao);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,texture);
   ...
   gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA8,gl.RGBA,gl.UNSIGNED_BYTE,source);gl.uniform1i(gl.getUniformLocation(program,'image'),0);gl.drawArrays(gl.TRIANGLES,0,3);
   ...
   const error=gl.getError();if(error!==gl.NO_ERROR)throw new Error(`CAPTURE_WEBGL_ERROR: ${error}`);
   return pixels;
  },
  dispose(){gl.deleteTexture(texture);gl.deleteVertexArray(vao);gl.deleteProgram(program);gl.getExtension('WEBGL_lose_context')?.loseContext();},
 };
}
