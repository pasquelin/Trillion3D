...
import {createTriangleDiagnosticMaterial,triangleGeometry,triangleSalt} from './triangleDiagnostic.ts';
...
test('exact pages wireframe uses non-indexed submitted triangles',()=>{
 ...
 assert.ok(drawn.every(item=>item.material instanceof THREE.MeshLambertMaterial && item.material.vertexColors && !item.material.wireframe));
...
test('reference backend wireframe expands source triangles instead of MeshBasicMaterial.wireframe',()=>{
 ...
 const backend=referenceBackend({source,metadata:{primitives:[],selectedTriangles:2,selectedNodes:[],totalNodes:0,schema:1,status:'ready',key:'t',scope:'full'},indices:new Map(),associations:new Map()});
