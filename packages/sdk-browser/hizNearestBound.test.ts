...
/** Deux `float32` voisins, et un double entre eux dont l'arrondi au plus proche **monte**. */
function voisins() {
  f32[0] = 0.75;
  const low = f32[0];
  bits[0] += 1;
  const high = f32[0];
  return { low, high, between: low + (high - low) * 0.8 };
}

test('la borne envoyée au noyau ne dépasse jamais la profondeur qu elle minore', () => {
  const { low, high, between } = voisins();
  assert.ok(Math.fround(between) > between, 'le cas choisi doit arrondir vers le haut');
  assert.equal(Math.fround(between), high);
  assert.ok(hizNearestBound(between, 0) <= between);
  assert.equal(hizNearestBound(between, 0), low);
  // Sur toute une plage de profondeurs, y compris des valeurs déjà représentables et des valeurs
  // juste au-dessus d'un `float32`, la borne reste sous son entrée.
  for (const base of [1e-7, 0.001, 0.5, low, high, 0.9999999, 1]) {
    for (const value of [base, base * (1 + 1e-9), base * (1 - 1e-9)]) {
      ...
      assert.ok(bound > value * (1 - 1e-6), 'et elle ne descend que d un ulp');
    }
  }
...
test('une couche coplanaire retranche de la borne exactement les unités dont elle avance le cluster', () => {
  const nearest = 0.875;
  ...
  // Une profondeur au ras du plan proche ne peut pas repasser de l'autre côté.
  assert.ok(hizNearestBound(1e-44, 15) >= 0);
});
...
test('la borne corrigée ne peut que faire dessiner davantage, jamais moins', () => {
  const { low, high, between } = voisins();
  // La pyramide voit exactement `low` : la borne arrondie au plus proche rejetterait un cluster qui
  // peint encore son pixel, la borne corrigée ne le rejette pas.
  const pyramid = pyramide(low);
  ...
  assert.equal(hizRejects(pyramid, box(hizNearestBound(high + (high - low) * 64, 0))), true);
