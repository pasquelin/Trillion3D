...
export function createComparisonCompositor(renderer: THREE.WebGLRenderer) {
  ...
  scene.add(mesh);
  const layoutId = (layout: ComparisonLayout) =>
    layout === 'side-by-side'
      ? 1
      : layout === 'wipe'
        ? 2
  ...
  return {
    render(
      ...
      material.uniforms.mapA.value = mapA;
      material.uniforms.mapB.value = mapB;
      material.uniforms.layout.value = layoutId(layout);
      material.uniforms.wipe.value = Math.min(1, Math.max(0, wipe));
      material.uniforms.toggle.value = toggle;
