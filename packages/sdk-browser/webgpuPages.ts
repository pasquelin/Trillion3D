...
import { SHADER, BLEND_SHADER } from './webgpuPagesShaders.ts';
import {
  viewProj,
  remap,
  PAGES_GREEN,
...
export const webgpuPagesBackend: BackendFactory = (context) => {
  ...
  const pageCatalog = [...new Set(allPages.map((page) => page.url))],
    pageCatalogIds = new Map(pageCatalog.map((url, index) => [url, index]));
  ...
  for (let i = 0; i < allPages.length; i++)
    allPages[i].keyIndex = pageCatalogIds.get(allPages[i].url);
  ...
  const bootstrapKeys = new Int32Array(bootstrap.length),
    bootstrapKey = new Uint8Array(keyCount);
  for (let i = 0; i < bootstrap.length; i++) {
    bootstrapKeys[i] = keyOf(bootstrap[i]);
    bootstrapKey[bootstrapKeys[i]] = 1;
  }
  let bootstrapReady = false,
    bootstrapLoading: Promise<void> | undefined,
  ...
  let residencyPending = false,
    residencyRunning = false;
  ...
  let pending: Promise<unknown> = Promise.resolve(),
    drawn: PageRec[] = [],
    targetSize: [number, number] = [viewport?.[0] ?? 1, viewport?.[1] ?? 1];
  const shown: PageRec[] = [],
    desired: PageRec[] = [];
  ...
  for (let i = 0; i < packedPages.length; i++) {
    ...
    if (list) list.push(i);
    else pageIndicesByUrl.set(packedPages[i].url, [i]);
  }
  ...
  let texturePump: Promise<void> | undefined,
    textureColorSize: [number, number] = [1, 1],
  ...
  const pumpTextures = () => {
    ...
    const run = async () => {
      ...
      while (textureJobs.length && admitted + textureJobs[0].bytes <= textureBudget) {
        ...
        try {
          ...
          textureSkipped++;
          diagnosticFailure('progressive-texture-upload-failed', error);
        }
      }
      if (textureJobs.length && textureJobs[0].bytes > textureBudget) {
        textureJobs.shift();
        textureSkipped++;
      }
      if (colorLayers.length && mapsTexture)
      ...
      if (dataLayers.length && dataMapsTexture)
        await generateMaterialMips(
          gpuDevice!,
          dataMapsTexture,
          'rgba8unorm',
    ...
    texturePump = run().finally(() => {
      texturePump = undefined;
    });
    return texturePump;
  };
  ...
  const blendCuts = new Map<(typeof blendGpu)[number], PageRec[]>(),
    blendDrawnPages: PageRec[] = [];
  const visibleBlend: typeof blendGpu = [],
    blendFrustum = new THREE.Frustum();
  const selectBlend = (device: GPUDevice) => {
    if (pagedBlendGpu.size) {
      const previousLength = blendDrawnPages.length;
      let count = 0,
        changed = false;
      ...
      if (changed || count !== previousLength) {
        ...
        for (const rec of blendDrawnPages) {
          const item = rec.sourceMesh && pagedBlendGpu.get(rec.sourceMesh);
          if (!item) continue;
          let cut = blendCuts.get(item);
          if (!cut) blendCuts.set(item, (cut = []));
          cut.push(rec);
        }
      }
    }
    for (const item of blendGpu) {
      if (!item.paged) {
        if (item.bounds && !blendFrustum.intersectsBox(item.bounds)) blendFrustumRejected++;
        else visibleBlend.push(item);
        continue;
      }
      const cut = blendCuts.get(item);
      if (!cut?.length) {
        blendFrustumRejected++;
        continue;
      }
      if (cut !== item.cut) {
        ...
        if (
          item.cut &&
          cut.length === item.cut.length &&
          cut.every((rec, i) => rec === item.cut![i])
        )
          blendCuts.set(item, item.cut);
        else {
          let count = 0;
  ...
  const encodeBlend = (device: GPUDevice, encoder: GPUCommandEncoder, uniformBase: number) => {
    if (!pipelineBlend || !visibleBlend.length || !colorView || !depthView || !uniformBuffer)
      return;
    const textured = !!(
    ...
    device.queue.writeBuffer(
      ...
      uniformPacked.subarray(
        ...
        (uniformBase + visibleBlend.length) * (UNIFORM_STRIDE / 4),
      ),
    );
    ...
    for (let i = 0; i < visibleBlend.length; i++) {
      ...
      if (
        ...
        gpuDrawCalls += 2;
        blendDrawCalls += 2;
        blendSubmittedTriangles += (2 * item.count) / 3;
      } else {
        pass.setPipeline(
          textured
        ...
        gpuDrawCalls++;
        blendDrawCalls++;
        blendSubmittedTriangles += item.count / 3;
      }
    }
    pass.end();
    ...
    if (traceEnabled)
      traceDiagnostic('transparent-encoding', 'Transparents sélectionnés et encodés', () => ({
        ...
        candidates: blendGpu.length,
        visibleMeshes: visibleBlend.length,
        ...
        passes: visibleBlend.length ? 2 : 0,
      }));
  };
  ...
  const ensurePageTable = (device: GPUDevice) => {
    ...
    pageTableFloats = new Float32Array(bytes / 4);
    pageTableInts = new Uint32Array(pageTableFloats.buffer);
    pageTable?.destroy();
    shadeBindGroup = undefined;
    visBindGroup = undefined;
    visHizBindGroup = undefined;
  ...
  const syncResidencyMirror = () => {
    ...
    engineDiagnostic(
      ...
      { frame, journal: journalResident, resident },
    );
    residencyDirty = true;
    journalResident = 0;
    residentOutsideTable.clear();
    for (let c = 0; c < pageCatalog.length; c++) {
      const url = pageCatalog[c],
        page = cache.get(url),
  ...
  const syncRows = () => {
    ...
    let count = 0,
      ...
      monotone = true;
    for (let i = 0; i < packedPages.length; i++) {
      const offsetWords = residentOffsetWords[i],
        rec = packedPages[i],
        index = rec.array;
      ...
      newRowPage[row] = i;
      newRowSource[row] = source;
      packedRecs[row] = rec;
      packedPositions[row] = position;
      packedPageIndex[row] = i;
    }
    commitRows(count, monotone);
    candidateCount = candidates;
    candidateOverflow = overflow + Math.max(0, candidates - drawSlots);
  };
  ...
  const syncRowsFromCut = () => {
    ...
    for (let i = 0; i < drawn.length && count < drawSlots; i++) {
      const rec = drawn[i];
      if (rec.transparent) continue;
      const pageIndex = pageIndexByRec.get(rec);
      if (pageIndex === undefined) continue;
      const offsetWords = residentOffsetWords[pageIndex],
        index = rec.array,
        position = pagePositions[pageIndex];
      ...
      newRowPage[row] = pageIndex;
      newRowSource[row] = source;
      packedRecs[row] = rec;
      packedPositions[row] = position;
      packedPageIndex[row] = pageIndex;
    }
    commitRows(count, monotone);
  };
  ...
  const uploadDirtyRows = (device: GPUDevice) => {
    ...
    device.queue.writeBuffer(
      pageTable,
      dirtyFrom * PAGE_INFO_STRIDE,
      pageTableFloats.buffer as ArrayBuffer,
      dirtyFrom * PAGE_INFO_STRIDE,
      (dirtyTo - dirtyFrom + 1) * PAGE_INFO_STRIDE,
    );
    dirtyFrom = drawSlots;
    dirtyTo = -1;
  };
  ...
  const encodeVis = (device: GPUDevice, camera: THREE.PerspectiveCamera, itemsDirty: boolean) => {
    ...
    if (gpuHiz && packedCount >= 2) {
      if (!noOccluderHistory) {
        ...
        if (!occluders || occluders === packedCount) {
          ...
          occluders = splitOccludersFlat(packedCount, hizBounds, hizRest);
          boundsForAll = true;
        }
      } else {
        projectBoxesFlat(
    ...
    for (let i = 0; i < packedCount; i++) {
      ...
      if (itemsDirty) {
        ...
        drawItemWords[word + 1] = visBin(packedRecs[i]!);
    ...
    for (let slot = 0; slot < 7; slot++) {
      ...
      visUniPacked[base + 18] = gpuSmall ? 7 : 0;
      ...
      visInts[base + 22] = gpuFrameActive ? (gpuSelection?.maskOffset ?? 0) : 0;
    ...
    shadeInts[21] =
      diagnostic === 'wireframe' ? 1 : diagnostic === 'pages' ? 3 : diagnostic === 'beauty' ? 0 : 2;
    ...
    if (
      ...
      shadeBindGroup = device.createBindGroup({
        ...
        entries: [
          ...
          {
            binding: 6,
            resource: (mapsArrayView ??= mapsTexture.createView({ dimension: '2d-array' })),
          },
          { binding: 7, resource: mapsSampler },
          { binding: 8, resource: { buffer: shadeUniform } },
          {
            binding: 9,
            resource: (dataMapsArrayView ??= dataMapsTexture.createView({ dimension: '2d-array' })),
          },
        ],
      });
    }
    if (
      ...
      if (!visHizBindGroup && gpuHiz)
        visHizBindGroup = device.createBindGroup({
          layout: visBindGroupLayout,
          entries: [
            { binding: 0, resource: { buffer: cache.buffer } },
            { binding: 1, resource: { buffer: concatPos } },
            { binding: 2, resource: { buffer: pageTable } },
            { binding: 3, resource: { buffer: gpuHiz.flags } },
            { binding: 4, resource: { buffer: visUniform, offset: 0, size: 96 } },
            { binding: 5, resource: { buffer: concatUv } },
            { binding: 6, resource: visMaps },
            { binding: 7, resource: mapsSampler },
    ...
    const visGroupFor = (slot: number, rest: boolean) => {
      if (
        ...
        return;
      const flags = rest ? gpuHiz?.flags : zeroFlags;
      if (!flags) return;
    ...
    const drawVis = (pass: GPURenderPassEncoder, rest: boolean) => {
      ...
      for (let i = 0; i < packedCount; i++) {
        ...
        pass.setPipeline(pipeline);
        pass.setBindGroup(0, group);
        pass.draw(packedRecs[i]!.array!.length, 1, 0, i);
        gpuDrawCalls++;
      }
    };
    ...
    if (gpuHiz) {
      ...
      for (let i = 0; i < packedCount; i++)
        if (!hizRest[i]) drawnOccluderUrls[urlIndexOfPage[packedPageIndex[i]]] = 1;
  ...
  const encodeDraws = (device: GPUDevice, camera: THREE.PerspectiveCamera) => {
    ...
    for (let i = 0; i < packedCount; i++) {
      ...
      packedInts[base + 36] = pageTableInts![row * fallbackWords + 24];
      packedInts[base + 37] = pageTableInts![row * fallbackWords + 25];
      packedInts[base + 38] = diagnostic === 'wireframe' ? 1 : 0;
    }
    if (packedCount && uniformBuffer)
      device.queue.writeBuffer(
        uniformBuffer,
        0,
        uniformPacked.subarray(0, packedCount * (UNIFORM_STRIDE / 4)),
      );
    const encoder = createRenderEncoder(device);
    const pass = encoder.beginRenderPass({
    ...
    for (let i = 0; i < packedCount; i++) {
      ...
      const group = bindGroupFor(device, position),
        pipeline = pipelineFor(packedRecs[i]!);
  ...
  const budgetedResidency = (pages: readonly PageRec[]) => {
    ...
    for (let i = 0; i < pages.length; i++)
      if (!bootstrapKey[keyOf(pages[i])]) residencyScratch.push(pages[i]);
  ...
  const updatePins = () => {
    ...
    if (traceEnabled) {
      pinsBefore.clear();
      for (const url of pinnedUrls()) pinsBefore.add(url);
    }
    keepEpoch++;
    keepCount = 0;
    const keep = (key: number) => {
      if (keepStamp[key] !== keepEpoch) {
        keepStamp[key] = keepEpoch;
        keepList[keepCount++] = key;
      }
    };
    for (let i = 0; i < bootstrapKeys.length; i++) keep(bootstrapKeys[i]);
    for (let i = 0; i < shown.length; i++)
      if (!gpuFrameActive || shown[i].transparent) keep(keyOf(shown[i]));
    ...
    if (deferredDrops.size) {
      ...
      for (let i = 0; i < keepCount; i++) keepUrls.add(pageCatalog[keepList[i]]);
    ...
    if (added.length || removed.length)
      traceDiagnostic('residency-pins', 'Pins GPU mis à jour', () => ({
        frame,
        added: traceSet('pins.added', added),
        removed: traceSet('pins.removed', removed),
        pinned: traceSet('pins', pinned),
        bootstrap: traceSet('pins.bootstrap', [...bootstrapUrls]),
        wanted: traceSet('pins.wanted', wantedUrls()),
        shown: traceSet(
          'pins.shown',
  ...
  const ensureBootstrap = async () => {
    ...
    bootstrapLoading = (async () => {
      ...
      engineDiagnostic(
        ...
        { version: 1, pages: bootstrap.length, slots },
      );
      traceDiagnostic(
        'coverage-bootstrap-start',
        'Chargement de la couverture complète de secours',
        () => ({
          frame,
          pages: bootstrap.length,
          pageIds: pageRefs(bootstrap.map((page) => page.url)),
          slots,
          queueWaitMs: 0,
        }),
      );
      let next = 0;
      const workers = Array.from({ length: Math.min(8, bootstrap.length) }, async () => {
        while (next < bootstrap.length) {
          const page = bootstrap[next++];
          context.signal?.throwIfAborted();
          if (lost) throw new Error('WEBGPU_LOST');
          if (!hasBytes(page)) {
            const key = pageRequestUrl(page);
            const array = await context.readPage!(key);
            context.signal?.throwIfAborted();
            if (lost) throw new Error('WEBGPU_LOST');
            backend.acceptPage!(key, array);
          }
        }
      });
      ...
      for (const page of bootstrap) {
        context.signal?.throwIfAborted();
        if (lost) throw new Error('WEBGPU_LOST');
        await cache!.load(page.url, context.signal);
        cache!.pin(page.url);
        markPinned(keyOf(page));
      }
      bootstrapReady = true;
      ...
      traceDiagnostic(
        ...
        () => ({
          frame,
          pages: bootstrap.length,
          bootstrap: traceSet('bootstrap', [...bootstrapUrls]),
          slots,
  ...
  const ensureResident = async (wanted: readonly PageRec[], jobFrame: number, jobId: number) => {
    ...
    traceDiagnostic('residency-ensure-start', 'Vérification de la résidence GPU demandée', () => ({
      ...
      pages: traceSet('ensure', urls),
      wanted: traceSet('ensure.wanted', urls),
      loaded: traceSet(
        'ensure.loaded',
    ...
    for (let i = 0; i < wanted.length; i++) {
      ...
      try {
        await cache.load(rec.url, context.signal);
      } catch (error) {
        if (wantedStamp[key] !== wantedEpoch && String(error).includes('ALL_PAGES_PINNED'))
          continue;
        throw error;
      }
      if (lost || !cache) throw new Error('WEBGPU_LOST');
      if (wantedStamp[key] === wantedEpoch || bootstrapKey[key]) {
        cache.pin(rec.url);
        markPinned(key);
      }
    }
    traceDiagnostic('residency-ensure-end', 'Résidence GPU vérifiée', () => ({
      ...
      cpuWorkIncluded: true,
      gpuQueueWaitIncluded: false,
      pinned: traceSet('pins', pinnedUrls()),
    }));
  };
  const queueResident = (wanted: PageRec[]) => {
    ...
    wantedCount = 0;
    for (let i = 0; i < wanted.length; i++) {
      const key = keyOf(wanted[i]);
      if (wantedStamp[key] !== wantedEpoch) {
        wantedStamp[key] = wantedEpoch;
        wantedList[wantedCount++] = key;
      }
    }
    updatePins();
    residencyQueue.length = 0;
    queuedEpoch++;
    for (let i = 0; i < wanted.length; i++) {
      const page = wanted[i],
        key = keyOf(page);
      if (!hasBytes(page) || queuedStamp[key] === queuedEpoch) continue;
      queuedStamp[key] = queuedEpoch;
      residencyQueue.push(page);
    }
    residencyPending = true;
    const queued = residencyQueue;
    if (traceEnabled)
      traceDiagnostic('residency-queue', 'Résidence GPU mise en file', () => ({
        ...
        pages: traceSet(
          'queue',
          queued.map((page) => page.url),
        ),
        wanted: traceSet('wanted', wantedUrls()),
        loaded: traceSet(
          'queue.loaded',
          queued.filter((page) => !!cache?.get(page.url)).map((page) => page.url),
        ),
        queueDepth: queued.length,
        residentPages: cache?.stats().residentPages ?? null,
      }));
    ...
    pending = Promise.resolve().then(async () => {
      ...
      traceDiagnostic('residency-job-start', 'Job de résidence GPU démarré', () => ({
        ...
        pages: traceSet(
          'job',
          residencyQueue.map((page) => page.url),
        ),
        elapsedMs: null,
        cpuWorkIncluded: true,
        gpuQueueWaitIncluded: false,
      ...
      try {
        while (residencyPending) {
          residencyPending = false;
          await ensureResident(residencyQueue, jobFrame, jobId);
        }
      } catch (error) {
        diagnosticFailure('coverage-upload-failed', error);
        if (/LOST|DISPOSED/i.test(String(error))) lost = true;
        throw error;
      } finally {
        residencyRunning = false;
        traceDiagnostic('residency-job-end', 'Job de résidence GPU terminé', () => ({
          ...
          pages: traceSet('job', wantedUrls()),
          loaded: traceSet(
            'job.loaded',
            wantedUrls().filter((url) => !!cache?.get(url)),
          ),
          residentPages: cache?.stats().residentPages ?? null,
          queueWaitMs: started - queuedAt,
          cpuWorkIncluded: true,
  ...
  const adoptGpuCut = () => {
    ...
    for (const id of cut.result.pageIds) {
      ...
      if (rec) gpuWanted.push(rec);
    }
    partitionByPass(desired, true, transparentScratch);
    desired.length = 0;
    appendAll(desired, gpuWanted, transparentScratch);
    ...
    shown.length = 0;
    appendAll(shown, drawableScratch, transparentScratch);
    drawn.length = 0;
    ...
    visible = desired.length;
    selectedTriangles = triangleSum(shown);
    uncoveredTriangles = counts.uncoveredTriangles;
    submittedTriangles = counts.drawnTriangles + blendSubmittedTriangles;
    frustumRejected = cut.result.frustumRejected;
    lodLevel = cut.result.lodLevel;
  ...
  const renderGpuCut = (
    ...
    const request = (key: number) => {
      if (requestedStamp[key] !== requestedEpoch) {
        requestedStamp[key] = requestedEpoch;
        requestedList[requestedCount++] = key;
      }
    };
    ...
    coverageBudgetLimited = requestedCount > slots;
    ...
    else if (budgetPixelError > 0 && requestedCount < slots * 0.7)
    ...
    if (wasLimited !== coverageBudgetLimited)
      coverageBudgetEvent = {
        ...
        requiredSlots: requestedCount,
        slots,
        fallbackRetained: bootstrapReady,
        pixelError: budgeted,
      };
    if (!bootstrapReady) {
      gpuMetricsReady = false;
    ...
    if (transparent && !coverageBudgetLimited) {
      const transition = (key: number) => {
        if (transitionStamp[key] !== transitionEpoch) {
          transitionStamp[key] = transitionEpoch;
          transitionCount++;
        }
      };
    ...
    if (candidateOverflow) {
      ...
      engineDiagnostic(
        ...
        { residentCandidates: candidateCount + candidateOverflow, maxCandidates: drawSlots },
      );
      dropGpuSelection();
    ...
    if (!renderPathLogged) {
      ...
      engineDiagnostic('first-render-path', 'Configuration du premier rendu WebGPU', {
        ...
        residentCandidates: candidateCount,
      });
    }
    ...
    if (traceEnabled)
      traceDiagnostic('frame', 'Snapshot complet de la frame WebGPU', () => ({
        ...
        coverage: {
          loaded: traceSet(
            'frame.loaded',
            packedRecs.slice(0, packedCount).map((page) => page!.url),
          ),
          wanted: traceSet(
            'frame.wanted',
  ...
  const backend: WebgpuPagesBackend = {
    ...
    async prepare() {
      ...
      try {
        ...
        pipelineBack = gpuDevice.createRenderPipeline({
          layout,
        ...
        pipelineBackCw = gpuDevice.createRenderPipeline({
          layout,
          vertex: { module, entryPoint: 'vs' },
          fragment,
        ...
        pipelineNone = gpuDevice.createRenderPipeline({
          layout,
          vertex: { module, entryPoint: 'vs' },
          fragment,
        ...
        pipelineBlend = gpuDevice.createRenderPipeline({
          layout,
          vertex: { module, entryPoint: 'vs' },
          fragment: {
            module,
        ...
        for (const rec of allPages) {
          ...
          const attr = rec.attributes.position;
          if (!attr) continue;
          ...
          gpuDevice.queue.writeBuffer(buffer, 0, xyz.buffer);
          positionBuffers.set(rec.attributes, buffer);
        }
        for (let i = 0; i < packedPages.length; i++)
          pagePositions[i] = positionBuffers.get(packedPages[i].attributes);
        ...
        for (const copy of blendCopies) {
          if (isTransmissive(copy.material)) {
            if (context.gpuCanvas) throw new Error('UNSUPPORTED_TRANSMISSION');
            continue;
          }
          ...
          if (!position) {
            ...
            position = gpuDevice.createBuffer({
              size: xyz.byteLength,
              usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
            });
            gpuDevice.queue.writeBuffer(position, 0, xyz.buffer);
            positionBuffers.set(copy.geometry.attributes, position);
          }
          ...
          const index = gpuDevice.createBuffer({
          ...
          if (normalAttr) {
            ...
            normal = gpuDevice.createBuffer({
              size: Math.max(12, data.byteLength),
              usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
            });
            gpuDevice.queue.writeBuffer(normal, 0, data);
          }
          ...
          blendGpu.push(item);
          if (paged && item.sourceMesh) pagedBlendGpu.set(item.sourceMesh, item);
          scene.remove(copy);
        }
        ...
        try {
          ...
          concatPos = upload(pos);
          concatUv = upload(uv);
          concatNrm = upload(nrm);
          ...
          for (const rec of allPages) {
            const mat = visMaterial(rec.material);
            addColor(mat.map);
          ...
          if (layers > gpuDevice.limits.maxTextureArrayLayers)
            throw new Error(
              `TEXTURE_ATLAS_LAYERS: ${layers} texture layers exceed the device limit ${gpuDevice.limits.maxTextureArrayLayers}`,
            );
          mapsTexture = gpuDevice.createTexture({
          ...
          const clearLayer = (
            ...
            const pass = fallbackEncoder.beginRenderPass({
              colorAttachments: [
                {
          ...
          for (let layer = 0; layer < layers; layer++)
            clearLayer(mapsTexture, layer, { r: 1, g: 1, b: 1, a: 1 });
          ...
          if (dataLayers > gpuDevice.limits.maxTextureArrayLayers)
            throw new Error(
              `TEXTURE_ATLAS_LAYERS: ${dataLayers} texture layers exceed the device limit ${gpuDevice.limits.maxTextureArrayLayers}`,
            );
          dataMapsTexture = gpuDevice.createTexture({
          ...
          try {
            blendBindGroupLayout = gpuDevice.createBindGroupLayout({
              entries: [
                {
                  binding: 0,
                  visibility: GPUShaderStage.VERTEX,
            ...
            const makeBlend = (cullMode: GPUCullMode) => {
              const descriptor: GPURenderPipelineDescriptor = {
                layout: gpuDevice.createPipelineLayout({
                  bindGroupLayouts: [blendBindGroupLayout],
                }),
              ...
              return gpuDevice.createRenderPipelineAsync
                ? gpuDevice.createRenderPipelineAsync(descriptor)
                : Promise.resolve(gpuDevice.createRenderPipeline(descriptor));
            };
            for (const item of blendGpu) item.group = undefined;
            pipelineBlendTextured = await makeBlend('none');
          ...
          shadeUniform = gpuDevice.createBuffer({
            size: 256,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
          });
          visBindGroupLayout = gpuDevice.createBindGroupLayout({
            entries: [
              {
                binding: 0,
                visibility: GPUShaderStage.VERTEX,
          ...
          zeroFlags = gpuDevice.createBuffer({
            size: Math.max(4, drawSlots * 4),
            usage: GPUBufferUsage.STORAGE,
          });
          visUniform = gpuDevice.createBuffer({
            size: 7 * 256,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
          });
          ...
          if (typeof visModule.getCompilationInfo === 'function') {
            ...
            if (visInfo.messages.some((message) => message.type === 'error'))
              throw new Error('VIS_SHADER');
          }
          ...
          const visLayout = gpuDevice.createPipelineLayout({
          ...
          const makeVis = (
            ...
            gpuDevice.createRenderPipeline({
              ...
              primitive: { topology: 'triangle-list', cullMode: cull, frontFace },
          ...
          const scoped = async <T>(run: () => T): Promise<T> => {
            if (typeof gpuDevice.pushErrorScope === 'function')
              gpuDevice.pushErrorScope('validation');
            const value = run();
            const error =
              typeof gpuDevice.popErrorScope === 'function'
                ? await gpuDevice.popErrorScope()
                : null;
          ...
          shadeBindGroupLayout = gpuDevice.createBindGroupLayout({
            entries: [
        ...
        if (context.gpuCanvas && blendGpu.length && !pipelineBlendTextured)
    ...
    render(camera) {
      ...
      traceDiagnostic('residency-admission', 'Admission des ensembles demandés', () => ({
        ...
        requested: traceSet('admission.requested', [...requested]),
        wanted: traceSet(
          'admission.wanted',
          desired.map((page) => page.url),
        ),
      ...
      if (!bootstrapReady) {
        ...
        traceDiagnostic('frame', 'Snapshot de frame en attente de couverture GPU', () => ({
          ...
          cpu: sample,
          coverage: {
            loaded: traceSet('frame.loaded', []),
            wanted: traceSet(
              'frame.wanted',
              desired.map((page) => page.url),
            ),
            shown: traceSet('frame.shown', []),
            bootstrap: traceSet(
              'frame.bootstrap',
              bootstrap.map((page) => page.url),
            ),
          ...
          budget: {
            slots,
            requested: traceSet('frame.requested', [
              ...bootstrapUrls,
              ...desired.map((page) => page.url),
            ]),
            limited: coverageBudgetLimited,
      ...
      traceDiagnostic('residency-transition', 'Transition de couverture calculée', () => ({
        ...
        requested: traceSet('transition.requested', [...requested]),
        transition: traceSet('transition.all', [...transition]),
        slots,
      }));
      ...
      try {
        ...
        try {
          if (!lost && !context.signal?.aborted) {
            ...
            await ensureResident(shown, frame, ++residencyJob);
            if (shown.some((page) => !cache?.get(page.url)))
              // eslint-disable-next-line no-unsafe-finally -- A failed restore invalidates the main view.
              throw new Error('SURFACE_GPU_COVERAGE_INCOMPLETE');
            drawn = shown.slice();
          ...
          // eslint-disable-next-line no-unsafe-finally -- Preserve the restore failure for the caller.
          throw error;
        } finally {
    ...
    pendingUrls() {
      return collectPendingUrls(
        !bootstrapReady ? bootstrap : coverageBudgetLimited ? [] : desired,
        pendingScratch,
      );
    },
    pageUrls() {
    ...
    acceptPage(url, array) {
      ...
      traceDiagnostic('page-accepted', 'Page CPU acceptée pour résidence GPU', {
        ...
        wanted: recs.some((rec) => wantedStamp[keyOf(rec)] === wantedEpoch),
        pinned: recs.some((rec) => pinnedAt[keyOf(rec)] >= 0),
      });
    },
    dropPage(url) {
      const recs = byUrl.get(url);
      ...
      const pinned = recs.some((rec) => pinnedAt[keyOf(rec)] >= 0),
        wanted = recs.some((rec) => wantedStamp[keyOf(rec)] === wantedEpoch);
      if (pinned || wanted) {
    ...
    metrics() {
      ...
      for (const item of blendGpu)
        vertexBytes += item.index.size + (item.uv?.size ?? 0) + (item.normal?.size ?? 0);

      return {
        coverageReady: bootstrapReady,
        ...
        transparentMeshes: visibleBlend.length,
