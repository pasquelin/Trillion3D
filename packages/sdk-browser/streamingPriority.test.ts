...
function record(url:string,centre:[number,number,number],radius:number,parentError:number,streamUrl?:string):PriorityRecord{
 return {url,streamUrl,min:[centre[0]-radius,centre[1]-radius,centre[2]-radius],max:[centre[0]+radius,centre[1]+radius,centre[2]+radius],
  matrix:identity,lodError:0,sphere:[...centre,radius],parentError,parentSphere:[...centre,radius]};
}
...
test('at equal error the nearer bundle is asked for first',()=>{
 const near=record('near',[0,0,0],1,.5),far=record('far',[0,0,-60],1,.5);
...
test('at equal distance the bundle whose absence shows the most pixels is asked for first',()=>{
 const loud=record('loud',[0,0,0],1,4),quiet=record('quiet',[0,0,0],1,.01);
...
test('a bundle inherits the worst error of the clusters it carries, since one request serves them all',()=>{
 ...
 const records=[
  record('a0',[0,0,0],1,1,'bundle-a'),record('a1',[0,0,0],1,1,'bundle-a'),
  record('b0',[0,0,0],1,.001,'bundle-b'),record('b1',[0,0,0],1,8,'bundle-b'),
 ];
 assert.deepEqual(order(records),['bundle-b','bundle-a']);
});

test('a cluster already resident is not asked for again',()=>{
 const held=record('held',[0,0,0],1,4);held.array=new Uint32Array(3);
 assert.deepEqual(order([held,record('missing',[0,0,-60],1,.5)]),['missing']);
...
test('a cache with no cluster error falls back on the screen footprint of the bounds',()=>{
 ...
 const small:PriorityRecord={url:'small',min:[-.1,-.1,-1],max:[.1,.1,1],matrix:identity};
