...
test('Importer preserves full triangle coverage, limits the slice, repairs corrupt cached pages and rejects changed sources',async()=>{
 const root=await mkdtemp(join(tmpdir(),'gltf-pages-'));try{
  ...
  const again=await prepare(input,output,'slice',1,options);assert.equal(again.key,slice.key);assert.equal(again.reusedPages,2);
  const page=join(output,'reference','slice',slice.key,slice.primitives[0].pages[0].url);await writeFile(page,Buffer.alloc(12));const repaired=await prepare(input,output,'slice',1,options);assert.equal(repaired.reusedPages,1);
