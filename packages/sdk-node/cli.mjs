...
const result=await prepare(input,output,scope,triangleBudget,{executable:process.env.WEB_GEOMETRY_COMPILER_BIN??process.env.RTL_ASSET_COMPILER_BIN,resourceBaseUrl,signal:controller.signal,onProgress:event=>process.stderr.write(`${JSON.stringify(event)}\n`)});
