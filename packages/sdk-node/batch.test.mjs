 ...
 */
async function compilerPrinting(root, summary, code) {
...
const job = (id) => ({ id, source: `${id}.obj`, cache: `cache/${id}`, resourceBaseUrl: `/${id}/` });
...
test('A16 prepareMany returns the summary of a partially successful batch', async () => {
  ...
  try {
    const summary = summaryOf('partial', [
      { job: 'a', status: 'ready', pointer: { status: 'ready', key: 'abc', scope: 'full' } },
