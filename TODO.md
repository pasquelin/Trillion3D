...
2. **Fait, scène simple face** : les transparents paginés d'un même pipeline tiennent en une tranche et quelques ordres (4 288 appels → 9 au banc `transparents-ordres`), le plan trié est lu par la carte seule, et l'argument indirect par item a disparu. **Rien n'est gagné sur une scène double face** : le dos et la face posent deux pipelines à chaque item, aucune tranche ne fusionne, 6 016 appels avant comme après. Le seul remède est de porter le côté rasterisé par instance sous un pipeline sans élimination matérielle, et il coûte +42 % de passe transparente mesurés sur apple metal-3 (11,0 → 15,4 ms, dont +24 % de rasterisation seule) : refusé tant qu'une autre forme n'est pas trouvée. **Reste aussi** : la transmission garde une tranche par entrée tant que le volume du matériau est un uniforme à décalage dynamique — le rapatrier dans la fiche d'item porte `VSOut` à seize variables inter-étages, le maximum que la norme garantit ; et une primitive non paginée garde son appel tant que la géométrie transparente non paginée n'est pas concaténée comme l'est la paginée. Fichiers `webgpuBlend*`.
3. **Fait, moitié lancements** : la descente entière tient dans UNE passe, chaque niveau lancé à plat sur le nombre de nœuds de son étage — majorant que le rangement compte une fois pour toutes —, et une image n'ouvre plus que six commandes, au lieu de 3·profondeur+3, soit 42 à profondeur treize. Ce qui coûtait n'était ni la passe ni le nombre de fils mais l'armement de l'argument indirect, 17 à 19 µs la copie contre 1 µs la passe ; il n'en reste que trois, pour les trois listes dont le rangement ne connaît aucun majorant. Par image sur apple metal-3, mêmes pages voulues et dessinées au bit près : 0,4665 → 0,1605 ms à profondeur treize, 0,2595 → 0,1495 à cinq (banc `coupe-lancements-gpu`). **La sélection persistante est refusée, mesures à l'appui** (`gpuDagCutFrontier.test.ts`) : la frontière de la descente doit être réexaminée à chaque image, l'erreur écran de chaque nœud changeant dès que la caméra bouge, et les seuls nœuds qu'une coupe gardée économiserait plafonnent à 0,47 % de ce que l'image relit — 0,50 % au saut de caméra, 0,47 % sur scène animée. **Les fils persistants sont refusés** : WGSL ne garantit aucun progrès entre groupes de travail, une attente active d'un groupe sur un autre peut bloquer sans recours. **Reste, et c'est le vrai chiffre de cette ligne** : 378 479 grappes vivantes par image pour 21 955 utiles (`gpuDagLiveWgsl.ts`), parce que rien n'élague par le HAUT — un nœud ne porte que le plafond d'erreur de son remplaçant, jamais le plancher d'erreur de son sous-arbre, si bien qu'aucun étage trop grossier ne peut tomber d'un coup. Nouveau champ de nœud, donc compilateur et version de format. Fichiers `gpuDag*`.
