...
3. **Fait, moitié lancements** : la descente entière tient dans une passe, chaque niveau lancé à plat sur la taille de son étage, six commandes par image au lieu de 3·profondeur+3 — 0,26 → 0,15 ms à profondeur 5, un niveau valant ~26 µs ouvert derrière ses copies contre ~1,5 µs à plat (banc `coupe-lancements-gpu`, mêmes pages au bit près). **Persistance refusée** : la frontière doit être réexaminée à chaque image, ce qu'une coupe gardée économiserait plafonne à 0,47 % (`gpuDagCutFrontier.test.ts`). **Fils persistants refusés** : WGSL ne garantit aucun progrès entre groupes de travail. **Reste le vrai chiffre de la ligne** — 378 479 grappes vivantes pour 21 955 utiles : la carte n'élague pas par le haut, faute du plancher d'erreur du sous-arbre. Ce n'est pas un travail de compilateur ni de format — `cullingBounds` le dérive déjà côté navigateur et la coupe processeur s'en sert (`pageSelectionCutNode.ts:45`) ; `DagRoot.culling` le jette avant le rangement. Mesuré : il retire 86 144 candidates sur 258 688, soit 69,9 % des trop grossières. Coût : nœud de 16 à 24 flottants, et `errorFloorAt` en WGSL — un plancher surestimé retire de la géométrie, donc preuve processeur contre carte obligatoire, comme `erreur-ecran-cpu-gpu.mjs` pour la projection. Fichiers `gpuDag*`.
