...
//
// The bench and the proofs run on models anybody can fetch (`scripts/mesure/assets.ts`), so the
...
//
// It reads the caches `node scripts/mesure/assets.ts` writes and never compiles anything itself:
// a missing cache fails here by name, with the command that produces it.
//
// node --experimental-strip-types --test test/justesse/scenes-publiques.ts
import test from 'node:test';
import assert from 'node:assert/strict';
import { existsSync, readdirSync } from 'node:fs';
import { join } from 'node:path';
import { readCacheManifest } from '../../scripts/mesure/cacheManifest.ts';
import { ASSETS, sceneDerived } from '../../scripts/mesure/scene.ts';
...
function openScene(scene: string) {
  ...
  assert.ok(
    ...
    `no cache for ${scene}: run \`node scripts/mesure/assets.ts --only ${scene}\`` +
      ' (a facade is written first by `node scripts/mesure/scenes/facade.ts --seed <n>`)',
  );
...
function provenScenes() {
  ...
  assert.ok(
    facades.length > 0,
    'no facade scene on disk: run `node scripts/mesure/scenes/facade.ts --seed 7`' +
      ' then `node scripts/mesure/assets.ts --only facade-7`',
  );
