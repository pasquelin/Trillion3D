        ...
        try {
          for (const fixture of fixtures) {
            ...
            try {
              ...
              const outputCanvas =
                canvas ??
                backend.scene.children.find((object) => object.userData.blit)?.material.uniforms
                  .image.value.image;
              ...
              try {
                ...
                check(
                  surface.cameraWorld[0] === 0.8,
                  fixture.name + ': surface camera was ignored',
                );
                const surfacePasses = passes.slice(surfaceStart);
                check(
                  !surfacePasses.some(
                    (pass) =>
                      pass.name.startsWith('WG HDR composition') && pass.colorAttachments === 2,
                  ),
                ...
                equal(
                  backend.capture(),
                  beforeSurface,
                  fixture.name + ' secondary restore GPU target',
                );
                check(
                  viewport[0] === 73 &&
                    viewport[1] === 45 &&
