export interface ComputeSetup {
  groups: number;
  makeBuffer: (size: number, bufferUsage?: GPUBufferUsageFlags) => GPUBuffer;
...
export function setupCompute(
  device: GPUDevice,
  module: GPUShaderModule,
  cap: number,
  bindEntries: GPUBindGroupLayoutEntry[],
  slots: number,
  drawItemU32: number,
): ComputeSetup {
  const groups = Math.ceil(cap / 64),
    usage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC;
  const makeBuffer = (size: number, bufferUsage: GPUBufferUsageFlags = usage) =>
