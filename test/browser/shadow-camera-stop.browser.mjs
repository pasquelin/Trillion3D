...
import { ENGINES } from '../../scripts/mesure/optionsCote.mjs';
...
try {
  ...
  const stop = await page.evaluate(
    async ({ sdkUrl, width, height }) => {
      ...
      const { poseAt, VIEWS } = await import('/mesure/poses.mjs');
      ...
      window.scene = scene;
      // The bench sun (`scripts/mesure/lampes.mjs`), with its shadow.
      scene.addLight({
        id: 'sun',
        kind: 'directional',
