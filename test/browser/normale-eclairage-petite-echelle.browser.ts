...
// `test/justesse/normale-eclairage-petite-echelle.ts`.
//
// node --experimental-strip-types test/browser/normale-eclairage-petite-echelle.browser.ts
import assert from 'node:assert/strict';
import { construireCas, ecart } from '../justesse/normaleEclairageCas.ts';
...
console.log(
  `OK: ${lignes.length} cases, one run of the lighting texts — see` +
    ' test/browser/normale-eclairage-petite-echelle.browser.ts',
);
