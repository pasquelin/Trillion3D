...
//
//   node --experimental-strip-types test/browser/water-pass-webgpu.browser.mjs
import assert from 'node:assert/strict';
import { preuveDansLaPage, preuveSaine } from '../appui/preuvePageMoteur.mjs';
import { BACKGROUND, GROUND, WATER } from '../appui/waterPassCases.mjs';

const resultat = await preuveDansLaPage('waterPassPage.mjs', 'waterPass', 'Water pass');
...
const srgb = (c) => Math.round((c <= 0.0031308 ? c * 12.92 : 1.055 * c ** (1 / 2.4) - 0.055) * 255);
...
const expected = { basin, 'declared-deeper': basin, 'nothing-behind': nothing, blend };
const TOLERANCE = 3;
