...
//
// node --experimental-strip-types test/browser/inverse-transposee-petite-echelle.browser.ts
...
import {
  ...
  empaqueteCas,
  veriteTerrain,
} from '../justesse/inverseTransposeCas.ts';
...
console.log(
  `OK: ${lignes.length} cases, one run of the WGSL kernel — see` +
    ' test/browser/inverse-transposee-petite-echelle.browser.ts',
);
