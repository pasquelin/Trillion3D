...
test('the packed distribution is self-contained at its declared boundaries', () => {
  ...
  try {
    ...
    assert.ok(statSync(join(root, 'package/dist/sdk-node/cli.ts')).mode & 0o111);
    ...
    const probe = spawnSync(
      ...
      [
        '--input-type=module',
        '--eval',
        "const sdk=await import('web-geometry');const p=await sdk.getSdkProvenance();if(!sdk.hierarchyUpdateBatch||!sdk.prepare||!p.files['dist/sdk/node.ts'])process.exit(2);for(const path of ['/core','/node','/browser'])try{await import('web-geometry'+path);process.exit(3)}catch(e){if(e.code!=='ERR_PACKAGE_PATH_NOT_EXPORTED')process.exit(4)}",
      ],
