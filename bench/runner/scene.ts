...
export const sceneDerived = (scene: string, assets: string = ASSETS) =>
  join(assets, `${scene}-derived`);

const cachePret = (scene: string, assets: string) =>
...
export function scenesOf(flags: Map<string, string>, assets: string = ASSETS) {
 ...
 */
export function gltfUrlIn(assets: string, scene: string) {
...
export const sceneGltf = (scene: string) => gltfUrlIn(ASSETS, scene);
