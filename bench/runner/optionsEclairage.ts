...
export function lightingSettings(
  ...
  return {
    ...
    // Contract lights placed by generic rule in `lampes.ts`: count, shadow status, and motion.
    ...
    // `--soleil` adds the generic directional light from `lampes.ts` with cascades.
