...
import { dag, racine, type DagPage } from './support/dagCut.ts';
...
function differencesAnneau(attendu: string[], obtenu: string[], name: string) {
  ...
  if (c.nombre)
    c.premier =
      ...
      '`packages/sdk-browser/src/world/render/draw.ts` calls it only when the network is idle, after a delay';
  return c;
}
